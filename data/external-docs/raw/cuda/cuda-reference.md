...
CUDA kernels are launched using triple angle-brackets `<<<blocks, threads>>>` defining grid and workgroup dimensional maps.
...
void launchKernel(float *data, int n) {
    ...
    // Launch doubleValueKernel with gridSize blocks and blockSize threads
    doubleValueKernel<<<gridSize, blockSize>>>(data, n);
...
void executeConcurrentStreams(float *hostData, int n) {
    ...
    doubleValueKernel<<<1, 256, 0, stream1>>>(devData1, n);
    ...
    doubleValueKernel<<<1, 256, 0, stream2>>>(devData2, n);
