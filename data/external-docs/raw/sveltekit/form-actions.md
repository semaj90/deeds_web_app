### Form Actions
Form actions in SvelteKit provide a clean way to handle POST requests from HTML <form> elements, typically for data mutations.
...
* **Named Actions**: Multiple actions can be defined (e.g., login, register). These are invoked using the action attribute in the form: <form action="?/login" method="POST">.
