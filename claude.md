...
- **OpenCode skills vs instructions**: `.opencode/skills/<name>/SKILL.md` (with `name`+`description` frontmatter) = on-demand agent-requested context. `.opencode/command/<name>.md` = slash commands invoked via `/name`. `instructions` array in `opencode.jsonc` = permanent system context (every session). Do NOT put command files in `instructions` — Gemma4 reads them as tasks. Skill routing table lives in `docs/ai-os/opencode-skill-routing.md` (keyword → skill mapping injected as permanent instruction).
