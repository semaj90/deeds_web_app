...
> Directory: vscode-extension/src
...
| Gate | Status | Detail |
|------|--------|--------|
| G17 | ❌ FAIL | 2/8 files — use env.server.ts getters |

_Gates checked: G17. Run `npm run index:codebase:fast && npm run agents:write` to refresh._
## Todos + Enhancements

- **[HIGH]** [G17] Fix **G17** No hardcoded localhost URLs: 2/8 files — use env.server.ts getters
...
- `wiki_note_lookup({ query: "vscode-extension src", limit: 5 })` — KAG narrative + audit score
- `audit_hotspots({ limit: 10 })` — if this dir is failing gates, surfaces the broader hotspot set
- `read_file({ filePath: "vscode-extension/src/<file>" })` — fetch any file's contents (sandboxed to src/)
