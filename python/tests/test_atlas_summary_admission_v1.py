...
def test_all_claims_admit_makes_the_summary_eligible_and_binds_exact_text_and_revisions(monkeypatch) -> None:
    stub_db(monkeypatch)
    r = admit_summary_v1(TS["chunkId"], TS["chunkEvidenceRevision"], CLAIM, judge("SUPPORTED"), MODEL)
...
def test_a_single_non_admit_claim_makes_the_whole_summary_ineligible(monkeypatch) -> None:
    ...
    for verdict in ("UNSUPPORTED_CLAIM", "PARTIALLY_SUPPORTED", "UNKNOWN"):
        r = admit_summary_v1(TS["chunkId"], TS["chunkEvidenceRevision"], CLAIM, judge(verdict), MODEL)
...
def test_judge_failure_is_review_never_eligible(monkeypatch) -> None:
    stub_db(monkeypatch)
    r = admit_summary_v1(TS["chunkId"], TS["chunkEvidenceRevision"], CLAIM, lambda _m: "garbage", MODEL)
...
def test_wrong_chunk_revision_fails_closed_before_any_report(monkeypatch) -> None:
    stub_db(monkeypatch)
    try:
        admit_summary_v1(TS["chunkId"], "sha256:" + "0" * 64, CLAIM, judge("SUPPORTED"), MODEL)
...
def test_empty_summary_has_no_claims_and_is_not_eligible(monkeypatch) -> None:
    stub_db(monkeypatch)
    r = admit_summary_v1(TS["chunkId"], TS["chunkEvidenceRevision"], "  ", judge("SUPPORTED"), MODEL)
...
def test_changed_text_changes_the_bound_output_hash_and_seal(monkeypatch) -> None:
    stub_db(monkeypatch)
    a = admit_summary_v1(TS["chunkId"], TS["chunkEvidenceRevision"], CLAIM, judge("SUPPORTED"), MODEL)
    b = admit_summary_v1(TS["chunkId"], TS["chunkEvidenceRevision"], CLAIM + " ", judge("SUPPORTED"), MODEL)
