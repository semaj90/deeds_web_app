...
RAPIDS 26.06 API references used by this implementation:
- cugraph.Graph.from_cudf_edgelist(..., edge_attr=..., vertices=...)
- cugraph.leiden(G, max_iter=..., resolution=..., random_state=..., theta=...)
- cugraph.louvain(G, max_level=..., resolution=..., threshold=...)
"""
...
def canonicalize_undirected_edges(
    ...
    Duplicate A-B / B-A rows are aggregated. This is safe only because the
    request has already explicitly declared undirected projection semantics.
    """
...
def run_cugraph_partition(req: CommunityPartitionRequestV1) -> CommunityPartitionResponseV1:
    ...
    if canonical_edges:
        ...
        # cuGraph needs a typed empty edge list when the projection contains
