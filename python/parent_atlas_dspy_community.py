...
Community partitioning remains deterministic graph computation owned by
Neo4j GDS or the RAPIDS challenger. DSPy is used only after membership is
frozen, to label/summarize a community from exact evidence. It cannot alter
membership or invent graph edges.

The module stays import-safe when DSPy is unavailable so contract tests do not
force the optimization/training environment into the RAPIDS or LangExtract env.
"""
