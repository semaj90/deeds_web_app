...
validate metric/data contracts without forcing the optimization environment.
"""
...
class RepairQualityObservationV1:
...
@dataclass(frozen=True, slots=True)
class SystemCostObservationV1:
    latency_budget_score: float
    cache_reuse_rate: float
    tool_call_budget_score: float = 1.0
    gpu_budget_score: float = 1.0
...
def atlas_repair_quality_score_v1(observation: RepairQualityObservationV1) -> tuple[float, str, bool]:
    score = (
        0.18 * _p(observation.retrieval_recall_at_5)
        + 0.18 * _p(observation.localization_recall_at_5)
        + 0.14 * _p(observation.exact_evidence_coverage)
        + 0.16 * float(observation.targeted_tests_passed)
        + 0.11 * float(observation.typecheck_passed)
        + 0.11 * float(observation.regression_free)
        + 0.07 * _p(observation.patch_minimality)
        + 0.05 * (1.0 - _p(observation.false_edit_rate))
    )
    failures: list[str] = []
    if not observation.targeted_tests_passed:
        failures.append("TARGETED_TESTS_FAILED")
    if not observation.typecheck_passed:
        failures.append("TYPECHECK_FAILED")
    if not observation.regression_free:
        failures.append("REGRESSION_DETECTED")
    if observation.exact_evidence_coverage < 0.8:
        failures.append("LOW_EXACT_EVIDENCE_COVERAGE")
    return round(score, 6), ("All hard repair quality gates passed." if not failures else "; ".join(failures)), not failures


def atlas_system_cost_score_v1(observation: SystemCostObservationV1) -> float:
    return round(
        0.40 * _p(observation.latency_budget_score)
        + 0.30 * _p(observation.cache_reuse_rate)
        + 0.15 * _p(observation.tool_call_budget_score)
        + 0.15 * _p(observation.gpu_budget_score),
        6,
    )


def atlas_repair_score_v1(observation: RepairMetricObservationV1) -> tuple[float, str]:
    """GEPA-facing score. Operational efficiency cannot rescue a failed repair."""
    quality_score, feedback, hard_gate_passed = atlas_repair_quality_score_v1(observation.quality)
    cost_score = atlas_system_cost_score_v1(observation.cost)
    score = 0.0 if not hard_gate_passed else 0.9 * quality_score + 0.1 * cost_score
...
def build_repair_program_v1() -> Any:
    ...
    class RepairProgramV1(dp.Module):
        ...
        def forward(self, failure: str, context_manifest: str, constraints: str) -> Any:
            diagnosis = self.diagnose(failure=failure, context_manifest=context_manifest, constraints=constraints)
