...
def bind_and_adapt() -> None:
    ...
    report = {
        ...
        "evaluationUnit": "SOURCE_FILE",
        "evaluationUnitNote": "packet_key is 1:1 with source_ref in atlas_packets; ~82% of bindings are source_ref-level, so this is file-level gold, not packet/chunk-level. A packet-level set must be built separately once lineage coverage improves.",
        "poolDepthPerRun": POOLED_K,
        "runs": [*DENSE_RUNS, "LEXICAL"],
        "queuePath": str(QUEUE_OUT.relative_to(ROOT)).replace("\\", "/"),
        "queueBytes": QUEUE_OUT.stat().st_size,
        "queueSha256": checksum(QUEUE_OUT.read_bytes()),
        "provenancePath": str(PROVENANCE_OUT.relative_to(ROOT)).replace("\\", "/"),
        "provenanceSha256": checksum(PROVENANCE_OUT.read_bytes()),
        "querySetChecksum": checksum("".join(sorted(q["queryPacketKey"] for q in queue)).encode("utf-8")),
        "gradedCount": 0, "reviewStatus": "PENDING",
