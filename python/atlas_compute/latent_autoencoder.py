...
class NestedAutoencoderConfig:
    ...
    latent64_dim: int = 64
    reconstruction_128_weight: float = 1.0
    reconstruction_64_weight: float = 0.75
    ...
    def validate(self) -> None:
        ...
        if self.latent128_dim != 128 or self.latent64_dim != 64:
            raise ValueError("reference nested dimensions must be 128 and 64")
        if self.latent64_dim > self.latent128_dim:
            raise ValueError("latent64 cannot exceed latent128")
        for name in (
...
class NestedSemanticAutoencoder(nn.Module):
    """768 -> 128, with latent64 == latent128[..., :64]."""
    ...
    def __init__(self, config: NestedAutoencoderConfig | None = None) -> None:
        ...
        self.encoder = nn.Sequential(
            ...
            nn.GELU(),
            nn.Linear(self.config.hidden_dim, self.config.latent128_dim),
    ...
    def encode(self, semantic_768: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        latent128 = F.normalize(self.encoder(semantic_768), p=2, dim=-1)
    ...
    def forward(self, semantic_768: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        latent128, latent64 = self.encode(source)
...
def nested_autoencoder_loss(
    ...
    total = (
        ...
        cfg.cosine_weight * (cos128 + cos64) +
...
def build_training_receipt(
    *,
    ...
    return {
        "schema": "atlas.nested-semantic-autoencoder-training-receipt.v1",
