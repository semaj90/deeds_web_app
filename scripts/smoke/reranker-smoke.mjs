...
async function mxbaiRerank(query, candidates) {
  const payload = {
    query,
    candidates: candidates.map(c => ({ packet_key: c.packet_key, text: c.text_for_rerank })),
    batch_size: 8,
  };
...
async function main() {
  ...
  try {
    ...
    const candidates = dedupedHits.map(h => {
    ...
    const finalCandidates = [...candidates]
      .map(c => ({ ...c, ...(rerankedMap.get(c.packet_key) ?? { rerank_score: -99, rerank_rank: 99 }) }))
    ...
    for (const c of finalCandidates) {
      ...
      console.log(`${c.ann_rank} -> ${c.rerank_rank} ${deltaStr} ann=${c.ann_score.toFixed(4)} ce=${c.rerank_score.toFixed(4)} ${c.source_ref?.slice(-60) ?? '?'}`);
    }

    const top5Keys = rerankRes.ranked.slice(0, FINAL_TOP).map(r => r.packet_key);
    const verifyRes = await client.query(
      `SELECT packet_key FROM atlas_packets WHERE packet_key = ANY($1::text[])`,
      [top5Keys]
    );
    const joined = verifyRes.rows.length;
    console.log(`\nTop-${FINAL_TOP} reranked packets joined to atlas_packets: ${joined}/${FINAL_TOP}`);
    console.log(`Reranker latency: ${rerankRes.latency_ms.toFixed(0)}ms`);
    console.log(`VRAM peak: ${rerankRes.vram_peak_mb.toFixed(0)}MB`);

    if (joined !== FINAL_TOP) process.exitCode = 1;
  } finally {
    client.release();
    await pool.end();
  }
