...
(async () => {
  try {
    ...
    if (featMap.feature !== 'ace-context') {
      ...
      if (String(featMap.feature).toLowerCase() !== 'ace-context')
        throw new Error('feature-map feature mismatch');
    }
    // featPatch should have patchCard or patchCard.type
    const fp = featPatch.patchCard || featPatch;
    if (!fp.type && !(fp.patchCard && fp.patchCard.type))
      throw new Error('feature patch-card missing type');

    // ace-patch-card should include id or content_hash
    if (
      !acePatch.id &&
      !acePatch.content_hash &&
      !(acePatch.patchCard && (acePatch.patchCard.id || acePatch.patchCard.content_hash))
    ) {
