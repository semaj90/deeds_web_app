...
async function readNdjsonMap(path, keyField='card_id'){
  ...
  try{
    ...
    for await (const line of rl){
      ...
      try{ const obj = JSON.parse(line); const key = obj[keyField] || obj.id; if(key) map.set(key, obj); }catch(e){}
    }
  }catch(e){}
  return map;
}
...
async function main(){
  ...
  const summariesRaw = [];
  try{
    const rl = readline.createInterface({ input: createReadStream(SUMMARIES), crlfDelay: Infinity });
    for await (const line of rl){ if(!line.trim()) continue; try{ summariesRaw.push(JSON.parse(line)); }catch(e){} }
  }catch(e){ console.error('No summaries file found at', SUMMARIES); return process.exit(1); }
  ...
  for(const s of summariesRaw){
    ...
    const isDry = tags && tags.includes('source:dry-run') || (typeof s.summary==='string' && s.summary.startsWith('DRY_RUN'));
  ...
  for(const [card, entries] of byCard.entries()){
    ...
    merged.push({card_id: card, sourceRef: pick.sourceRef||null, summary: pick.summary||'', keywords: pick.keywords||[], tags: pick.tags||[], file: pick.file||null});
  }
  ...
  for(const m of merged){
    const existing = existingMap.get(m.card_id) || existingMap.get(m.card_id.replace(/^file:/,'')) || null;
    const vector = existing && Array.isArray(existing.vector) ? existing.vector : [];
    const point = { id: m.card_id, vector, payload: { card_id: m.card_id, sourceRef: m.sourceRef, summary: m.summary, keywords: m.keywords, tags: m.tags, file: m.file } };
