export function normalizeTaskPayload(input) {
  ...
  if (typeof obj.description === 'string' && obj.description.trim().length > 0) {
  ...
  } else if (obj?.context?.summary && typeof obj.context.summary === 'string' && obj.context.summary.trim()) {
    ...
    if (typeof obj.action === 'string' && obj.action.trim()) parts.push('Action: ' + obj.action.trim());
  ...
  out.context.recent_memory = Array.isArray(out.context.recent_memory) ? out.context.recent_memory : (obj.recent_memory ? [obj.recent_memory] : []);
  out.context.ace_packet_key = out.context.ace_packet_key || obj.ace_packet_key || 'ace:packet:latest';
  ...
  out.context.sourceRefs = Array.isArray(out.context.sourceRefs) ? out.context.sourceRefs : (Array.isArray(obj.sourceRefs) ? obj.sourceRefs : []);
  out.context.featureLabels = Array.isArray(out.context.featureLabels) ? out.context.featureLabels : (Array.isArray(obj.featureLabels) ? obj.featureLabels : []);
  out.context.domainTopology = out.context.domainTopology || obj.context?.domainTopology || {};
  out.context.retrievalLanes = Array.isArray(out.context.retrievalLanes) ? out.context.retrievalLanes : (Array.isArray(obj.retrievalLanes) ? obj.retrievalLanes : []);
  out.context.graphSummary = out.context.graphSummary || obj.context?.graphSummary || {};
  out.context.qdrantCollections = Array.isArray(out.context.qdrantCollections) ? out.context.qdrantCollections : (Array.isArray(obj.qdrantCollections) ? obj.qdrantCollections : []);
  out.context.embeddingModel = out.context.embeddingModel || obj.embeddingModel || '';
  out.context.postgresSchemaVersion = out.context.postgresSchemaVersion || obj.postgresSchemaVersion || '';
  out.context.migrationState = out.context.migrationState || obj.migrationState || '';

  // constraints
  out.constraints = Array.isArray(obj.constraints) ? obj.constraints.map(String) : defaults.constraints.slice();

  // expected_output
  out.expected_output = (obj.expected_output && typeof obj.expected_output === 'object') ? Object.assign({}, defaults.expected_output, obj.expected_output) : Object.assign({}, defaults.expected_output);
...
export function validateTaskPayload(payload) {
  ...
  if (!payload.description || typeof payload.description !== 'string' || payload.description.trim().length === 0) return false;
