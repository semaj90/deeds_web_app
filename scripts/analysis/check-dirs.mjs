...
const targets = ['sveltekit-frontend','models','node_modules','docs','scripts','package-lock.json','yarn.lock','pnpm-lock.yaml','.cache','.venv','backups','archive','artifacts','claude-mem','.git'];
...
fs.writeFileSync(path.join(ROOT,'.tmp','check-dirs.json'), JSON.stringify({ generated: new Date().toISOString(), items: out }, null, 2));
console.log('WROTE .tmp/check-dirs.json');
