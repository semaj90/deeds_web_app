#!/usr/bin/env node
/**
 * redis-semantic-packet-manager.mjs
 *
 * Regex + NLP JavaScript script to manage Redis packet semantics.
 ...
 *
 * Usage:
 *   node scripts/redis-semantic-packet-manager.mjs --source graphify --action "build_tools"
 *   node scripts/redis-semantic-packet-manager.mjs --source todo --action "generate_prompt"
 *   node scripts/redis-semantic-packet-manager.mjs --source redis --pattern "ace:*" --semantic extract
 */
...
class PromptPacketBuilder {
  ...
  fromGraphifyOutput(graphifyPath) {
    try {
  ...
  extractDescriptionAfter(text, startIndex) {
...
class RedisPacketManager {
  static encodeToolPacket(packet) {
  ...
  static encodePromptPacket(packet) {
  ...
  static decodeToolPacket(redisPacket) {
    return JSON.parse(redisPacket.value);
  }

  static decodePromptPacket(redisPacket) {
    return JSON.parse(redisPacket.value);
  }

  static formatForRedis(packet) {
...
function generatePromptFromSource(source) {
...
function extractSemantics(pattern) {
...
function printHelp() {
  console.log(`Usage: node redis-semantic-packet-manager.mjs [options]
...
Examples:
  ...
  node redis-semantic-packet-manager.mjs \\
