        ...
        body: JSON.stringify({
          ...
          filter: {
            should: [
              ...
              { key: 'canonicalSourceRef', match: { value: canonical.toLowerCase() } },
              { key: 'canonicalSourceRef', match: { value: stripped.toLowerCase() } },
              { key: 'file_path', match: { value: canonical } },
              { key: 'file_path', match: { value: stripped } },
              { key: 'file_path', match: { value: canonical.toLowerCase() } },
              { key: 'file_path', match: { value: stripped.toLowerCase() } },
              { key: 'packet_key', match: { value: row.packet_key } },
              { key: 'packetKey', match: { value: row.packet_key } },
            ],
          },
        }),
        signal: AbortSignal.timeout(10_000),
