...
if (LIMIT > 0) {
  ...
  // files = files.slice(0, LIMIT); // moved to payloads.slice
}
...
for (const file of files) {
  const rel = file.rel ?? '';
  if (!rel.startsWith('sveltekit-frontend/src/')) continue;
...
if (WRITE) {
  ...
  // Use codebase_chunks_768 as the primary target for codebase tagging
  ...
  try {
    let patched = 0;
    let missesCount = 0;
    // Apply limit to THE ACTUAL WRITE LOOP
    const writeBatch = LIMIT > 0 ? payloads.slice(0, LIMIT) : payloads;
    
    for (const p of writeBatch) {
      ...
      if (points.length === 0) {
        ...
        const pt = points[0];
        console.log(`Patching Qdrant point ${pt.id} [${p.path}]`);
        ...
        if (res.ok) {
          ...
          console.error(`  Failed to patch point ${pt.id}:`, await res.text());
        }
    }
    console.log(`Successfully patched ${patched} points in Qdrant.`);
  } catch (err) {
