...
async function main() {
  ...
  if (!inputFile) {
    console.error('\n[Error] Usage: node bitfrost-packet-upsert-optimized.mjs <input.ndjson> [--dry-run] [--cleanup] [--resume=<batch>]');
    process.exit(1);
  }
  ...
  try {
    ...
    // Cleanup old keys if requested
    if (CLEANUP) {
      console.log('[Cleanup] Removing stale bifrost:* keys...');
      const oldKeys = await redis.keys('bifrost:*');
      if (oldKeys.length > 0) {
        await redis.del(...oldKeys);
        console.log(`[Cleanup] Removed ${oldKeys.length} keys`);
      }
    }
