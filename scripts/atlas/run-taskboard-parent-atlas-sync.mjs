 ...
 *   3. [--with-codebase] codebase feature map + feature graph
 *   4. [--with-graphify] graphify-batch-karpathy-analysis (GDS + Karpathy + summaries)
 *   5. [--with-kmeans]   graphify:feature-labels + domain-topology (KMeans Neo4j sync)
 *   6. [--with-feature-gap] atlas:feature-gap (isolated / under-linked node report)
 *   7. atlas-parent-indexing
...
try {
  ...
  // ── Step 3: codebase semantic map + feature graph ────────────────────────
  if (WITH_CODEBASE) {
    const mapArgs = [];
  ...
  // ── Step 4: graphify → GDS → Karpathy GPU blend → Qdrant → Redis ────────
  if (WITH_GRAPHIFY) {
    const graphifyArgs = [];
  ...
  // ── Step 5: KMeans feature labels + domain topology → Neo4j sync ─────────
  if (WITH_KMEANS) {
  ...
  // ── Step 6: feature-gap recommendations (isolated / under-linked nodes) ──
  if (WITH_FEATURE_GAP) {
  ...
  // ── Step 7: parent atlas indexing ────────────────────────────────────────
  runNodeScript('scripts/atlas-parent-indexing.mjs', ['--apply']);
  mark('atlas-parent-indexing', 'ok');

  // ── Step 8: validation + consistency audit ────────────────────────────────
