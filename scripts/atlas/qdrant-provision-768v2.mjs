#!/usr/bin/env node
/**
 * qdrant-provision-768v2.mjs
 *
 * 768-dim variant of qdrant-provision-384v2.mjs. 384 is not a valid
 * embeddinggemma Matryoshka truncation boundary (768/512/256/128 only) —
 * see src/lib/server/vector/lane-registry.ts.
 *
 * Provisions the codebase_chunks_768_v2 Qdrant collection according to the
 ...
 *
 * Usage:
 *   node scripts/atlas/qdrant-provision-768v2.mjs --dry-run
 *   node scripts/atlas/qdrant-provision-768v2.mjs --apply
 *   npm run atlas:qdrant:provision:768v2:dry
 *   npm run atlas:qdrant:provision:768v2:apply
 */
...
const TARGET_COLLECTION = 'codebase_chunks_768_v2';
...
async function main() {
  ...
  if (DRY_RUN) {
    ...
    log('  npm run atlas:qdrant:provision:768v2:apply');
    process.exit(0);
  }
  ...
  // Use a zero-filled vector on the contract's first required vector; the
  // sentinel is excluded from retrieval via payload filter. Built from the
  // contract dynamically (not hardcoded to a specific name/dimension) so this
  // stays correct if the contract's required vector or its size ever changes.
  log('Writing contract metadata sentinel…');
  const sentinelVectorName = contract.requiredVectors[0];
  const sentinelVectorDim = contract.namedVectors[sentinelVectorName];
  const zeroVector = new Array(sentinelVectorDim).fill(0);
  await qdrantPost(`/collections/${TARGET_COLLECTION}/points`, {
    points: [
      {
        id: 0,
        vector: { [sentinelVectorName]: zeroVector },
        payload: {
