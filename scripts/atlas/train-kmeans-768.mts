#!/usr/bin/env node
/**
 * Phase 2, Step 9: K-means Clustering on 768-dim Vectors
 *
 * 768-dim variant of train-kmeans-384.mts. 384 is not a valid embeddinggemma
 * Matryoshka truncation boundary (768/512/256/128 only) — see
 * src/lib/server/vector/lane-registry.ts and
 * src/lib/server/vector/embeddinggemma-prefix384.ts for the contract.
 *
 ...
 *
 * Reads from a 768-dim parquet snapshot (see
 * scripts/atlas/duckdb/freeze-vector-snapshot-5k-768.mts to generate one from
 * atlas_packets.embedding, halfvec(768), canonical embeddinggemma output).
 *
 * Usage:
 *   npx tsx train-kmeans-768.mts [--verbose]
 */
...
const SNAPSHOT_PATH = path.join(__dirname, '../../data/atlas-ml/snapshot_5k_768dim.parquet');
...
async function trainKMeans(): Promise<void> {
  ...
  try {
    ...
    // Store centroids to Redis (namespaced kmeans768:* to avoid colliding
    // with the legacy 384-dim centroid:kmeans:* keys)
    if (verbose) console.log('[K-means] Storing centroids to Redis...');

    for (let k = 0; k < K; k++) {
      const key = `centroid:kmeans768:${k}`;
    ...
    // Verify storage
    const stored = await redis.keys('centroid:kmeans768:*');
