...
/**
 * CREATE QDRANT COLLECTION: codebase_chunks_768
 *
 * 768-dim variant of create-qdrant-codebase-384.mjs. 384 is not a valid
 * embeddinggemma Matryoshka truncation boundary (768/512/256/128 only) —
 * see src/lib/server/vector/lane-registry.ts.
 *
 * Creates a clean 768-dim Qdrant collection for embeddinggemma vectors.
 * This is the canonical mirror for semantic search.
 *
 * ⚠️ SAFETY: codebase_chunks_768 is the LIVE production collection
 * (105,761+ points as of this writing). Unlike the 384-dim script this
 * was ported from — which targeted an always-empty experimental
 * collection — this script REFUSES to delete a non-empty collection
 * unless --force is passed explicitly. Do not pass --force against a
 * populated production collection without a verified backup/snapshot.
 *
 * Collection schema:
 * - Named vector: content (768-dim, Cosine distance)
 * - Named vector: summary (768-dim, Cosine distance)
 * - Payload indexes: packet_key, source_ref, feature_id, som_cluster, etc.
 *
 * Usage:
 *   node scripts/atlas/create-qdrant-codebase-768.mjs [--skip-if-exists] [--force]
 */
...
try {
  ...
  if (collectionExists) {
    ...
    existingPointsCount = data.result?.points_count ?? 0;
    console.log(`   ⚠️  Collection "${COLLECTION_NAME}" already exists (${existingPointsCount} points)`);
    report.steps.push({
      name: 'check_exists',
      status: 'SKIPPED',
      reason: 'Collection already exists',
      points_count: existingPointsCount
    });
    ...
    if (skipIfExists) {
      ...
      fs.writeFileSync(
        path.resolve(TMP_DIR, 'qdrant-768-create-report.json'),
        JSON.stringify(report, null, 2)
      );
      process.exit(0);
    }

    if (existingPointsCount > 0 && !force) {
      console.error(`   ❌ REFUSING to delete non-empty collection (${existingPointsCount} points).`);
      console.error(`      This is the live production 768-dim collection. Pass --force to override,`);
      console.error(`      only after verifying you have a snapshot/backup.\n`);
      report.status = 'REFUSED';
      report.steps.push({
        name: 'safety_guard',
        status: 'REFUSED',
        reason: `Non-empty collection (${existingPointsCount} points) requires --force`
      });
      fs.writeFileSync(
        path.resolve(TMP_DIR, 'qdrant-768-create-report.json'),
        JSON.stringify(report, null, 2)
...
// ── Step 3: Create collection with 768-dim vectors ──────────────────────
console.log('Step 3: Creating collection with 768-dim vectors...');
...
// ── Write report ──────────────────────────────────────────────────────
const reportPath = path.resolve(TMP_DIR, 'qdrant-768-create-report.json');
