...
async function runSmoke() {
  ...
  try {
    ...
    // Process lookup returns exit code 1 if no process found, which is fine
  ...
  // Deduce status
  if (profile.redisConnected && profile.qdrantConnected) {
    profile.status = 'PASS';
    console.log('\n🎉 Workstation VRAM & Pipeline Hygiene Smoke Check PASSED!');
    process.exit(0);
  } else {
    profile.status = 'FAIL';
    console.error('\n🔴 Workstation VRAM & Pipeline Hygiene Smoke Check FAILED due to essential service offline.');
    process.exit(1);
  }
}
