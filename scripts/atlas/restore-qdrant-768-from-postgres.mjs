...
/**
 * RESTORE QDRANT 768-DIM FROM POSTGRES CANONICAL
 *
 * 768-dim variant of restore-qdrant-384-from-postgres.mjs. 384 is not a
 * valid embeddinggemma Matryoshka truncation boundary (768/512/256/128
 * only) — see src/lib/server/vector/lane-registry.ts.
 *
 * Restores codebase_chunks_768 Qdrant collection from Postgres canonical packets.
 * Read-only from Postgres, upserts into Qdrant.
 *
 * Only processes rows where atlas_packets.embedding IS NOT NULL (halfvec(768),
 * canonical embeddinggemma output — as of this writing 61,659/61,659 rows
 * already populated, so a full restore run is a rebuild/repair tool, not a
 * required backfill).
 *
 * Usage:
 *   node scripts/atlas/restore-qdrant-768-from-postgres.mjs [--dry-run]
 *   node scripts/atlas/restore-qdrant-768-from-postgres.mjs --apply [--batch=100]
 */
...
console.log('\n♻️  RESTORE QDRANT 768-DIM FROM POSTGRES\n');
...
try {
  ...
  console.log(`   ✓ Found ${eligibleCount} rows with ${SOURCE_COLUMN}\n`);
  report.steps.push({
    name: 'count_eligible',
    status: 'OK',
    count: eligibleCount
  ...
  fs.writeFileSync(
    path.resolve(TMP_DIR, 'qdrant-768-restore-report.json'),
  ...
  for (let i = 0; i < numBatches; i++) {
    ...
    const rows = await pool.query(`
      SELECT
        ...
        kmeans_cluster,
        payload,
        ${SOURCE_COLUMN}::text AS source_embedding,
        summary,
        updated_at
      FROM atlas_packets
      WHERE ${SOURCE_COLUMN} IS NOT NULL
    ...
    const points = rows.rows.map((row) => ({
      id: row.packet_id,
      vector: {
        content: parsePgVector(row.source_embedding),
        summary: parsePgVector(row.source_embedding)
      },
      payload: {
...
const reportPath = path.resolve(TMP_DIR, 'qdrant-768-restore-report.json');
