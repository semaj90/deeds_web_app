...
/**
 * PHASE 85 P6: REBUILD GEMMA4 SUMMARIES + 768-DIM EMBEDDINGS
 *
 * 768-dim variant of rebuild-gemma4-summaries-384.mjs. 384 is not a valid
 * embeddinggemma Matryoshka truncation boundary (768/512/256/128 only) —
 * see src/lib/server/vector/lane-registry.ts. Ollama's embeddinggemma
 * /api/embeddings already returns native 768-dim vectors, so this variant
 * is simpler than the 384 original: no truncation step needed at all.
 *
 * Populates missing summaries and 768-dim embeddings for canonical storage.
 *
 * Flow:
 * 1. Select chunks missing summary_text or summary_embedding (halfvec(768))
 ...
 * 5. Call embeddinggemma for summary_embedding (native 768-dim, no truncation)
 ...
 *
 * Usage:
 *   node scripts/atlas/rebuild-gemma4-summaries-768.mjs                    [dry-run, default]
 *   node scripts/atlas/rebuild-gemma4-summaries-768.mjs --apply             [write to DB]
 *   node scripts/atlas/rebuild-gemma4-summaries-768.mjs --apply --sample=10 [test 10 items]
 */
...
if (!llamaProbe.ok || !ollamaProbe.ok || !qdrantProbe.ok) {
  ...
  fs.writeFileSync(
    path.resolve(TMP_DIR, 'rebuild-summaries-768-report.json'),
  ...
  for (let i = 0; i < embedBatches; i++) {
    ...
    const results = await mapLimit(batch, embedConcurrencyArg, chunk =>
      embedSummary(chunk._generated_summary).then(embedding => ({
        ...chunk,
        _embedding_768: embedding
      })).catch(err => ({
        ...chunk,
        _embedding_error: err.message
      }))
    );
    const successful = results.filter(r => r._embedding_768 && !r._embedding_error);
  ...
  for (const chunk of chunksWithEmbeddings) {
    ...
    if (!dryRun) {
      await pool.query(query, [
        ...
        JSON.stringify(validateVector768(chunk._embedding_768)),
        chunk.id
      ]);
      upsertCount++;
    }
  ...
  if (!dryRun && chunksWithEmbeddings.length > 0) {
    ...
    const points = chunksWithEmbeddings.map((chunk) => ({
      ...
      vector: {
        ...
        summary: validateVector768(chunk._embedding_768)
      },
      payload: {
    ...
    for (let i = 0; i < points.length; i += 100) {
      ...
      const upsertRes = await fetch(`${QDRANT_URL}/collections/${QDRANT_COLLECTION}/points?wait=true`, {
        method: 'PUT',
...
const reportPath = path.resolve(TMP_DIR, 'rebuild-summaries-768-report.json');
fs.mkdirSync(TMP_DIR, { recursive: true });
...
/**
 * Ollama's embeddinggemma /api/embeddings already returns the native
 * 768-dim vector — no truncation needed (unlike toVector384() in the
 * 384-dim original, which sliced+renormalized to an invalid MRL boundary).
 * This just validates the dimension came back correct.
 */
function validateVector768(vector) {
  if (!Array.isArray(vector)) return null;
  if (vector.length !== EXPECTED_DIMENSION) {
    throw new Error(`Expected ${EXPECTED_DIMENSION}-dim embedding, got ${vector.length}`);
  }
  return vector;
}
