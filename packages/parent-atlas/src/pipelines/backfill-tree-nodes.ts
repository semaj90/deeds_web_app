...
const log = {
  info: (msg: string) => console.log(`[tree-nodes] ${msg}`),
...
async function detectPacketTable(client: any): Promise<string> {
...
async function findExistingTreeNodeId(client: any, { sourceRef, packetKey, nodeType }: { sourceRef?: string; packetKey?: string; nodeType: string }): Promise<string | null> {
    ...
    for (const file of files) {
      ...
      try {
        ...
        if (error.code === '23505') {
          ...
          stats.errors.push(`Document node ${sourceRef}: ${error.message ?? String(err)}`);
        }
      }
      ...
      for (const packet of packets) {
        ...
        try {
          ...
          if (error.code === '23505') {
            ...
            stats.errors.push(`Chunk node ${packet.packet_key}: ${error.message ?? String(err)}`);
          }
        }
      }
