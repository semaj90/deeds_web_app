 ...
 *
 * TODO(GPH-17): once 8095 exposes Consiliency node_id/file_id/symbol_id for all
 * chunks, make `allow_compatibility_ids=false` the production default/gate.
 */
export function adaptAtlasAstEvidenceToStructuralInput(input: {
  ...
  const fileId = compatibilityId('file', [evidence.file_path, evidence.source_revision]);
  const chunks = evidence.chunks.map((chunk, index) => {
    ...
    const upstreamChunkId = chunk.upstream_chunk_id
      ?? compatibilityId('chunk', [upstreamNodeId, chunk.start_byte, chunk.end_byte]);

    return {
      upstream_node_id: upstreamNodeId,
      upstream_file_id: chunk.upstream_file_id ?? fileId,
  ...
  if (compatibilityNodeIds > 0) {
    diagnostics.push(`COMPATIBILITY_NODE_IDS:${compatibilityNodeIds}`);
  }
