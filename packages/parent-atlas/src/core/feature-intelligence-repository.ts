  ...
  return {
    async upsertFeature(input: FeatureV1): Promise<FeatureV1> {
      ...
      await withTransaction(pool, async (client) => {
        ...
        `, [feature.feature_id, feature.feature_key, feature.feature_label, feature.domain,
          feature.parent_feature_id ?? '', feature.status, feature.feature_revision, feature.producer_revision]);

        const featureId = row.rows[0]?.feature_id;
        if (!featureId) throw new Error('feature upsert returned no feature_id');
        if (featureId !== feature.feature_id) {
          throw new Error(`canonical feature identity contradiction: key ${feature.feature_key} maps to ${featureId}, not ${feature.feature_id}`);
        }

        await client.query('DELETE FROM atlas_feature_aliases WHERE feature_id = $1', [featureId]);
        for (const alias of feature.aliases) {
          await client.query(`
            INSERT INTO atlas_feature_aliases(feature_id, alias, normalized_alias)
            VALUES ($1, $2, lower(trim($2))) ON CONFLICT DO NOTHING
          `, [featureId, alias]);
        }
      });
      return feature;
    ...
    async insertEvidence(input: FeatureEvidenceV1): Promise<FeatureEvidenceV1> {
      ...
      await withTransaction(pool, async (client) => {
        ...
        `, [evidence.evidence_id, evidence.evidence_kind, evidence.source_ref,
          ...
          evidence.confidence, JSON.stringify(evidence.payload),
        ...
        `, [evidence.feature_id, evidence.evidence_id, evidence.relation_type,
          evidence.polarity, evidence.confidence, evidence.relationship_id ?? '']);
      });
      return evidence;
    },
    ...
    async persistRelationship(input: FeatureRelationshipV1): Promise<FeatureRelationshipV1> {
      ...
      await withTransaction(pool, async (client) => {
        ...
        `, [relationship.relationship_id, relationship.relationship_type,
          relationship.participant_count, relationship.relationship_degree,
          relationship.relationship_degree_kind, relationship.source_ref,
          relationship.source_revision, relationship.relationship_revision,
          relationship.producer_revision, relationship.confidence, JSON.stringify(relationship.metadata)]);

        await client.query('DELETE FROM atlas_relationship_members WHERE relationship_id = $1', [relationship.relationship_id]);
        await client.query('DELETE FROM atlas_relationship_cardinality WHERE relationship_id = $1', [relationship.relationship_id]);
        await client.query('DELETE FROM atlas_relationship_evidence WHERE relationship_id = $1', [relationship.relationship_id]);

        for (const [ordinal, participant] of relationship.participants.entries()) {
          await client.query(`
            INSERT INTO atlas_relationship_members (
              relationship_id, member_ordinal, role, entity_type, entity_id, entity_revision, source_ref
            ) VALUES ($1, $2, $3, $4, $5, $6, $7)
          `, [relationship.relationship_id, ordinal, participant.role, participant.entity_type,
            participant.entity_id, participant.entity_revision ?? null, participant.source_ref ?? null]);
        }
        ...
        for (const constraint of relationship.cardinality) {
          ...
          `, [relationship.relationship_id, constraint.role, constraint.min,
        ...
        const validation = await client.query<{ valid: boolean }>(
          'SELECT atlas_validate_relationship($1) AS valid', [relationship.relationship_id]);
    ...
    async upsertRelationshipEmbedding(
      ...
      await pool.query(`
        ...
        DO UPDATE SET relationship_revision = EXCLUDED.relationship_revision,
          embedding = EXCLUDED.embedding, source_checksum = EXCLUDED.source_checksum,
          view_refs = EXCLUDED.view_refs, created_at = now()
      `, [projection.relationship_id, projection.relationship_revision,
        projection.embedding_model_revision, projection.projection_revision,
        vectorLiteral(embedding), projection.source_checksum, projection.view_refs]);
      return projection;
    },

    async searchRelationshipEmbeddings(
      embedding: readonly number[], limit = 20,
      ...
      const result = await pool.query<{ relationship_id: string; distance: number }>(`
        SELECT relationship_id, embedding <=> $1::vector AS distance
        FROM atlas_relationship_embeddings
        ORDER BY embedding <=> $1::vector LIMIT $2
      `, [vectorLiteral(embedding), boundedLimit]);
      return result.rows.map((row) => ({ relationship_id: row.relationship_id, distance: Number(row.distance) }));
    },

    async findRelationshipsForEntities(
      entityIds: readonly string[], relationshipTypes: readonly string[] = [], limit = 100,
    ...
    async persistDynamicHyperedge(input: DynamicHyperedgeCandidateV1): Promise<DynamicHyperedgeCandidateV1> {
      ...
      await pool.query(`
        INSERT INTO atlas_dynamic_hyperedge_candidates (
          ...
          join_keys, source_refs, source_revisions, evidence_refs, confidence, source_snapshot_revision
        ) VALUES ($1, $2, $3, $4::jsonb, $5::jsonb, $6::text[], $7::text[], $8::text[], $9, $10)
        ON CONFLICT (dynamic_relationship_id) DO UPDATE SET
          query_id = EXCLUDED.query_id, relationship_type = EXCLUDED.relationship_type,
          participants = EXCLUDED.participants, join_keys = EXCLUDED.join_keys,
          source_refs = EXCLUDED.source_refs, source_revisions = EXCLUDED.source_revisions,
      ...
      `, [candidate.dynamic_relationship_id, candidate.query_id, candidate.relationship_type,
        JSON.stringify(candidate.participants), JSON.stringify(candidate.join_keys), candidate.source_refs,
        candidate.source_revisions, candidate.evidence_refs, candidate.confidence, candidate.source_snapshot_revision]);
      return candidate;
    },
    ...
    async insertStateReceipt(input: FeatureStateReceiptV1): Promise<FeatureStateReceiptV1> {
      ...
      await pool.query(`
        INSERT INTO atlas_feature_state_receipts (
          ...
          $12::text[], $13::text[], $14::text[], $15::text[], $16::jsonb, $17::jsonb, $18::timestamptz)
      `, [receipt.receipt_id, receipt.feature_id, receipt.feature_revision,
        receipt.evidence_snapshot_revision, receipt.state_revision, receipt.input_evidence_hash,
        receipt.evaluator_revision, receipt.state.state, receipt.state.completion, receipt.state.confidence,
        receipt.state.priority, receipt.state.blockers, receipt.state.recommendations,
        receipt.state.satisfied_evidence, receipt.state.blocking_evidence,
        JSON.stringify(receipt.state.priority_signals), JSON.stringify(receipt.state), receipt.emitted_at]);
      return receipt;
    },
  };
}
