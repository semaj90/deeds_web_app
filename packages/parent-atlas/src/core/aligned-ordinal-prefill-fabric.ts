...
export const alignedOrdinalRegistrySchema = z.object({
  ...
  for (let index = 0; index < value.rows.length; index += 1) {
    if (value.rows[index]!.ordinal !== index) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['rows', index, 'ordinal'], message: 'registry ordinals must be dense 0..N-1' });
    }
  }
  ...
  for (const projection of value.projections) {
    ...
    if (projection.row_count !== value.row_count) {
...
export const canonicalStructuralCoordinateSchema = z.object({
  ...
  if (value.end_byte <= value.start_byte) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['end_byte'], message: 'end_byte must exceed start_byte' });
  }
});
...
export const smartPacketCoordinateRefSchema = z.object({
  ...
  if (value.structural && value.structural.canonical_id !== value.canonical_id) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['structural', 'canonical_id'], message: 'structural canonical_id mismatch' });
  }
  if (value.structural && value.structural.packet_key !== value.packet_key) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['structural', 'packet_key'], message: 'structural packet_key mismatch' });
  }
});
...
export const gpuResidencyLeaseSchema = z.object({
  ...
  if (value.cuda_ipc_handle_ref !== null && value.residency !== 'CUDA') {
    ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['cuda_ipc_handle_ref'], message: 'CUDA IPC handle reference requires CUDA residency' });
  }
...
export function buildAlignedOrdinalRegistry(input: {
  registry_revision: string;
  source_snapshot_revision: string;
  tensor_snapshot: TensorSnapshotV1;
  projections: AlignedProjectionRefV1[];
  ...
  const body = {
    schema: 'atlas.aligned-ordinal-registry.v1' as const,
    registry_revision: input.registry_revision,
    source_snapshot_revision: input.source_snapshot_revision,
    row_identity_checksum: tensor.row_identity_checksum,
    row_count: tensor.row_count,
    rows: tensor.rows,
    projections,
    producer_revision: input.producer_revision,
    ordinal_is_canonical: false as const,
...
export function buildSmartPacketCoordinateRef(input: {
  packet_key: string;
  canonical_id: string;
  registry: AlignedOrdinalRegistryV1;
  structural?: CanonicalStructuralCoordinateV1 | null;
  producer_revision: string;
}): SmartPacketCoordinateRefV1 {
  ...
  const body = {
    ...
    hypergraph_artifact_id: byKind.get('HYPERGRAPH') ?? null,
    producer_revision: input.producer_revision,
    canonical_authority: false as const,
  };
...
export function buildGpuResidencyLease(input: Omit<GpuResidencyLeaseV1, 'schema' | 'lease_checksum' | 'canonical_authority'>): GpuResidencyLeaseV1 {
  const body = {
    schema: 'atlas.gpu-residency-lease.v1' as const,
    ...input,
    canonical_authority: false as const,
  };
...
export function buildCompiledPrefillReceipt(input: Omit<CompiledPrefillReceiptV1, 'schema' | 'prefill_identity_checksum' | 'canonical_authority'>): CompiledPrefillReceiptV1 {
  ...
  const identity = {
    ...
    source_snapshot_revision: input.source_snapshot_revision,
    registry_revision: input.registry_revision,
    row_identity_checksum: input.row_identity_checksum,
    ...
    adapter_revision: input.adapter_revision ?? null,
    tokenizer_revision: input.tokenizer_revision,
    prompt_template_revision: input.prompt_template_revision,
    tool_schema_revision: input.tool_schema_revision,
    evidence_artifact_checksums,
    gpu_lease_checksums,
    compiled_prefill_artifact_id: input.compiled_prefill_artifact_id,
    compiled_prefill_checksum: input.compiled_prefill_checksum,
  };
  return compiledPrefillReceiptSchema.parse({
    schema: 'atlas.compiled-prefill-receipt.v1',
    ...input,
    evidence_artifact_checksums,
    gpu_lease_checksums,
