...
const EXT_LANGUAGE = new Map<string, string>([
  ['.ts', 'typescript'],
  ['.tsx', 'typescript'],
  ['.js', 'javascript'],
  ['.mjs', 'javascript'],
...
function normalizedPathDir(value: unknown): string | null {
  const text = String(value ?? '').trim();
  if (!text) return null;
  if (!/[\\/]/.test(text)) return 'root';
  const dir = path.dirname(text.replace(/\\/g, '/')).replace(/\\/g, '/');
  return dir && dir !== '.' ? dir : null;
}

function missingFields(object: Record<string, unknown>, fields: string[]): string[] {
  return fields.filter((field) => !String(object[field] ?? '').trim());
}

export function classifyDomain(sourceRef = '', featureId = '', summary = ''): string {
...
export function classifyOntology(sourceRef = '', featureId = '', functionSymbol = ''): string {
...
export function classifyTopology(sourceRef = '', featureId = '', domainClass = ''): string {
...
export function buildSummaryContext(row: Record<string, unknown> = {}): Record<string, unknown> {
  ...
  const domainClass = firstText(
    ...
    classifyDomain(sourceRef, featureId ?? '', String(row.summary ?? '')),
    workspace.domain,
  );
...
export function formatSummaryContext(context: Record<string, unknown> = {}): string {
  return [
