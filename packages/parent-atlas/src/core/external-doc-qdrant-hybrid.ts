...
export const externalDocsHybridPointSchema = z.object({
  ...
  ast_observation_kinds: z.array(z.string().min(1)).default([]),
  langextract_classes: z.array(z.string().min(1)).default([]),
  tags: z.array(z.string().regex(/^[a-z0-9_.:-]+=[^=]+$/)).max(256).default([]),
  kmeans_cluster: z.number().int().nonnegative().nullable().default(null),
  som_cell: z.string().regex(/^\d{1,2}:\d{1,2}$/).nullable().default(null),
...
export function hybridWirePoint(point: ExternalDocsHybridPointV1): QdrantHybridWirePointV1 {
  ...
  return {
    ...
    payload: {
      ...
      ast_observation_kinds: parsed.ast_observation_kinds,
      langextract_classes: parsed.langextract_classes,
      tags: parsed.tags,
      kmeans_cluster: parsed.kmeans_cluster,
      som_cell: parsed.som_cell,
