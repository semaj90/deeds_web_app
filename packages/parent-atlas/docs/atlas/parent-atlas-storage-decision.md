...
- REST, gRPC, MCP, A2A, and QUIC are adapter layers around Parent Atlas, not part of retrieval or storage business logic.
- Keep the canonical retrieval contract in `RetrievalFacade`; route all protocol-specific behavior through the client/service boundary.
- QUIC terminates at the edge; it does not change the runtime contract.
