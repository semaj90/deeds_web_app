...
- [ ] treesitter-chunker is the current structural extraction implementation in the Python sidecar path. Tree-sitter + ast-grep are parsing/query primitives. Boundary IR AstUnit facts project into atlas_ast_nodes with packet_key null at that stage. AstUnit is the permanent canonical contract, not the third party package itself. LangExtract is the grounded NLP/extraction lane beside the structural lane, not instead of it.
- [ ] Validation commands:
...
## GS1.49 - S180-6B live discovery correction: `atlas_packets` DOES exist, `packet_qdrant_bridge` is a projection (2026-08-04)

Corrects a bad conclusion from the prior session's S180-6 Task 1 (`SESSION-181-S180-6-HANDOFF.md`), which claimed `atlas_packets`/`codebase_chunk_index` don't exist in the live database. That conclusion was derived by reading the Drizzle schema *file* and an earlier failed query — never a direct live query. Re-derived every claim from a live, read-only `docker exec` transaction against `legal-ai-postgres`/`legal_ai_db`. Zero mutations performed. Full reports: `docs/reports/session-182-s180-6b-live-packet-storage-discovery.{json,md}`.

- [x] **SQL/RUNTIME** — verified `to_regclass('public.atlas_packets')` and `to_regclass('public.packet_qdrant_bridge')` both resolve to real tables (`relkind='r'`) on the default search path. `LIVE_ATLAS_PACKETS_RELATION: PRESENT`.
- [x] **SQL** — `atlas_packets` has **61,659 rows**; `packet_key` 100% present, 100% unique, DB-enforced `UNIQUE(packet_key)` + `PRIMARY KEY(packet_id)`. `codebase_chunk_index` has **52,417 rows** (also falsely reported absent). `PREVIOUS_MISSING_TABLE_CONCLUSION: INVALIDATED`.
- [x] **SQL** — `packet_qdrant_bridge` (4,725 rows) has no FK, PK=`packet_key`, zero content/provenance columns, `matched_by='source_ref_relative_path'` for 100% of rows, and **100% of its `packet_key` values join into `atlas_packets.packet_key`**. `PACKET_QDRANT_BRIDGE_ROLE: PROJECTION_BRIDGE`, `CANONICAL_PACKET_OWNER: PROVEN` = `atlas_packets`.
- [x] **SQL** — both `drizzle.__drizzle_migrations` and `public.migrations` are **empty** (0 rows). `atlas_packets`' live 200+ column shape was never applied through either tracked migration path — built out-of-band. `ATLAS_PACKETS_MIGRATION: UNKNOWN` (not APPLIED, not UNAPPLIED — no evidence either way).
- [x] **SQL** — `atlas_packets.chunk_id`/`symbol_id` are 94.7% populated (58,365/61,659) real UUIDs, but **zero rows join** to `codebase_chunk_index` via `.id` (uuid) or `.chunk_id` (text). The real working join is `atlas_packets.source_ref = codebase_chunk_index.source_ref` (52,417 matches). `CHUNK_ID_STRUCTURAL_JOIN: FAIL_OR_ORPHANED`, `SYMBOL_ID_STRUCTURAL_JOIN: FAIL_OR_ORPHANED`, `SOURCE_REF_JOIN: WORKING_52417`.
- [x] **SQL** — `atlas_packets.tree_node_id` is live `text` with **no FK constraint** (contradicts session-181's "PROVEN JOIN, FK enforced" — that read only the Drizzle schema file, which models it as `uuid` + FK to `atlasTreeNodes`; live reality differs, matching GS1.10/GS1.47's provisional-identity findings for `tree_node_id`).
- [x] **AST** — `source_revision`/`representation_id` (expected by `qdrant-sync-payload.ts`) have no literal live column. Closest analogues: `workspace_revision`/`representation_revision` (both `NOT NULL DEFAULT 0`) and `source_representation_id`/`projection_representation_id` (both **0% populated**, exist but dead). `SOURCE_REVISION_OWNER: NOT_PROVEN`, `REPRESENTATION_ID_OWNER: NOT_PROVEN`.
- [x] **AST** — grepped 200+ live TypeScript files referencing `atlasPackets`/`atlas_packets` across `ace/`, `retrieval/`, `workers/`, `mcp/`, `routes/api/`. `atlas_packets` is heavily wired, not orphaned code.
- [x] **AST** — found `identity-worker.ts` writes/reads `atlasPackets.packet_key` (snake_case) and `.set({repository_id, directory_id, ...})` (snake_case keys) against a schema that defines the property as camelCase `packetKey` — `IDENTITY_WORKER_PACKET_KEY_WRITE: BROKEN` (fixed in GS1.50 below).
- [x] Validation commands used (all inside `BEGIN TRANSACTION READ ONLY … ROLLBACK`):
  - `docker exec legal-ai-postgres psql -U legal_admin -d legal_ai_db -c "SELECT to_regclass('public.atlas_packets'), to_regclass('public.packet_qdrant_bridge');"`
  - `docker exec legal-ai-postgres psql -U legal_admin -d legal_ai_db -c "SELECT count(*) FROM public.atlas_packets;"`
  - `docker exec legal-ai-postgres psql -U legal_admin -d legal_ai_db -c "SELECT count(*) FROM drizzle.__drizzle_migrations;"`
- [ ] NOT done this pass: no DDL, no backfill, no `graphify:daily`, no Qdrant mutation. `SESSION-181-S180-6-HANDOFF.md` and `docs/reports/session-180-s180-6-task1-identity-audit.md` are preserved as correction history but must not drive further migration/schema decisions.

## GS1.50 - S180-6C: `identity-worker.ts` Drizzle writer fixed; all `atlas_packets` writers audited; representation-field ownership traced to zero producers (2026-08-04)

Bounded follow-up to GS1.49's `IDENTITY_WORKER_PACKET_KEY_WRITE: BROKEN` finding. Full report: `docs/reports/session-183-s180-6c-writer-correctness-audit.md`. Regression test: `sveltekit-frontend/src/lib/server/workers/identity-worker.spec.ts` (3/3 pass).

- [x] **AST/RUNTIME** — root cause is worse than casing: `repository_id`/`directory_id`/`file_id`/`module_id`/`symbol_id`/`chunk_id` exist as real raw Postgres columns on `atlas_packets` but have **no Drizzle property at all** in `schema/atlas-packets.ts` (not camelCase, not snake_case — absent). Renaming alone would not have fixed this.
- [x] **RUNTIME** — fixed `identity-worker.ts`: both `.where()` clauses now use `eq(atlasPackets.packetKey, ...)` (the one field of six that does have a real Drizzle property); `buildCanonicalEnvelope()`'s packet-row reads switched to the real camelCase properties that exist (`sourceRef`, `featureId`, `directoryPath`, `featureLabel`, `qdrantPointId`, `neo4jNodeId`, `createdAt`); `.update().set({...})` reduced to the four fields that are real Drizzle columns (`featureId`, `identityLane`, `identityConfidence`, `updatedAt`) — the six unmapped identity-hierarchy fields were removed from the write rather than invented. `IDENTITY_WORKER_STATIC_FIX: PASS`.
- [x] **RUNTIME** — added a duplicate-row guard (`packet.length > 1` fails closed to quarantine instead of silently taking `packet[0]`) — `packet_key` carries a live `UNIQUE` constraint so this should be unreachable, but no defense previously existed.
- [x] **RUNTIME** — new regression test proves: (1) `eq()` is never called with `undefined`, (2) `.set()` keys are exactly `['featureId','identityConfidence','identityLane','updatedAt']`, (3) missing-target and duplicate-target both fail closed without calling `db.update`. `IDENTITY_WORKER_FIXTURE_WRITE: PASS` — **against a mocked fixture, not a live Postgres round-trip** (see next finding for why).
- [x] **AST** — found a **second, separate** bug in the same file, deliberately left unfixed (out of this fix's scope): `createPermissionManager(envelope)` then `.canWrite()` — but `permission-manager.ts`'s `createPermissionManager(pool, redisClient?)` expects a Postgres pool, not an envelope, and `PermissionManager` has **no `canWrite()` method at all** (only an unrelated async `checkAccess(...)`). This still blocks the real write path today — `processPacketIdentity()` throws before reaching `db.update()`, caught silently by the outer try/catch. `PRODUCTION_WRITER_READY: NOT_YET`. Tracked as follow-up S180-6C-2, not conflated with the Drizzle fix.
- [x] **AST** — audited all 7 `.update(atlasPackets)`/`.insert(atlasPackets)` call sites repo-wide (`som-clustering.ts`, `packet-summary-pipeline.ts`, `feature-label-enricher.ts`, `summary-freshness-checker.ts`, `routes/api/admin/batch-embeddings/embed/+server.ts`, `hyperrag-packet-pipeline.ts`, plus `identity-worker.ts`). Only `identity-worker.ts` was broken — the other 6 all correctly use `atlasPackets.packetKey` + camelCase `.set()`/`.values()` keys. `ATLAS_PACKET_WRITER_AUDIT: PASS`. One minor unrelated finding: `hyperrag-packet-pipeline.ts`'s insert passes `embeddingModel`, not a real schema column (silently dropped by Drizzle's insert builder, non-identity-breaking, not fixed).
- [x] **AST** — traced `sourceRepresentationId`/`projectionRepresentationId`: **zero writers found anywhere in the repository** — matches GS1.49's live 0%-coverage finding exactly; the columns are structurally unreachable as currently wired, not merely under-used. `REPRESENTATION_ID_OWNER: NOT_PROVEN`, `REPRESENTATION_ID_COVERAGE: 0%`.
- [x] **AST** — traced `workspaceRevision`/`representationRevision`: read/threaded through dozens of retrieval files (`hydrate-candidates.ts`, `retrieve-candidates.ts`, `search-runtime.ts`, `qdrant-packet-projection.ts`, etc.) and Zod contracts, but **none of the 7 `atlas_packets` writer call sites ever set them** — they only ever hold their `DEFAULT 0`. `SOURCE_REVISION_OWNER: NOT_PROVEN` (confirmed again; no analogue is ever written either — `workspace_revision`/`representation_revision` are the wrong granularity for `source_revision` regardless: workspace-wide/contract-version vs. per-source-occurrence).
- [x] Per operator instruction: did **not** add a `source_revision` column, did **not** default `representationId` to `'semantic_768'` anywhere.
- [ ] NOT done this pass, explicitly deferred:
  - S180-6C-2 — fix the `createPermissionManager`/`canWrite()` mismatch (needed before a true live fixture write can be proven).
  - S180-6E — structural-orphan repair design for `chunk_id`/`symbol_id` (still `FAIL_OR_ORPHANED`; do not backfill Qdrant payload `chunk_id`/`symbol_id` from these fields until resolved).
  - S180-6F/G — Qdrant reconciliation classifier + 10-point fixture migration. No Qdrant touched this pass.
  - `graphify:daily` — not run.
  - Source-ref fan-out measurement (`SELECT source_ref, COUNT(*), COUNT(DISTINCT packet_key) ... GROUP BY source_ref HAVING COUNT(*) > 1`) — `source_ref` may be one-to-many; needed before treating it as sufficient for canonical reconciliation, only as candidate discovery.

### Next bounded step (S180-6C-2, read-only-safe until the permission fix lands)
