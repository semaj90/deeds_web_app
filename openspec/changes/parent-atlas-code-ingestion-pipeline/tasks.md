...
- [x] **GPH-02** Canonical Graphify contracts — nominated existing owners in `design.md`: `treesitterChunkerChunkSchema` for revision-qualified source byte spans; `structuralSymbolNominationSchema` followed by `symbolResolutionSchema`/`symbolVersionSchema` for proposal vs canonical identity; `structuralReferenceFactSchema` for revision-qualified typed edge/reference facts, with `treesitterChunkerXrefEdgeSchema` retained as raw parser transport. No duplicate identity, embedding, ranking, or Qdrant fields added. Focused structural contract/normalizer tests and strict OpenSpec validation pass.
- [x] **GPH-03** Replacement parity — bounded replay recorded in `docs/reports/ast-replacement-parity.{json,md}` on 2026-09-24: six fixtures passed against the current `ast-grep-extractor.ts` owner and the `miniforge-nlp-sidecar /ast/chunk` candidate; required symbols missing 0, span mismatches 0, identity changes 0, malformed fixture isolated. This proves behavioral parity for the bounded corpus only; it does not select production ownership, alter lifecycle state, or authorize legacy supersession.
- [x] **GPH-04** Stable symbol identity — the existing GIS registry keeps logical `stable_symbol_id` derived independently from source revision, derives `symbol_version_id` with source revision/node/declaration evidence, and stores byte spans on the version row. `packages/parent-atlas/test/symbol-registry-identity-v1.test.mjs` proves same logical symbol across two source revisions retains its stable ID while version IDs differ; moving only the byte span changes neither ID and the span is persisted as version evidence. Creation remains gated by explicit `allow_create`; the test uses a mock pool and performs no database writes. This is code-level separation proof, not live registry readback or lifecycle promotion.
- [ ] **GPH-05** Ownership proof — enumerate AST/symbol extraction paths and establish the sole canonical production owner, identity/revision semantics, consumers, persistence, fallback, and lifecycle evidence. 2026-09-24 read-only result: `GPH_05_OWNER_PARTIAL`; see `docs/reports/ast-ownership-gph05-v1.md`. No lifecycle or runtime behavior changed.
- [ ] **GPH-06** Superseded-import guard — only after GPH-05 proves ownership, prevent new canonical callers from importing the retiring implementation; this does not perform owner cutover or lifecycle transition.
