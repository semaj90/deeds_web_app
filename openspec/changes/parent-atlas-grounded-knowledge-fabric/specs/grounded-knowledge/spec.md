...
#### Scenario: Contextual evidence is ambiguous
- **WHEN** more than one candidate matches a contextual anchor
- **THEN** the resolver returns unresolved and does not choose a source identity
...
#### Scenario: Claim projection is current but not admitted
- **WHEN** a claim passes deterministic freshness checks but no canonical admission receipt exists
- **THEN** the projection remains non-canonical and writes are false
