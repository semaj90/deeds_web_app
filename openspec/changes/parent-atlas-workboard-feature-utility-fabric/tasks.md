...
| NS-8D id suggestions (NOT applied) | `scripts/atlas/suggest-task-ids-v1.mjs` -> `.tmp/atlas/ns-8d-audit/task-id-suggestions-v1.json` + `.patch` | 2,007 suggestions across 51 files; 0 drift/source-missing skips; patch dry-run passes; task-ledger edits = 0; operator adoption remains separate |
...
1. **NS-8D adoption remains optional/operator-owned.** The fresh advisory is `.tmp/atlas/ns-8d-audit/task-id-suggestions-v1.patch` (2,007 proposed IDs across 51 files; dry-run passes). It remains unapplied; this task proves only that the read-only suggester works. Review/adoption is a separate, broad multi-ledger change and was not authorized by this proof.
...
- [x] Normalize topic key (`namespace/name[/version]`); derive `topic_id` via existing UUIDv8 (R3); title is display only. **Proven 2026-09-23:** the existing helper now canonicalizes namespace/name/version and calls shared `deriveUUID('atlas.topic-identity.v1', { taxonomyRevision, topicKey })`; it records `UUID_DERIVATION_REVISION`. Display label is excluded from identity derivation. Unit coverage verifies normalization, UUIDv8 shape, title separation, malformed path rejection, and noncanonical/no-write output. No topic table or migration was created/applied.
- [x] Version-distinct topics stay distinct where versions differ materially (e.g. TypeScript 5 vs 7). **Proven 2026-09-23:** same namespace/name with versions `5.9` and `7.0` emits distinct keys and UUIDv8 IDs in `topic-identity-v1.spec.ts`.

### WFU-02 — LanguageRegistryV1
- [ ] Language detect + registry; reuse `atlas_ast_nodes.language` and treesitter-chunker language names before adding any enum. **Read-only owner census (2026-09-23):** live `atlas_ast_nodes.parser_language` values are `typescript=9,526`, `svelte=1,477`, `markdown=150`, `sql=37`, `json=20`, `javascript=63` (11,273 total). The Node Tree-sitter provider accepts grammar IDs `typescript/tsx/javascript/jsx`; the sidecar chunker map includes `typescript/tsx/javascript/python/rust/go/java` plus React aliases; document/data owners also emit labels such as `markdown`, `jsonl`, `text`, and `ndjson`. These are overlapping but non-equivalent vocabularies, and `parser_language` is the live column name (not `language`). **Partial contract proof (2026-09-23):** added `LanguageRegistryV1` as a read-only reconciler over source-owned, namespaced vocabularies and explicitly evidenced alias bindings. It keeps content/parser/grammar/extension namespaces separate; exact explicit bindings resolve, known-but-unmapped labels return `UNMAPPED`, absent labels return `UNSUPPORTED`, and conflicting targets return `AMBIGUOUS`. Package TypeScript build and 4 focused tests pass, including deterministic output under reordered owner snapshots. No source-owner adapters or production detection are wired yet, so this task remains open; next gate is ingesting the existing vocabularies without duplicating their alias maps. **Partial owner-export proof (2026-09-25):** the Node Tree-sitter provider now exports its grammar vocabulary from the same tuple that defines its language type; the Python sidecar `/capabilities` response exposes `FILE_EXTENSION` and `SIDECAR_LANGUAGE` vocabularies plus explicit bindings derived from `_AST_LANGUAGE_EXTENSIONS`, each with a content-derived SHA-256 revision. Focused checks pass (Node 7/7; Python 1/1). These are owner snapshots only: no database vocabulary observation, registry assembly, alias promotion, parser behavior change, or persistence is claimed. Next: bind a read-only `atlas_ast_nodes.parser_language` observation with query/schema revision, then assemble the existing registry from these snapshots.

### WFU-03 — KeywordEvidenceV1
- [x] Keyword/identifier extraction and rg / ast-grep query builders; output = evidence objects, never canonical writes. **Proven 2026-09-23:** added `packages/parent-atlas/src/core/keyword-evidence-v1.ts` as a deterministic query-planning contract over the existing bounded `runExactRgJson`/`buildRgJsonArgs` and `classifyStructuralQueryV1` owners. It returns capped exact, identifier, and path terms plus fixed-string rg and non-executable AST query plans; no subprocess execution, ranking, identity creation, or persistence. Tests verify deterministic replay, path extraction, term caps, blank-query rejection, and `canonicalAuthority=false`/`writesPerformed=false`.

### WFU-04 — AST/rg evidence adapter
- [x] **WFU-04a** revision-qualified ast-grep evidence resolves deterministically to a canonical `treeNodeId` when exact evidence is sufficient, otherwise stays a typed `UNRESOLVED` / `AMBIGUOUS` / `LINEAGE_MISMATCH` (wording rewritten from "ast-grep evidence resolves `treeNodeId`"; closure does not require a high resolution rate). **Done 2026-09-23, `WFU_04_EXACT_AST_RESOLUTION_PROVEN`** (`docs/reports/wfu-04-ast-grep-tree-node-resolution-v1.json`): one pure resolver `resolveAstGrepObservationTreeNodeV1` in `packages/parent-atlas/src/core/ast-grep-tree-node-resolution-v1.ts` (the earlier exact-span function is now a wrapper over the same core, not a second algorithm). Precedence: normalized source ref (AST-bridge convention, no case folding) -> exact source revision (only `sha256:` prefix equivalence; missing/sentinel => `LINEAGE_MISMATCH`) -> `upstream_node_id` claim (must exist and contain the span; failure never falls through) -> exact span -> unique containing node (frozen, admitted) -> zero => `UNRESOLVED`, >1 => `AMBIGUOUS`; no confidence, name matching, best-overlap or first-wins. `adaptAstGrepMatches` fills `upstream_node_id` from best-overlap, so it is treated as a claim, not proof. Tests: 13 new + 2 legacy + 4 existing structural query/identity = 19/19 pass (positive: upstream id, exact span, unique containment, non-ASCII UTF-8 bytes, replay; negative: missing/wrong/sentinel revision, same span other file, same name other file, 2 containing nodes, duplicate upstream id, no candidate, overlap-only, stale revision). Read-only live census: 461 ast-grep nominations vs 565-node frozen AST snapshot = 0 resolved / 461 `UNRESOLVED` (the observation stream covers 20 files, the snapshot 6 different files, intersection 0); live `atlas_ast_nodes` has only 98 rows with revision+span (also disjoint). A snapshot self-replay CONTROL (not evidence) resolves 565/565. `atlas_callable_search` corroboration: 0 resolved outputs to compare (agree/missing/stale/conflict all 0). Not established: `CURRENT_SOURCE_AUTHORITY_PROVEN`, canonical AST promotion, symbol-version admission, callable-search completeness, Graphify freshness. 0 store writes.
- [ ] **WFU-04b** rg evidence resolves `sourceRef`; a real current ast-grep observation resolves through the WFU-04a resolver against an admitted same-revision AST candidate set with live readback (reuse `AstGrepObservationV1`; do NOT use `atlas_callable_search` as the resolver). Still open: the current observation stream and the AST snapshot cover disjoint files, so no live observation has been resolved. **Partial implementation proof (2026-09-23):** `packages/parent-atlas/src/core/ast-grep-tree-node-resolution-v1.ts` now resolves only an exact `(source_ref, source_revision, byte_start, byte_end)` match against supplied `atlas_ast_nodes` rows; path/revision/span mismatch and duplicate candidates remain explicit non-ID outcomes. Six focused tests/build pass. Live read-only census: 11,273 AST rows, only 98 with `sha256:` source revisions; those 98 have unique exact location keys. This does not yet prove a real current ast-grep observation matches an admitted workspace frame, and rg’s workspace-relative `sourceRef` is not by itself canonical source authority. Keep this task open until a same-source-revision extracted observation is read through the resolver with exact live readback; do not use the sparse `atlas_callable_search` projection as a substitute.
- [x] Record the registry coverage gap: `atlas_callable_search` 285 rows vs `atlas_ast_nodes` 11,067 — explain before relying on it. **Live read-only refresh (2026-09-23):** current counts are 285 vs 11,273 (the older 11,067 AST count is stale). The raw table-size ratio is not semantic coverage because AST includes files, schemas, routes, headings, and many non-callable declarations. More importantly, the 285 projection rows contain 191 NULL `tree_node_id`, 94 distinct non-null IDs, and only 9 rows with an exact `tree_node_id` match in `atlas_ast_nodes`. `node_kind` is NULL on 191 rows; the other 94 comprise 65 functions, 23 types, and 6 classes. Therefore `atlas_callable_search` is not a complete AST registry and must not be used as the general AST evidence resolver; first WFU-04 adapter subtask remains open. SQL was read-only; no rows changed.

### WFU-05 — OKF vocabulary adapter
- [ ] Map evidence onto `.okf` vocabulary (domain, language, concepts, relations `IMPLEMENTS/CALLS/READS/WRITES/DEPENDS_ON`). OKF defines vocabulary; it does not retrieve. **Owner gap recorded (2026-09-23):** the existing AST dry-run classifies through `domain-taxonomy.ts`, not `.okf`; `okf-topic-ingestion.ts` derives provisional concepts but does not validate them against a loaded `.okf` vocabulary. Root `.okf/manifest.yaml` lists missing predicate `ontology.yaml` and domain schemas (`retrieval.yaml`, `cache.yaml`, `database.yaml`) absent from `.okf/domains/`; present predicate registry `.okf/predicates/feature-relationships.yaml` defines `depends_on` and `implements` but not the requested `CALLS/READS/WRITES`. The separate MCP catalog serves `docs/.okf/registry.yaml`, so it is not the root manifest/vocabulary loader. Do not map guessed aliases or mark complete until the authoritative manifest/schema set and relation vocabulary are reconciled; no `.okf` files changed.

### WFU-06 — Postgres topic/domain schema
- [x] **WFU-06a** Classify the 12 existing tables (R2) as CANONICAL_OWNER / BACKEND / ADAPTER / DEAD in `docs/architecture/runtime-ownership-registry.json`. **Read-only classification 2026-09-23:** refreshed exact counts for all 12 tables and traced active code readers/writers; role matrix is recorded under `capabilities.domain_classification.persistence_surfaces` and detailed in `docs/reports/wfu-topic-domain-table-ownership-v1.json`. No data/table mutation. This does not resolve the separate operator-owned `domain_class` owner decision, populate `domain_taxonomy_v1`, or authorize cleanup of populated legacy tables.
...
- [ ] Multi-predicate query (domain + language + FTS + active) shows Bitmap AND/OR plan; record `EXPLAIN (ANALYZE, BUFFERS, WAL, SETTINGS)` and `pg_aios` output (R5). **Partial proof (2026-09-23):** a 32,768-row temporary fixture with separately indexed domain, language, active, and FTS predicates selected `BitmapAnd` by the default planner; it combined the independent domain/language B-trees, with FTS and active as residual filters. Temporary table/indexes and data were rolled back. PG 18.4 reports `io_method=worker`; `pg_aios` readback was 0 in-flight handles, so AIO execution/performance is NOT proven. Live `domain_taxonomy_v1` has 0 rows and `atlas_packets` has 0 `canonical=true` rows, preventing an equivalent production-corpus proof. Keep open. Receipt: `docs/reports/wfu-bitmap-aio-fixture-v1.json`.
- [x] **`btree_gin` decision (2026-09-23, read-only): NOT NEEDED for the current contract.** `btree_gin` is absent; live PostgreSQL has `pg_trgm 1.6` and `vector 0.8.3`. The topic/domain owner census found no persisted topic-search relation or mixed multicolumn GIN query to justify installing it. Existing `atlas_ontology_concepts` already has separate GIN(`aliases`) and B-tree(`namespace`/`concept_type`) indexes; PostgreSQL can consider separate-index bitmap combinations without `btree_gin`. Revisit only if a measured workload needs a multicolumn GIN operator class that requires the extension. No extension/index/table mutation; this closes only the extension decision, not WFU-07's plan proof.

### WFU-08 — FeaturePacketV1
- [x] **PROVEN, fixture-only (2026-09-23):** added `FeaturePacketV1` in the existing `@atlas/semantic-contracts` owner as a revision-qualified envelope around the existing `DomainFeaturePacketSchema` (no second feature payload owner). It binds caller-owned `packetKey`, `sourceRef`, `sourceRevision`, `workspaceRevision`, `producerRevision`, and `featureRevision`; carries evidence refs for topic/domain/language labels; checks nested packet-key and feature-revision equality; and computes a deterministic SHA-256 checksum using the shared canonical-hashing owner. Evidence list order/duplicate refs normalize deterministically. `canonicalAuthority=false`, `writesPerformed=false`, `promotionAuthorized=false`. Also fixed `canonicalHashJSON`: its old JSON replacer-array implementation omitted nested object keys not present at the root; recursive key sorting now retains nested fields and is covered by regression tests. Package TypeScript build and focused tests pass 7/7. No DB/cache/vector writes or promotion. Report: `docs/reports/feature-packet-v1.json`.
...
- [ ] Retire the adapter's legacy defaulted scalar fields (`lowRankScore=0.5`, `estimatedMinutes=15`, `remainingRequiredGates=999`, ...) once a consumer census shows nothing reads them (unchanged for now). **Read-only consumer census (2026-09-23): NOT RETIRABLE.** `adapt-openspec-controller-to-ranker-v3.mjs` still emits defaults for `remainingRequiredGates`, `estimatedMinutes`, `lowRankScore`, and other compatibility scalars; its deterministic sorter consumes `remainingRequiredGates` and `estimatedMinutes`. `build-openspec-challenger-tournament-v1.mjs::scoreOf` still recognizes `lowRankScore` as a fallback score. `python/build_low_rank_task_recommendation_v2.py` explicitly excludes legacy scalar inputs and consumes only qualified `featureVector` columns, so it is a compatibility guard rather than a scalar consumer. The v2 adapter retains the same defaults. No fields were removed or altered; keep this task open until the actual remaining consumers migrate to qualified features and deterministic ordering is proven unchanged. Receipt: `docs/reports/wfu-legacy-scalar-consumer-census-v1.json`.
...
- [ ] Studio docs panel (2026-09-23, `STUDIO-DOCS-SSR-01`, PARTIAL — do not close WFU-12): `DocCorpusPanel.svelte` in `(app)/admin/atlas`, snapshot from `+page.server.ts` via `/api/admin/atlas/docs-corpus` (`DocIntelligenceStudioSnapshotV1`), native `<details>` sections + plain-GET search that works without JS; runtime-vs-doc version drift, AIO / bitmap shown as CAPABILITY vs PLANNER_SELECTED vs PRODUCTION_OBSERVED (never observed), analysis readiness. Canonical corpus is EMPTY and the DOC-06A handoff is BLOCKED (`external-doc-studio-readiness-v1.json`), so canonical search is unproven; local hits are labelled REFERENCE_ONLY / NONCANONICAL. Live readback: PG 18.4, pgvector 0.8.3, `io_method=worker`, `pg_aios` and `uuidv7()` present, GIN containment plans as Bitmap Index/Heap Scan (capability proof, not proof every query uses AIO). Report: `docs/reports/doc-corpus-studio-smoke-v1.json`.
- [x] Progress = native `<progress value max>` + label (2026-09-23): `/atlas/studio/openspec/+page.svelte` now renders a labelled native progress element with a clamped 0–100 value and CSS styling; no Anime.js dependency or confidence semantics. Svelte server compilation passes with zero warnings. Detailed proof: `docs/reports/wfu-studio-progress-native-v1.json`.
...
- [x] **TOPO-01 PROVEN (2026-09-23, static ownership/caller audit):** all ten listed implementation groups are classified in the registry's cross-cutting `implementation_inventory.dag_topology_helpers` section. The inventory introduces no canonical owner: typed graph runtime = BACKEND; parity pair and spectral multihop = EXPERIMENT; OAK, error-fix DAG, KAG runner, document citation DAG, S-Graph taxonomy, agentic fix proposal, and workboard metadata = ADAPTER. Evidence is scoped to implementation role and repository references, not claims of live production activity. TS DAG helpers stay per-purpose because their node/edge schemas and side-effect boundaries differ; a Kahn implementation alone is not an owner-convergence reason. `frozen-dag-v1.mjs` was inspected but is not API-compatible with WFU declarations and has no production caller established. Receipt: `docs/reports/wfu-topology-ownership-v1.json`; runtime registry audit and focused tests below.
...
- [x] **TOPO-02 PROVEN (2026-09-23):** keep `scripts/atlas/lib/wfu-metadata.mjs` local. It validates the WFU declaration grammar, resolves logical task identities, and detects dependency cycles; `frozen-dag-v1.mjs` models revisioned execution nodes/lifecycle/checksums instead, so replacing the WFU helper would conflate contracts. No shared Node/TypeScript owner with a compatible API and production caller was found. No code movement or runtime behavior change.
- [x] **TOPO-03 PROVEN (2026-09-23, live read-only audit):** added `scripts/atlas/audit-taxonomy-forest-invariants-v1.mjs` and a pure invariant evaluator/test. Live Postgres read-only transaction returned 5,527 nodes, 5,527 unique keys, 1 root, 0 duplicate keys, 0 missing parents, 0 cycle-reaching starts, max parent chain 4, and `isBranchingForest=true` (the `parent_key` relation is a forest with in-degree <=1). Typed-edge multi-parent target baseline: `PART_OF=408`, `IS_A=8`, `INHERITS_FROM=8`. Focused fixture tests pass 2/2; no DB/projection writes. Receipt: `docs/reports/taxonomy-forest-invariants-v1.json`.
...
- [x] **TOPO-05 PROVEN (2026-09-23):** `python/atlas_oak_kernel.py::oak_traverse` is the existing owner: PostgreSQL-backed ontology uses its bounded read-only recursive traversal; configured OAK adapters must implement `OboGraphInterface` and route to `ancestors()` / `descendants()`. Added a spy-backed route test for both directions; focused OAK tests pass 9/9. Runtime check: oaklib 0.7.4 has `oaklib.utilities.graph.networkx_bridge`, a conversion utility, but no registered NetworkX adapter implementation. Reuse the existing OAK/Postgres owner; do not add a NetworkX traversal adapter. No store writes. Receipt: `docs/reports/topology-oaklib-owner-v1.json`.
- [x] **TOPO-06 contract resolved (2026-09-23, live read-only evidence):** `taxonomy_nodes.parent_key` is the hierarchical single-parent relation and requires a branching-forest invariant (no duplicate/missing nodes, cycles, or multiple parents). Do not use undirected `nx.is_forest` as a substitute for that directed contract. `taxonomy_edges` relations (`PART_OF`, `IS_A`, `INHERITS_FROM`) are typed semantic graphs: require per-relation directed acyclicity and report multi-parent targets; do not reject them merely for failing a forest test. The existing read-only audit now records this contract and checks typed DAGs. Live: hierarchy 5,527 nodes / 1 root / 0 missing parents / 0 cycles; typed DAGs all acyclic with multi-parent counts 408 / 8 / 8. Focused fixtures 3/3; canonicalAuthority=false; no writes. Receipt: `docs/reports/taxonomy-forest-invariants-v1.json`.
...
- [x] **NS-8D PROVEN (2026-09-23, proposal-only; patch NOT applied).** The existing `scripts/atlas/suggest-task-ids-v1.mjs` reads a freshly regenerated workboard snapshot and emits an advisory JSON plus a unified patch; source inspection confirms it never writes `tasks.md` or applies the patch. Fresh isolated run: 2,007 candidates and 2,007 suggested IDs across 51 files; 0 drift skips, 0 missing-source skips; `tasksFilesEdited=0`, `writesPerformed=false`. `git apply --check --unidiff-zero` passed against the generated patch. Snapshot SHA-256 `7758f42e737a31da47c5ebb55c520cd6d85e3161579f49e9179c6498b0456d84`; suggestion JSON SHA-256 `6b15b6166359dda41426cf25ccb3fcb0c4556b3d1ff1b774ccee9fb0b8b08b2a`; patch SHA-256 `41abf409ad1fb1286bbef633a9f6aec960e08104280ae56f3b974dcf8f4d44d2`. Detailed receipt: `docs/reports/task-id-suggestions-proposal-v1.json`. The generated `.tmp/atlas/ns-8d-audit/task-id-suggestions-v1.patch` remains unapplied; operator review/adoption of these 2,007 task-line edits is still a separate decision and is not claimed complete.
- [x] **NS-8E PROVEN (2026-09-23, read-only census):** added `scripts/atlas/audit-openspec-duplicate-task-ids-v1.mjs`, reusing the canonical `resolveDeclarations` identity resolver, and report `docs/reports/openspec-duplicate-task-ids-v1.json`. Current source-ledger census: 9,490 task rows / 3,255 open across 90 task files; 8 duplicate-ID groups affect 27 open ambiguous rows across their owning changes. The report includes every duplicate occurrence (including completed counterparts), section, line, task revision, and per-file/source-set checksums. Unit tests pass 2/2. No task ledger was edited by the audit; owners can use the list to plan renames. Canonical identity/promotion remains false.
...
- [x] **TOURNAMENT-02 / TOURNAMENT-03 PROVEN (2026-09-23):** `TestTaskRecommendation.test_task_recommendation_is_deterministic_and_non_authoritative` runs the same 40-row feature fixture twice and requires identical `outputChecksum`; it also asserts `canonicalAuthority=false` and `eligibleForAuthority=false`. This proves deterministic replay and non-authoritative status for the actual low-rank recommendation producer, not only a helper. Focused pytest replay passed; no persistence or promotion path is invoked.
