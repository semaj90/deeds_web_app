...
- [ ] Studio docs panel (2026-09-23, `STUDIO-DOCS-SSR-01`, PARTIAL — do not close WFU-12): `DocCorpusPanel.svelte` in `(app)/admin/atlas`, snapshot from `+page.server.ts` via `/api/admin/atlas/docs-corpus`, plain-GET search that works without JS; pinned/runtime versions, AIO (`io_method`, `pg_aios`), bitmap capability, freshness table. Postgres corpus is EMPTY (`DOC_CORPUS_POSTGRES_EMPTY`), so canonical search is unproven; local hits are labelled REFERENCE_ONLY. Live readback: PG 18.4, pgvector 0.8.3, `io_method=worker`, `pg_aios` and `uuidv7()` present, GIN containment plans as Bitmap Index/Heap Scan (capability proof, not proof every query uses AIO). Report: `docs/reports/doc-corpus-studio-smoke-v1.json`.
