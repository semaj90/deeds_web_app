...
**Status**: CAPTURE + PATCH A + PATCH B (2026-08-09) — architecture specified
in full; contract types (Patch A) and persistence tables (Patch B) are both
implemented and live-verified. No adapter (Patch C+) writes to these tables
yet — they exist but are unpopulated. Everything past Patch B is deliberately
not started, gate-by-gate, per this repo's established discipline for large
external plans (see `parent-atlas-agentic-repair-bundle-integration` and
...
| C | PageRank adapter emits `GraphAnalysisRun` + `graph_node_metrics(pagerank)`; existing `pagerankAuthority` promotion path unchanged | Not started |
| D | Community adapter — Louvain and Leiden emit the same normalized result shape via one contract, separate algorithm implementations | Not started |
| E | `community-taxonomy-policy.ts` evaluator — coverage/modularity/size-distribution/stability/purity, no promotion decision | Not started |
| F | CheiRank — reversed-orientation PageRank reuse | Not started |
| G | k-core — independent scalar metric run | Not started |
