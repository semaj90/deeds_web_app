...
- [ ] **EXTERNAL_DOC_ADMISSION_RUNNER_RESUMABLE_01** reliability follow-up, classification `PARTIAL_COMMIT_RECOVERY_GAP`: `run-external-doc-admission-v1.mts` commits one page per transaction and refuses to start unless both canonical tables are empty, so a crash after page N leaves a valid partial corpus a naive rerun refuses to resume. Not triggered (all 30 pages committed). Fix without a second SQL owner: exact page/chunk readback classifies already-correct pages `ALREADY_ADMITTED`, admits only missing pages, fails closed on conflicting rows; or an atomic whole-corpus path through the existing writer. Do not rewrite admitted rows.
