...
Phase V status 2026-09-23: VAL-01..07 and VER-none closed; OPEN = VAL-08 (OaK typed assertions), VAL-10 (19-chunk replay -> EXTERNAL_DOC_SUMMARY_FAITHFULNESS_PROVEN), VER-01..03, summary-level coverage signal (omission analysis moved out of VAL-03), and `CANONICAL-HASH-CONVERGENCE-01`. No persistent summary write is authorized before VAL-10.
...
- [ ] **VAL-10** Bounded acceptance canary: run the spine over the 19-chunk sample, then `--apply --limit 20` ONLY with an explicit operator answer (`ATLAS_DOC_SUMMARY_AUTHORIZED` + `--limit`); persist `atlas_external_doc_analyses` rows with the validation receipt reference; never `--all` without separate authorization.
