...
- [ ] **VAL-03** L2 exact technical-token validator (extract from `atlas_summary_faithfulness_v1.py`, do not fork): identifier/config/API tokens must occur verbatim in the chunk; never fuzzy-approve; plain descriptive words absent from the chunk are NOT a hard failure (the `low-selectivity` lesson) but are recorded.
- [ ] **VAL-04** L2 numeric/version validator: every number/version in the summary must occur in the chunk or its prompt header; mismatch => `UNSUPPORTED_NUMERIC_CLAIM` / `UNSUPPORTED_VERSION_CLAIM` => REJECT, no model call.
- [ ] **VAL-05** (primitive `verifySummaryClaimByteSpanV1` already exists in `summary-claim-validation-v1.ts` from the VAL-01 commit; remaining: wire it to read canonical chunk bytes and populate the slot) L2 source-span verifier: a judge-cited span (`startByte`/`endByte` + checksum) is re-read from canonical chunk bytes by code; a span the code cannot verify voids that claim's support (model-made offsets are never accepted).
- [ ] **VAL-06** `SummaryJudgeContextManifestV1`: judge input is exactly chunkId/revision, canonical chunk text, product/version/title/headingPath, summary, claim. No neighbours, Qdrant, web, SearXNG or ACE memory (tests assert nothing else can enter).
- [ ] **VAL-07** L4 judge as a bounded challenger: `independenceClass: SAME_MODEL_SEMANTIC_JUDGE` recorded in every receipt; accept both JSON shapes; parse failure = null (never a pass); gate closes only on 0 deterministic failures AND 0 material UNSUPPORTED claims; PARTIALLY_SUPPORTED routes to REVIEW. Optional later: an independent challenger judge (not a blocker).
