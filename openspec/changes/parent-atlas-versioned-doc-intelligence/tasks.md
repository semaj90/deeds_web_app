...
- [ ] **VAL-05** L2 source-span verifier: a judge-cited span (`startByte`/`endByte` + checksum) is re-read from canonical chunk bytes by code; a span the code cannot verify voids that claim's support (model-made offsets are never accepted).
