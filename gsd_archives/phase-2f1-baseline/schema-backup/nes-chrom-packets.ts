...
export const nesChromPackets = pgTable('nes_chrom_packets', {
  ...
  vectors: jsonb('vectors').notNull().default(sql`'{}'::jsonb`),
  ...
  pagerankIdx: index('idx_nes_chrom_packets_pagerank').on(table.pagerank),
  betweennessIdx: index('idx_nes_chrom_packets_betweenness').on(table.betweenness),
  eigenvectorIdx: index('idx_nes_chrom_packets_eigenvector').on(table.eigenvector),
  neo4jNodeIdIdx: index('idx_nes_chrom_packets_neo4j_node_id').on(table.neo4jNodeId),
  redisCentroidKeyIdx: index('idx_nes_chrom_packets_redis_centroid_key').on(table.redisCentroidKey),
  ...
  permissionsGin: index('idx_nes_chrom_packets_permissions_gin').using('gin', table.permissions),
  metadataGin: index('idx_nes_chrom_packets_metadata_gin').using('gin', table.metadata),
  topologyGin: index('idx_nes_chrom_packets_topology_gin').using('gin', table.topology),
  vectorsGin: index('idx_nes_chrom_packets_vectors_gin').using('gin', table.vectors),
