...
## Current OpenSpec reconciliation (2026-09-05)

### Document role and authority

This document is the **Parent Atlas Workstation integration projection and
dependency spine**. It does not own implementation tasks, supersede OpenSpec
task status, authorize writes, or establish identity/representation authority.

Authority hierarchy:

- OpenSpec `spec.md` and `tasks.md`: behavior and implementation proof gates.
- `docs/reports/openspec-portfolio-v1.json`: portfolio ownership authority.
- `docs/OPENSPEC-WORKBOARD.md`: mechanical global progress projection.
- This document: Workstation-specific cross-change dependency projection.
- `docs/reports/parent-atlas-workstation-openspec-workboard-v2.json`: classified
  Workstation reconciliation view.

If this projection disagrees with an owning OpenSpec change, the OpenSpec owner
wins. No checkbox here closes an OpenSpec task.

### Current P0 gate

`CURRENT-SOURCE-OWNER-RECONCILIATION-01` is the active gate.
...
- **Proven bounded:** source-to-chunk materialization binding for the recorded
  50-source / 434-chunk physical cohort.
- **Not authorized:** chunk `source_ref` mutation, semantic promotion, graph
  projection, or GPU residency expansion.

The bounded physical chunk proof must not be read as proof of stable namespace
...
For current planning, Phase 11 is `PARTIAL`, Phase 12 is
`BLOCKED_IDENTITY_AUTHORITY`, Phase 13 is `PARTIAL`, Phase 14 is
`IMPLEMENTED_BOUNDED`, Phase 15 is `BLOCKED_CURRENT_COHORT`, Phase 16 is
`BLOCKED_CURRENT_GRAPH_BINDING`, and Phase 17 is
`DOWNSTREAM_OF_SEMANTIC_ADMISSION`. These labels describe capability evidence,
not promotion authorization.

The authoritative Workstation projection is now the replay-proven v2 board:
`docs/reports/parent-atlas-workstation-openspec-workboard-v2.md` and
  `docs/reports/parent-atlas-workstation-openspec-workboard-v2.json`, generated by
`scripts/atlas/build-parent-atlas-workstation-openspec-workboard-v2.mjs` and
verified by `scripts/atlas/prove-parent-atlas-workstation-workboard-v2.mjs`.
It selects 59 relevant unchecked tasks and classifies them without treating every
checkbox as executable work. The current full-workspace authority blocker remains:
no completed Graphify execution matches the current workspace revision. Separately,
the bounded 08A receipt now proves 50 source namespaces and 434 chunk preimages,
  but stable source-registry/repository-namespace authority remains unresolved and
  the packet-membership preflight still finds `0` qualified packet candidates.
Therefore no packet-membership, semantic, graph, GPU, cache, or package promotion
is authorized.

`GRAPHIFY-DAILY-COORDINATOR-01` is closed locally in the authority ledger by the
committed bounded canary; the older checklist below is retained for historical
context and dependency explanation. Use the v2 board plus the owning OpenSpec
ledger for current status. Do not create a separate structural-GPU-residency
OpenSpec while its existing candidate-feature, GPU-substrate, tensor-residency,
topology, and lineage owners remain active.

### Current Workstation readiness matrix (2026-09-05)

| Dependency | Projected state | Authority / next dependency |
| --- | --- | --- |
...
| `semantic_768` | `OWNER_ESTABLISHED / CURRENT_COHORT_BLOCKED` | Existing canonical representation owner; EmbeddingGemma is a runtime detail only when bound by its receipt; current source authority and packet cohort admission remain blocked |
| `latent_256 → latent_128 → latent_64` | `DOWNSTREAM_BLOCKED` | Topology/tensor-residency owners; requires admitted semantic cohort |
| Centroid artifacts | `CONTRACT_PRESENT_POPULATION_BLOCKED` | Tensor-residency owner; derived IDs only |
| BitFrost invalidation | `PRIMITIVE_PROVEN` | ACE/BitFrost owner; live adoption not required by current runtime |
| BitFrost residency warming | `PROVEN_BOUNDED_FIXTURE / LIVE_GET_PROVEN` | ACE/BitFrost owner; live GET is proven, while live warming/adoption remains deferred to a separate residency gate |
...
`scripts/atlas/prove-workstation-live-capability-v1.mjs` passed with
`LIVE_CAPABILITY_PROVEN`. The proof used an explicit ioredis connection followed
by one GET and immediate disconnect; Valkey returned a natural miss, so the
result is `LIVE_GET_PROVEN`, not a fabricated cache hit. No SET, DEL, ZADD, or
SCAN was issued. The same bounded proof streamed `/v1/chat/completions` from
the observed `ornith-1.5-9b` model at `http://127.0.0.1:8090`, required HTTP
200, buffered SSE records, observed `[DONE]`, and received non-empty content.

Receipt: `docs/reports/workstation-live-capability-proof-v1.json`.
This proves runtime capability only. `canonicalAuthority=false`, production
plan path is false, canonical Postgres/Qdrant/Neo4j writes are zero, cache
writes are false, and proof-artifact filesystem output is explicitly counted.
It does not close source/chunk lineage, current-workspace coverage, semantic
cohort admission, or Workstation production readiness.
| Outcome ledger | `SCHEMA_OPEN` | MCP receipt owner; fresh-install and writer convergence remain separate |
| LSP stable-symbol canonicalization | `OPEN` | Language-resolution owner; observations remain read-only |

### Structural GPU residency ownership

No new cross-cutting OpenSpec owner is created. Ownership remains distributed:

- Structural facts: `parent-atlas-candidate-feature-execution-fabric`
- GPU graph/vector execution: `parent-atlas-gpu-graph-vector-substrate`
- Representation admission: `parent-atlas-topology-representation-admission`
- Centroid/tensor residency: `parent-atlas-tensor-residency-integration`
- ACE/BitFrost residency policy: `parent-atlas-ace-rlm-bitfrost-integration`
- Current source/revision authority: `parent-atlas-retrieval-lineage-dag-convergence`

Workstation milestone: the structural GPU spine is ready only when current
identity is qualified, `GraphOrdinalMapV1` is bound to the current graph,
`semantic_768` cohort admission is proven, CPU/GPU parity shares one graph
revision, centroid artifacts are revision-bound, and residency consumes derived
IDs without creating identity or retrieval votes.

Latest source-authority recheck (2026-09-05): the registry contains 22,604 rows
and 111 workspace bindings, but no selected authoritative source set. The
current 52-row hydration plan has 52 exact revision matches but only 9 content-
hydrated rows, 0 authoritative namespaces, and 0 evidence-span-ready rows.
The current Graphify batch plan remains blocked: 0 exact candidates and 52
revision/content mismatches. This is an upstream source-selection/authority
gap, not permission to manufacture revisions or run broad Graphify.

Owner recheck (2026-09-05): `audit-graphify-run-file-binding-v1.mjs` finds one
`COMPLETED_BOUND` owner, five `COMPLETED_UNBOUND` runs, three
`RUNNING_UNBOUND` runs, and two `RUNNING_BOUND_NOT_TERMINAL` runs.
`audit-current-graphify-source-revision-v1.mjs` still resolves a running run
with zero rows and returns `SOURCE_AUTHORITY_UNAVAILABLE`. The next repair is
therefore selector ownership: choose a completed bound execution explicitly and
prove its coverage against the intended current source set. Do not infer
authority from the running run or rewrite historical Graphify rows.

Selector correction (2026-09-05): `scripts/atlas/audit-current-graphify-source-revision-v1.mjs`
now selects the completed bound owner by an explicit read-only rule when no run is
provided (`COMPLETED_BOUND_OWNER_BY_FILE_COUNT`) and rejects non-terminal runs.
The selected owner is `369e4270-7689-4536-8816-4ec4a5517b3e` with 25,258 rows;
24,885 content hashes match, 370 mismatch, and 3 sources are unavailable.
Result remains `SOURCE_BYTES_NOT_PROVEN`; this improves diagnosis but does not
authorize lineage promotion or repair historical rows.
...
## Historical proof-gated status (2026-09-03; superseded by the matrix above)
...
### Historical active TODO (superseded by OpenSpec authority)
...
### Concept Fabric inventory (2026-09-04)

The read-only owner inventory is recorded in
`docs/reports/parent-atlas-concept-fabric-audit-v1.json` and is generated by
`scripts/atlas/audit-parent-atlas-concept-fabric-v1.mjs`. It confirms that the
repository already has the `.okf`/Zod schema layer, domain/concept contracts,
source/chunk and AST/LangExtract observers, retrieval/graph projections,
ACE/prefill, BitFrost/Valkey, DAG/synthesis, and language-resolution helpers.
The remaining work is bridge proof: incremental directory indexing, revision-sealed
external evidence, per-operator DAG parameters, strict ACE source ownership,
cache readback, and cross-engine language parity. No database, projection, cache,
external crawl, or LLM writes are implied by this inventory.
