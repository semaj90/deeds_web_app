...
Quick Start
: Use the
provider plugin generator script
to create a new provider in minutes:
python scripts/create_provider_plugin.py MyProvider --with-schema
Architecture Overview
The provider system uses a
router pattern
with
automatic discovery
:
...
(
router.py
): Maps model ID patterns to provider classes. A legacy
registry.py
alias still imports — new code should use
router
directly.
Factory
(
../factory.py
): Creates provider instances based on model IDs
Providers
: Implement the
BaseLanguageModel
interface (from
...
"GeminiLanguageModel"
,
# Will use default gemini-3.5-flash
provider_kwargs
=
{
"api_key"
...
Auto-discovered
: Uses Python entry points for automatic registration
Zero configuration
: Import langextract and the provider is available
Independent updates
...
"yourmodel-latest"
,
# Automatically finds the provider
)
...
Plugin loading is
lazy
- plugins are discovered when first needed
To manually trigger plugin loading:
lx.providers.load_plugins_once()
Set
LANGEXTRACT_DISABLE_PLUGINS=1
to disable plugin loading
Registry entries are tuples:
(patterns_list, priority_int)
How Provider Selection Works
When you call
lx.extract(model_id="gemini-3.5-flash", ...)
, here's what happens:
Factory receives model_id
: "gemini-3.5-flash"
Router searches patterns
: Each provider registers regex patterns
First match wins
: Returns the matching provider class
Provider instantiated
: With model_id and any kwargs
Inference runs
...
r'^gemini'
,
# Prefix: model_id="gemini-3.5-flash"
r'^palm'
# Legacy: model_id="palm-2"
)
class
GeminiLanguageModel
(
...
lx.extract()
to providers through several mechanisms:
...
4
,
# Parallel processing (provider-dependent)
max_char_buffer
=
3000
,
...
for a fully-functional plugin template with testing and documentation.
...
=
"
langextract_yourprovider:YourProviderLanguageModel
"
☐
...
__init__.py
☐
4. (Optional) Add Schema Support
(
schema.py
)
Create schema class inheriting from
langextract.core.schema.BaseSchema
Implement
from_examples()
class method
Implement
...
twine upload dist/*
Share your provider by opening an issue on
LangExtract GitHub
to get feedback and help others discover it
Consider submitting a PR to add your provider to
...
=
"
langextract_myprovider:MyProviderLanguageModel
"
#
...
The
@router.register
decorator patterns (e.g.,
r'^mymodel'
,
r'^custom'
) define which model IDs your provider supports
When users call
lx.extract(model_id="mymodel-3b")
, the router matches against these patterns
...
twine upload dist/
*
Now users can install and use your provider with just
pip install langextract-myprovider
!
...
Use
from_examples()
to build a schema from provided examples
Call
to_provider_config()
to get provider-specific kwargs
Pass these kwargs to your provider's
__init__()
Your provider uses the schema for structured output
For user-authored schemas passed as
output_schema=...
, also implement
BaseSchema.from_schema_dict()
. If the provider exposes lower-level schema
kwargs such as
response_schema
, list them in
output_schema_reserved_provider_kwargs()
so LangExtract can reject ambiguous
requests where both
output_schema
and provider-native schema kwargs are set.
Schemas returned from
from_schema_dict()
should expose
from_output_schema=True
;
BaseLanguageModel.apply_output_schema()
marks
...
⚠️
Note
: Adding a provider to the core package requires:
...
Explicit Control
: Users can force specific providers when needed
Pattern Priority
: All patterns have equal priority (0) by default
Common Issues
Provider Not Found
...
'unknown-model'
Solution
: Check available patterns with
langextract.providers.router.list_entries()
Plugin Not Loading
