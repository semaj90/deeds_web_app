...
The
`DEFAULT`
clause specifies a default value to use for the column if no value provided by the user when doing an
`INSERT`
.
...
values. The
`NOT NULL`
constraint enforces a column to
NOT
accept
NULL
values.
...
Drizzle ORM provides a standalone
`primaryKey`
operator for that:
```plaintext
...
set return type for reference callback or use a standalone
`foreignKey`
operator.
```plaintext
...
```
To declare multi-column foreign keys you can use a dedicated
`foreignKey`
operator:
```plaintext
...
```
## Indexes
Drizzle ORM provides API for both
`index`
and
`unique index`
declaration:
