...
For more info please refer to the official pg_vector
docs.
```plaintext
export const table = pgTable('table', {
...
or
`push`
commands with the PostGIS extension and don’t want PostGIS tables to be included, you can use
extensionsFilters
to ignore all the PostGIS tables
#### Column Types
`geometry`
Store your geometry data with the rest of your data
For more info please refer to the official PostGIS docs
docs.
```plaintext
...
`xy`
will be accepted for insert and mapped on select to an object with x, y coordinates. So, the database geometry will be typed as
`{ x: 1, y: 2 }`
with drizzle
type
