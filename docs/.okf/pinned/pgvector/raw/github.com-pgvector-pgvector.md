...
Use
binary quantization
for faster build times at scale
### Indexing Progress
Check
indexing progress
```sql
...
`hnsw.ef_search`
of 40, only 4 rows will match on average. For more rows, enable
...
Use
binary quantization
for smaller indexes and faster build times at scale.
...
Use
binary quantization
with re-ranking to keep indexes in-memory at scale.
...
Use
binary quantization
for indexes (with re-ranking for search)
