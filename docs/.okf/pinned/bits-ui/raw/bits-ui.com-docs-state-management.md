...
Each component's API reference will highlight what props are
`bindable`
. You can replace the
`value`
prop used in the examples on this page with any
`bindable`
prop.
## Two-Way Binding
The simplest approach is using Svelte's built-in two-way binding with
`bind:`
:
```
<script lang="ts">
...
Function Binding
that handles both getting and setting values:
```
<script lang="ts">
  import { ComponentName } from "bits-ui";
