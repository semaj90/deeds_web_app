...
Section 11.3
, it would be less useful for queries involving only
`y`
. Just how useful will depend on how effective the B-tree index skip scan optimization is; if
`x`
has no more than several hundred distinct values, skip scan will make searches for specific
`y`
values execute reasonably efficiently. A combination of a multicolumn index on
`(x, y)`
and a separate index on
`y`
might also serve reasonably well. For queries involving only
`x`
...
Prev | Up | Next
11.4. Indexes and ORDER BY | Home | 11.6. Unique Indexes
## Submit correction
