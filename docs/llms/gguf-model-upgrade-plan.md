...
- Active repo model: `vendor/models/gemma4-rotorquant:latest.gguf`
  - This is the active merged GRPO legal model in the repository.
- Desktop upgrade candidate: `C:\Users\james\Desktop\gemma4-rotorquant:latest-iq4xs\gemma4-rotorquant:latest-iq4xs.gguf`
  - This is the IQ4_XS upgrade candidate.
- Desktop duplicate/original merged file: `C:\Users\james\Desktop\gemma4-rotorquant:latest-iq4xs\gemma4-rotorquant:latest-merged-q4km.gguf`
- Supporting desktop artifacts:
  - `C:\Users\james\Desktop\gemma4-rotorquant:latest-iq4xs\gemma4-rotorquant:latest-iq4xs-direct.gguf`
  - `C:\Users\james\Desktop\gemma4-rotorquant:latest-iq4xs\gemma4-rotorquant:latest-f16.gguf`
  - `C:\Users\james\Desktop\gemma4-rotorquant:latest-iq4xs\gemma4-rotorquant:latest-f16-test.gguf`

## Path mapping

| Environment | Model path | Purpose |
|---|---|---|
| Repo active runtime | `c:\Users\james\Videos\deeds-web-app\vendor\models\gemma4-rotorquant:latest.gguf` | Current merged GRPO legal model used by `ROTORQUANT_MODEL_PATH` and `TURBO_MODEL_PATH`.
...
| Repo VLM runtime | `gemma4-rotorquant:latest.gguf + mmproj-gemma4.gguf` | Combined text + vision runtime when projector is enabled.
| Desktop candidate | `c:\Users\james\Desktop\gemma4-rotorquant:latest-iq4xs\gemma4-rotorquant:latest-iq4xs.gguf` | IQ4_XS upgrade candidate.
| Desktop backup | `c:\Users\james\Desktop\gemma4-rotorquant:latest-iq4xs\gemma4-rotorquant:latest-merged-q4km.gguf` | Original merged duplicate; should not be promoted.
| Desktop fallback | `c:\Users\james\Desktop\gemma4-rotorquant:latest-iq4xs\gemma4-rotorquant:latest-iq4xs-direct.gguf` | Direct IQ4_XS fallback without iMatrix calibration.
| Desktop artifacts | `c:\Users\james\Desktop\gemma4-rotorquant:latest-iq4xs\gemma4-rotorquant:latest-f16.gguf` and `gemma4-rotorquant:latest-f16-test.gguf` | Cleanup candidates after upgrade.
## Model clarification

- `gemma4-rotorquant:latest.gguf` is the merged GRPO legal text model with LoRA fused in.
- It is not the unified VLM model by itself; VLM behavior is provided separately via `mmproj-gemma4.gguf` and the `gemma4-rotorquant:latest` runtime alias.
- Therefore `LEGAL_LORA_PATH` must remain unset for these merged GGUFs.

## Validation result

- A temporary startup test was performed on port `8091` using `gemma4-rotorquant:latest-iq4xs.gguf`.
...
```powershell
$env:ROTORQUANT_MODEL_PATH = 'C:\Users\james\Desktop\gemma4-rotorquant:latest-iq4xs\gemma4-rotorquant:latest-iq4xs.gguf'
$env:TURBO_MODEL_PATH = 'C:\Users\james\Desktop\gemma4-rotorquant:latest-iq4xs\gemma4-rotorquant:latest-iq4xs.gguf'
$env:LEGAL_LORA_PATH = ''
...
- If testing VLM, start the server with the projector enabled by setting `TURBO_MMPROJ_PATH` and using the same launcher.
  - Use `gemma4-rotorquant:latest` as the VLM alias when available.
  ...
  - Example OpenAI-compatible image request via `/v1/chat/completions`:
    ...
    curl -s -X POST http://localhost:8090/v1/chat/completions \
      ...
      -d '{"model":"gemma4-rotorquant:latest","messages":[{"role":"user","content":"Describe this image in legal-evidence terms: list visible people, objects, actions, and locations."}],"image_url":"data:image/png;base64,<BASE64_IMAGE_DATA>","stream":false}'
    ```
  ...
  - Example repo-local `api/generate` style request:
    ...
    curl -s -X POST http://localhost:8090/api/generate \
      -H "Content-Type: application/json" \
      -d '{"model":"gemma4-rotorquant:latest","prompt":"Describe this image in legal-evidence terms. List visible people, objects, actions, and locations.","images":[{"data":"<BASE64_IMAGE_DATA>","format":"png"}],"stream":false}'
    ```
...
- For VLM testing, run the benchmark explicitly against the VLM alias and port:
  - `npm run turbo:bench -- --port 8080 --label vlm --model gemma4-rotorquant:latest`
- Measure:
  - tokens per second
  - first-token latency
...
5. Compare against the current `vendor/models/gemma4-rotorquant:latest.gguf` baseline.
...
```powershell
copy c:\Users\james\Videos\deeds-web-app\vendor\models\gemma4-rotorquant:latest.gguf `
  c:\Users\james\Videos\deeds-web-app\vendor\models\gemma4-rotorquant:latest-backup.gguf
```

2. Replace the repo model with the tested IQ4_XS candidate.

```powershell
copy C:\Users\james\Desktop\gemma4-rotorquant:latest-iq4xs\gemma4-rotorquant:latest-iq4xs.gguf `
  c:\Users\james\Videos\deeds-web-app\vendor\models\gemma4-rotorquant:latest.gguf
```
...
- Remove or archive the following desktop files if they are no longer needed:
  - `gemma4-rotorquant:latest-merged-q4km.gguf`
  - `gemma4-rotorquant:latest-f16.gguf`
  - `gemma4-rotorquant:latest-f16-test.gguf`
- Keep `gemma4-rotorquant:latest-iq4xs-direct.gguf` only as a fallback if the calibrated IQ4_XS candidate regresses.

## Notes

- `gemma4-rotorquant:latest-iq4xs.gguf` is the preferred candidate for upgrade.
- `gemma4-rotorquant:latest.gguf` in repo is already the merged GRPO model.
- `LEGAL_LORA_PATH` should remain unset for the merged candidate.

## Current runtime mode

- `gemma4-rotorquant:latest.gguf` is the current main model.
