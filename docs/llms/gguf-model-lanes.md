...
TURBO_MODEL_PATH=C:\Users\james\Videos\deeds-web-app\vendor\models\gemma4-rotorquant:latest.gguf
...
### 🧱 Component Breakdown
1. **Model Core (`gemma4-rotorquant:latest.gguf`)**: A **Gemma4 E4B (4.4B)** base model merged with a specialized **GRPO (Group Relative Policy Optimization)** fine-tuned legal LoRA. Quantized in **Q4_K_M** (5.33 GB) format on disk.
...
| :--- | :--- | :--- | :--- |
| `gemma4-rotorquant:latest.gguf` | 5.0 GB | Apr 5, 2026 | **ACTIVE**. The primary merged GRPO legal text-reasoning model. |
| `mmproj-gemma4.gguf` | 946 MB | Apr 11, 2026 | **ACTIVE**. Multimodal visual projector (SigLIP). |
| `lora/gemma4-rotorquant:latest-grpo/` | 169 MB | May 16, 2026 | **Archived**. Contains the raw `.safetensors` GRPO LoRA weights (incompatible with direct `llama-server --lora` execution, pre-baked into the main GGUF). |
| `lora/gemma4-rotorquant:latest-text/` | 143 MB | May 16, 2026 | **Archived**. Contains raw `.safetensors` text LoRA weights. |

---

### 2. Desktop Model Inventory (`Desktop/gemma4-rotorquant:latest-iq4xs/`)
* Located at: `C:\Users\james\Desktop\gemma4-rotorquant:latest-iq4xs\`
...
| :--- | :--- | :--- | :--- |
| `gemma4-rotorquant:latest-iq4xs.gguf` | 4.8 GB | **IQ4_XS** (iMatrix) | **KEEP / RECOMMEND UPGRADE**. Features importance matrix calibration. Saves ~200MB VRAM over Q4_K_M while matching or exceeding token quality. |
| `gemma4-rotorquant:latest-iq4xs-direct.gguf` | 4.8 GB | **IQ4_XS** (Direct) | **DELETE / DUPLICATE**. A redundant direct quantization pass without iMatrix calibration. |
| `gemma4-rotorquant:latest-merged-q4km.gguf` | 5.0 GB | **Q4_K_M** | **DELETE / DUPLICATE**. This is the exact original merged model that was copied to the app vendor folder under the name `gemma4-rotorquant:latest.gguf`. |
| `gemma4-rotorquant:latest-f16.gguf` | 68 MB | F16 | **DELETE**. A leftover testing or lightweight metadata export artifact. |
| `gemma4-rotorquant:latest-f16-test.gguf` | 68 MB | F16 | **DELETE**. Leftover testing artifact from the export pipeline. |
...
1. **VRAM Safety Upgrade**:
   Swap the active `gemma4-rotorquant:latest.gguf` in `vendor/models/` from **Q4_K_M** (5.0 GB) to the newer **IQ4_XS (iMatrix)** variant `gemma4-rotorquant:latest-iq4xs.gguf` (4.8 GB).
...
2. **Desktop Cleanup Allowlist**:
   * **Keep**: `gemma4-rotorquant:latest-iq4xs.gguf` (as our primary backup/upgrade file).
   * **Delete**: `gemma4-rotorquant:latest-iq4xs-direct.gguf`, `gemma4-rotorquant:latest-merged-q4km.gguf`, `gemma4-rotorquant:latest-f16.gguf`, `gemma4-rotorquant:latest-f16-test.gguf`.
...
|---|---|---|---|---|
| **A — Hermes** | `gemma4-rotorquant:latest` (Ollama alias of `gemma4-rotorquant:latest`) | Ollama `:11434` | 65 536 | Tool-calling sanity check, web search, agent behavior comparison |
| **B — TurboQuant** | `gemma4-rotorquant:latest.gguf` via `llama-server.exe` | `:8090` | 16 384 (`TURBO_CTX`) | Long-context code audit, TRACE MCP loop — **default OpenCode lane** |
| **C — RotorQuant / IQ4_XS** | `gemma4-rotorquant:latest-iq4xs.gguf` — swap `TURBO_MODEL_PATH` | same binary | same | Testing newer quantizations before promoting to `vendor/models/` |

OpenCode defaults to Lane B. To switch lanes interactively:

```bash
# Lane A
opencode run -m ollama/gemma4-rotorquant:latest --agent audit-hermes "..."
...
# .env TURBO_MODEL_PATH stays the same — it points to vendor/models/gemma4-rotorquant:latest.gguf
```

The `gemma4-rotorquant:latest-iq4xs-direct.gguf` (no imatrix) is a fallback if the calibrated variant shows
...
The `.safetensors` LoRAs in `vendor/models/lora/` are already merged into `gemma4-rotorquant:latest.gguf`.
...
python llama.cpp/convert_lora_to_gguf.py \
  ...
  --lora  vendor/models/lora/gemma4-rotorquant:latest-grpo/adapter_model.safetensors \
  --outfile vendor/models/lora/gemma4-rotorquant:latest-grpo.gguf \
  --outtype f16

# Then in .env:
# LEGAL_LORA_PATH=C:\...\vendor\models\lora\gemma4-rotorquant:latest-grpo.gguf
