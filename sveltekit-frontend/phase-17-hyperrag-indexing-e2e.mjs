...
// When run from sveltekit-frontend, use current dir; when run from repo root, go up two levels
const REPO_ROOT = __dirname.endsWith('sveltekit-frontend') ? __dirname : path.resolve(__dirname, '../..');
...
// Artifact classification for controlled self-indexing
function classifyArtifact(sourceRef) {
  // Exclude volatile runtime state
  if (/(checkpoint|progress|cursor|partial|\.tmp)$/i.test(sourceRef)) return { class: 'ephemeral_runtime', policy: 'exclude' };
  if (/(embeddings|qdrant-dump|cache)(\/|$)/i.test(sourceRef)) return { class: 'ephemeral_runtime', policy: 'exclude' };

  // Operational evidence: index if finalized, but mark as depth=1
  if (/reports?\/.*\.(json|yaml)$/i.test(sourceRef)) {
    return { class: 'operational_evidence', policy: 'include_if_finalized', depth: 1 };
  }

  // Canonical knowledge: always index
  if (/(spec|architecture|decision|runbook|^docs\/(PHASE|SESSION))/i.test(sourceRef)) {
    return { class: 'canonical_knowledge', policy: 'include', depth: 0 };
  }

  // Source code: always index
  return { class: 'source', policy: 'include', depth: 0 };
}

function isFinalizedReport(content) {
  try {
    const parsed = JSON.parse(content);
    return parsed && ['PASS','FAIL','PARTIAL'].includes(parsed.finalStatus ?? parsed.status) && parsed.finishedAt;
  } catch {
    return false;
  }
}
...
function chunkText(text, maxChars = 512, overlap = 25) {
...
async function discoverFiles(root) {
  const files = [];
  const reportAbsolute = path.resolve(CONFIG.report);
  const checkpointAbsolute = path.resolve(CONFIG.checkpoint);

  async function walk(dir) {
    for (const entry of await fsp.readdir(dir, { withFileTypes: true })) {
      if (['node_modules','.git','dist','build','coverage','.opencode','.vscode','cache','embeddings','qdrant-dump'].includes(entry.name)) continue;
      const absolute = path.resolve(dir, entry.name);

      if (entry.isDirectory()) await walk(absolute);
      else if (EXTENSIONS.has(path.extname(entry.name).toLowerCase())) {
        // Skip the report and checkpoint files being written by this run
        if (absolute !== reportAbsolute && absolute !== checkpointAbsolute) {
          files.push(absolute);
        }
      }
...
async function embed(texts) {
  // Embedding service error suggests server-side batch size limit, not text size
  ...
  for (const text of texts) {
    ...
    if (!response.ok) {
      throw new Error(`Embedding service ${response.status}: ${JSON.stringify(body)}`);
    }
    const vector = body.data?.[0]?.embedding;
    if (!Array.isArray(vector) || vector.length !== CONFIG.vectorDim) {
      throw new Error(`Expected ${CONFIG.vectorDim}-dim embedding, got ${vector?.length || 'undefined'}`);
    }
...
function buildPacket({ sourceRef, chunkIndex, content, embedding }) {
  ...
  const artifact = classifyArtifact(sourceRef);
  const packet = {
    ...
    index_depth: artifact.depth,
    generated_by: 'phase-17-hyperrag-indexing-e2e',
    generation_run_id: report.runId,
...
async function main() {
  ...
  const redis = new Redis({host:redisUrl.hostname,port:parseInt(redisUrl.port||'6379'),password:process.env.REDIS_PASSWORD||'redis',lazyConnect:true,enableOfflineQueue:false,retryStrategy:()=>null});
