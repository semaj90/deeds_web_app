...
Output: Trained model saved to ./models/naive_bayes_models.json
...
MODEL_PATH = MODELS_DIR / 'naive_bayes_models.json'
VECTORIZER_PATH = MODELS_DIR / 'tfidf_vectorizer.json'
...
def vectorize_features(rows: List[Dict[str, Any]]) -> Tuple[np.ndarray, TfidfVectorizer]:
    """
...
def train_classifiers(rows: List[Dict[str, Any]], dry_run: bool = False):
    ...
    print(f"[DATA] Training on {len(rows)} packets...")
