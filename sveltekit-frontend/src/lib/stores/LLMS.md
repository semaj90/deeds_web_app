...
- [ ] **test-coverage** `src/lib/stores/analytics.svelte.ts` fanIn=14 — add G26-pattern test (14 consumers depend on this)
- [ ] **test-coverage** `src/lib/stores/notifications.svelte.ts` fanIn=10 — add G26-pattern test (10 consumers depend on this)
...
- `read_file({ filePath: "sveltekit-frontend/src/lib/stores/<file>" })` — fetch any file's contents (sandboxed to src/)
