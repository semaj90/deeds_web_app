...
- [ ] **test-coverage** `src/lib/ai/model-ids.ts` fanIn=33 — add G26-pattern test (33 consumers depend on this)
- [ ] **hotspot** `src/lib/ai/model-ids.ts` (fanIn=33) — consider splitting or adding circuit breaker; top callers: `client-embed.ts`, `client-router.ts`, `client-router.ts`
...
- `read_file({ filePath: "sveltekit-frontend/src/lib/ai/<file>" })` — fetch any file's contents (sandboxed to src/)
