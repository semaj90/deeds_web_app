# LLMS.md — `sveltekit-frontend/src/lib/ai`

# LLMS.md — `sveltekit-frontend/src/lib/ai`

# LLMS.md — `sveltekit-frontend/src/lib/ai`

# LLMS.md — `sveltekit-frontend/src/lib/ai`

# LLMS.md — `sveltekit-frontend/src/lib/ai`












































































<!-- LLMS-ENRICH v1 · auto-generated by enrich-llms-md.mjs · do not edit manually -->

## Enhanced Graph Context


| File | Authority | Flags |
|------|-----------|-------|
| `model-ids.ts` | 0.779 | 0x9 |
| `client-router.ts` | 0.517 | 0x9 |
| `base64-fp32-quantizer.ts` | 0.449 | 0x8 |
| `client-llm-synthesis.ts` | 0.150 | 0x9 |
| `client-embed.ts` | 0.150 | 0xb |
| `citation-cache.ts` | 0.150 | 0x9 |
| `emotion-context.ts` | 0.150 | 0x9 |
| `unified-generation.ts` | 0.150 | 0xb |
| `client-cache.ts` | 0.150 | 0x9 |
| `client-quality.ts` | 0.150 | 0x8 |
## Audit Gates — Client AI (ONNX / routing)

> Auto-mapped from CLAUDE.md §"Unified Audit Gate System". Last enriched: 2026-05-17
> Run each check from the **sveltekit-frontend/** root.

### Tier A — Code Connectivity

**G1** Static ESM imports
```bash
rg "from.*MODULE" src/ --type ts --type svelte
```

### Tier C — Infrastructure

**G17** No hardcoded localhost — use ENV.* getters
```bash
rg "localhost|127\.0\.0\.1" src/lib/server/ --type ts  # expect 0 outside env.server.ts
```

### Tier D — Security + Runtime

**G20** SSR safety — browser globals behind onMount guard
```bash
rg "window\.|document\.|localStorage|IndexedDB" src/lib/MODULE --type svelte
```

### Tier E — Svelte 5 Rune Compliance

**G25** No rune calls in plain .ts files
```bash
rg "\$(?:state|derived|effect|props)\s*[(<]" src/lib/ --type ts --glob "!*.svelte.ts" --glob "!*.d.ts"  # expect 0
```

## TODO — Enhancements from ACE Analysis

> Generated from Redis ACE hits (code:graph:node:* + hotspot data). Regenerate: `node scripts/enrich-llms-md.mjs`.

- [ ] **G17** Audit for remaining hardcoded `localhost` / `127.0.0.1` — replace with `ENV.*` getters



<!-- /LLMS-ENRICH -->
<!-- LLMS-GEN v1 · do not edit below this line -->
<!-- generated: 2026-05-15T17:39:23.555Z · llms.md spec · regen: npm run llms:write -->

> Directory: sveltekit-frontend/src/lib/ai

## Snapshot

- 14 file(s), 0 handler(s)
- Files: 14

## Top Files

- `citation-cache.ts`
- `client-cache.ts`
- `client-embed.ts`
- `client-llm-synthesis.ts`
- `client-quality.ts`
- `client-router.ts`
- `emotion-context.ts`
- `gemma4-e2b-client.ts`
- `hypergraph.ts`
- `model-ids.ts`

## Agentic tool-calling — quick ACE hits

- `read_file({ filePath: "sveltekit-frontend/src/lib/ai/<file>" })`
- `verify_fix({ filePath: "sveltekit-frontend/src/lib/ai/+server.ts" })`

## How to use this file

Agents automatically pick up the nearest `LLMS.md` when editing. The root `LLMS.md` provides repo-wide rules.

Run `npm run llms:write` to regenerate.