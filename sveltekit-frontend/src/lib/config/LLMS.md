# LLMS.md — `sveltekit-frontend/src/lib/config`
...
- [ ] **test-coverage** `src/lib/config/env.server.ts` fanIn=18 — add G26-pattern test (18 consumers depend on this)
...
> Directory: sveltekit-frontend/src/lib/config
...
- `read_file({ filePath: "sveltekit-frontend/src/lib/config/<file>" })` — fetch any file's contents (sandboxed to src/)
