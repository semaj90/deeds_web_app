...
> Directory audit: src/lib/db/queries

## Snapshot

- shared library directory with 2 files, 0 API handlers, 2 Drizzle refs
- Audit score: **100/100**
- no audit signals
...
- `src/lib/db/queries/route-health-archive.ts`
...
- `graph_search({ query: "queries", topK: 8 })` — files in this dir with tags, TODOs, audit flags
- `wiki_note_lookup({ query: "db queries", limit: 5 })` — KAG narrative + audit score
- `audit_hotspots({ limit: 10 })` — if this dir is failing gates, surfaces the broader hotspot set
- `read_file({ filePath: "src/lib/db/queries/<file>" })` — fetch any file's contents (sandboxed to src/)
