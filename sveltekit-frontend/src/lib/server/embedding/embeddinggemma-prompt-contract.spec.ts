...
describe('EmbeddingGemma prompt authority', () => {
  it('freezes retrieval, code, document and classification prompts', () => {
    expect(encodeRetrievalQuery('find the packet writer').formattedText)
      .toBe('task: search result | query: find the packet writer');
    expect(encodeCodeRetrievalQuery('GraphifyStructuralMaterializer upsert failure').formattedText)
      .toBe('task: code retrieval | query: GraphifyStructuralMaterializer upsert failure');
    ...
    expect(encodeClassificationQuery('debug database query').formattedText)
      .toBe('task: classification | query: debug database query');
  });
  ...
  it('freezes native dimension and maximum context', () => {
    ...
    expect(value.nativeDimension).toBe(768);
    expect(value.maxContextTokens).toBe(EMBEDDINGGEMMA_MAX_CONTEXT_TOKENS);
    expect(value.maxContextTokens).toBe(2048);
  });
  ...
  it('makes artifact and executor revision part of the cache identity', () => {
    const promptedInput = encodeRetrievalQuery('same text');
    const a = buildEmbeddingGemmaCacheIdentity({
      modelRevision: 'model-a', artifactChecksum: 'a'.repeat(64), executorRevision: 'exec-1', promptedInput, representationRevision: 'semantic-512-v1',
    });
    const b = buildEmbeddingGemmaCacheIdentity({
      modelRevision: 'model-a', artifactChecksum: 'b'.repeat(64), executorRevision: 'exec-2', promptedInput, representationRevision: 'semantic-512-v1',
    });
