...
- [ ] **test-coverage** `src/lib/server/vector/qdrant-manager.ts` fanIn=99 — add G26-pattern test (99 consumers depend on this)
- [ ] **hotspot** `src/lib/server/vector/qdrant-manager.ts` (fanIn=99) — consider splitting or adding circuit breaker; top callers: `agents-context-source.ts`, `context-assembler.ts`, `multi-lane-retrieval.ts`
...
- `read_file({ filePath: "sveltekit-frontend/src/lib/server/vector/<file>" })` — fetch any file's contents (sandboxed to src/)
