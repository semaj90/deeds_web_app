# LLMS.md — `sveltekit-frontend/src/lib/server/rate-limit`
...
- [ ] **test-coverage** `src/lib/server/rate-limit/redis-rate-limit.ts` fanIn=10 — add G26-pattern test (10 consumers depend on this)
- [ ] **G17** Audit for remaining hardcoded `localhost` / `127.0.0.1` — replace with `ENV.*` getters
...
> Directory: sveltekit-frontend/src/lib/server/rate-limit
...
- `read_file({ filePath: "sveltekit-frontend/src/lib/server/rate-limit/<file>" })` — fetch any file's contents (sandboxed to src/)
