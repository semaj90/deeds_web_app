# AGENTS.md — `src/lib/server/rate-limit`
...
> Directory audit: src/lib/server/rate-limit
...
- Audit score: **90/100**
- no audit signals
- Tags: `src` `lib` `server` `test`

## Files (2)

- `src/lib/server/rate-limit/redis-rate-limit.test.ts`
...
- `graph_search({ query: "rate-limit", topK: 8 })` — files in this dir with tags, TODOs, audit flags
- `wiki_note_lookup({ query: "server rate-limit", limit: 5 })` — KAG narrative + audit score
- `audit_hotspots({ limit: 10 })` — if this dir is failing gates, surfaces the broader hotspot set
- `read_file({ filePath: "src/lib/server/rate-limit/<file>" })` — fetch any file's contents (sandboxed to src/)
