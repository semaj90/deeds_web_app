...
import {
	compilePageRankGpuFabricRequest,
	compilePageRankParityFabricRequests,
	pageRankGpuFabricRequestToArgs,
	pageRankParityFabricRequestToArgs,
} from './pagerank-gpu-fabric-request.js';
...
function fixture() {
	...
	const snapshot = {
		...
		producerRevision: 'snapshot-test-v1', createdAt: '2026-08-19T20:01:00.000Z',
	};
	const plan = {
		schema: 'atlas.pagerank-execution-plan.v1' as const, runId: 'run-test', canonicalRunOwner: 'PARENT_ATLAS_GRAPH_ANALYSIS' as const,
		algorithmFamily: 'PAGERANK' as const, algorithm: 'pagerank' as const, algorithmRevision: 'cugraph-pagerank-challenger-v1',
		...
		producerRevision: 'request-test-v1', createdAt: '2026-08-19T20:02:00.000Z',
	};
	return { snapshot, plan };
}

describe('PageRank parity fabric request compiler', () => {
	it('compiles matched Neo4j GDS and cuGraph calls from one plan', () => {
		const { snapshot, plan } = fixture();
		const pair = compilePageRankParityFabricRequests({ snapshot, plan });
		expect(pair.reference.mode).toBe('graph_pagerank_neo4j_gds');
		expect(pair.reference.executorId).toBe('NEO4J_GDS');
		expect(pair.challenger.mode).toBe('graph_pagerank_cugraph');
		expect(pair.challenger.executorId).toBe('CUGRAPH');
		expect(pair.reference.parameterHash).toBe(pair.challenger.parameterHash);
		expect(pair.reference.projectionSnapshotHash).toBe(pair.challenger.projectionSnapshotHash);
		expect(pair.reference.relationshipTypes).toEqual(pair.challenger.relationshipTypes);
		expect(pageRankParityFabricRequestToArgs(pair.reference)).toContain('graph_pagerank_neo4j_gds');
		expect(pageRankParityFabricRequestToArgs(pair.challenger)).toContain('graph_pagerank_cugraph');
	});

	it('preserves the existing challenger-only compiler', () => {
		const { snapshot, plan } = fixture();
	...
	it('rejects PPR because graph_node_key is only a parity coordinate, not seed identity', () => {
		const { snapshot, plan } = fixture();
		const ppr = {
			...plan, algorithm: 'personalized_pagerank' as const,
			parameters: { ...plan.parameters, personalization: { mode: 'PERSONALIZED' as const, seeds: [{ canonicalId: 'packet:seed', weight: 1 }] } },
		};
		expect(() => compilePageRankParityFabricRequests({ snapshot, plan: ppr })).toThrow(/global PageRank only/);
	});

	it('rejects a plan bound to a different projection hash', () => {
		const { snapshot, plan } = fixture();
		const mismatchedPlan = { ...plan, projection: { ...plan.projection, projectionHash: 'different-hash' } };
		expect(() => compilePageRankParityFabricRequests({ snapshot, plan: mismatchedPlan })).toThrow();
	});
});
