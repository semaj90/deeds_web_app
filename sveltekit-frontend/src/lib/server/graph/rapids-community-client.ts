...
export interface RapidsCommunityNodeV1 {
	nodeId: string;
}

export interface RapidsCommunityEdgeV1 {
	source: string;
	target: string;
	weight: number;
}
...
export async function runRapidsCommunityV1(input: RunRapidsCommunityInputV1): Promise<RapidsCommunityPartitionResponseV1> {
	...
	if (!response.ok) {
		throw new Error(`RAPIDS community sidecar ${response.status}: ${await response.text()}`);
	}
	return CommunityPartitionResponseSchema.parse(await response.json());
}

/**
 * Convert a challenger response into the same revisioned snapshot vocabulary
 * used by custom Union-Find and Neo4j GDS results. The backend's ordinal ids
 * remain local to that run; stable identity is rebuilt from canonical members.
 */
