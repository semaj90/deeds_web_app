...
function nodeIdentityExpression(variable: string): string {
	return `coalesce(${variable}.canonicalId, ${variable}.canonical_id, ${variable}.packetKey, ${variable}.packet_key, ${variable}.stableKey, ${variable}.id, ${variable}.sourceRef, ${variable}.source_ref)`;
}
			...
			for (const parentCanonicalId of frontier) {
				...
				for (const relation of plan.relationships) {
					...
					const relationLimit = Math.max(
						0,
						Math.min(relation.maxNeighbors, nodeRemaining, remainingEdges, remainingNodes, remainingCandidates),
					);
					if (relationLimit === 0) {
						budgetExhausted = true;
						break;
					...
					const query = `
						MATCH (start)
						WHERE ${nodeIdentityExpression('start')} = $parentCanonicalId
						...
						RETURN canonicalId,
						       coalesce(neighbor.stableKey, neighbor.id, canonicalId) AS stableKey,
