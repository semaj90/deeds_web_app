    ...
    try {
      const nodeQuery = caseId
        ? `MATCH (c:Case {id: $caseId})-[r*1..2]-(n)
				   RETURN DISTINCT elementId(n) AS nodeId, labels(n)[0] AS label,
				          COALESCE(n.title, n.name, n.term, n.code, '') AS title,
				          n.file_path AS filePath, n.qdrant_id AS qdrantId
				   LIMIT $maxNodes`
        : `MATCH (n)
				   RETURN DISTINCT elementId(n) AS nodeId, labels(n)[0] AS label,
				          COALESCE(n.title, n.name, n.term, n.code, '') AS title,
      ...
      const nodes: GraphNode[] = nodeResult.records.map((r) => ({
        ...
        filePath: r.get('filePath') ? String(r.get('filePath')) : undefined,
        qdrantId: r.get('qdrantId') ? String(r.get('qdrantId')) : undefined,
      }));
  ...
  if (req.includePageRank !== false) {
    ...
    if (n > 0 && n <= 2000) {
      try {
        ...
        console.warn(
          '[gpu-graph-analysis] GPU PageRank failed, falling back to CPU:',
        ...
        if (embeddings.length >= 2) {
          ...
          result.gpuClusters = {
            ...
            assignments: Array.from(kmeansResult.assignments).map((cluster, i) => ({
              ...
              filePath: embeddings[i]?.filePath,
              qdrantId: embeddings[i]?.qdrantId,
              cluster,
            })),
            centroids,
          };
          ...
          if (req.attentionQuery && req.attentionQuery.length === dim && centroids.length > 0) {
            try {
              ...
              console.warn(
                '[gpu-graph-analysis] attentionScoreGPU failed:',
                (attnErr as Error)?.message
              );
            }
          }
        }
...
async function fetchNodeEmbeddings(qdrantUrl: string, nodeIds: string[]): Promise<NodeEmbedding[]> {
