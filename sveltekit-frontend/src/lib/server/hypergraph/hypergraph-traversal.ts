 ...
 *
 * Canonical relation truth stays in Hyperedge/HyperedgeMember. This module only
 * controls how much relation evidence may be expanded for one request. It is not
 * a second semantic retrieval owner.
 */
...
/**
 * One structural hop. We retain each edge's activation score so bounded traversal
 * can propagate scores from admitted edges only.
 */
async function hop(
  anchorKey: string,
  hopNum: number,
  ...
  step: TraversalStep;
  edges: Hyperedge[];
  scoredEdges: Array<{ edge: Hyperedge; score: number }>;
}> {
  const { results } = await searchHyperedges({ member_key: anchorKey, limit });
  const decay = 1 / hopNum;
  const scores: Record<string, number> = {};
  const scoredEdges = results.map((result) => ({
    edge: result.edge,
    score: result.activationScore * decay,
  }));
  ...
  return {
    ...
    edges: scoredEdges.map(({ edge }) => edge),
    scoredEdges,
  };
}
...
export async function traverseMultihop(
  ...
  const result = await traverseMultihopBounded(
    anchorKey,
    {
...
/**
 * Resource-aware n-ary expansion.
 *
 * Invariant: only ADMITTED hyperedges may contribute a score or seed the next
 * frontier. A relation returned by storage but rejected by edge/member/token/time
 * budget is not evidence and cannot influence later hops.
 *
 * Frontier order is deterministic marginal-utility order:
 * activation score descending, member key ascending as tie-break.
 */
  ...
  outer: for (let h = 1; h <= maxHops && frontier.length > 0; h++) {
    ...
    for (const anchor of frontier) {
      ...
      for (const { edge, score } of scoredEdges) {
        ...
        const alreadyAdmitted = allEdges.has(edge.id);
        const nextMembers = memberCount + (alreadyAdmitted ? 0 : edge.members.length);
        const nextTokens = estimatedTokens + (alreadyAdmitted ? 0 : edgeTokenEstimate(edge));

        if (!alreadyAdmitted && allEdges.size + 1 > budget.maxEdges) {
          exhaustedBy.push('edges');
          break outer;
        }
        if (!alreadyAdmitted && nextMembers > budget.maxMembers) {
          exhaustedBy.push('members');
          continue;
        }
        if (!alreadyAdmitted && nextTokens > budget.maxTokens) {
        ...
        // Score propagation happens only after edge admission.
        for (const member of edge.members) {
          const key = member.member_key;
          if (key === anchor || visited.has(key)) continue;
...
export async function traverseHypergraph(params: HyperedgeSearchParams): Promise<TraversalResult> {
  ...
  switch (mode) {
    ...
    case 'multihop':
      return traverseMultihop(anchor, maxHops, Math.max(1, Math.ceil(limit / Math.max(1, maxHops))));
    default:
