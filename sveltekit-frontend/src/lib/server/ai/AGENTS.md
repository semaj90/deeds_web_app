...
- [ ] **hotspot** `src/lib/server/ai/compact-budgets.ts` (fanIn=18) — consider splitting or adding circuit breaker; top callers: `+server.ts`, `+server.ts`, `+server.ts`
...
| Gate | Status | Detail |
|------|--------|--------|
| G17 | ❌ FAIL | 1/38 files — use env.server.ts getters |

_Gates checked: G17. Run `npm run index:codebase:fast && npm run agents:write` to refresh._
## Todos + Enhancements

- **[HIGH]** [G17] Fix **G17** No hardcoded localhost URLs: 1/38 files — use env.server.ts getters
