...
- server module directory with 48 files, 0 API handlers, 7 Drizzle refs
- Audit score: **95/100**
...
| Gate | Status | Detail |
|------|--------|--------|
| G17 | ❌ FAIL | 1/48 files — use env.server.ts getters |

_Gates checked: G17. Run `npm run index:codebase:fast && npm run agents:write` to refresh._
## Todos + Enhancements

- **[HIGH]** [G17] Fix **G17** No hardcoded localhost URLs: 1/48 files — use env.server.ts getters
...
- **Paired tests**: 5/48 files have paired tests
