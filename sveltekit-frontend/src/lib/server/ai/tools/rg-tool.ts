...
import { executeEnhancedRgSearch, type RgSearchResult } from '$lib/server/indexer/rg-search-utility.js';
...
export async function runRgTool(input: RgToolInput): Promise<RgToolOutput> {
  ...
  const rawMatches = executeEnhancedRgSearch(query, searchPath).slice(0, limit);
  const llm_output = rawMatches
