...
/** Preserve deterministic K/V provenance without echoing raw values into traces. */
export function summarizeToolArgs(toolArgs: Record<string, unknown> | undefined): {
  keys: string[];
  checksum: string | null;
} {
  if (!toolArgs) return { keys: [], checksum: null };
  return { keys: Object.keys(toolArgs).sort(), checksum: recommendationHash(toolArgs) };
}
