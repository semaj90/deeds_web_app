...
function classifyQueryTier(query: string): CacheTier {
  ...
  if (/\b(statute|u\.?s\.?c|cfr|code|section|§|precedent|case law|ruling)\b/.test(q))
    return 'precedent';
...
export async function lookupCachedResponse(params: {
  ...
  try {
    ...
    if (expiresAt < new Date()) {
      ...
      qdrant.client
        .delete(qdrant.collections.llm_cache, {
    ...
    console.log(
      `[LLM Cache] ✅ HIT — similarity: ${topHit.score?.toFixed(3)}, cached: ${payload.cachedAt}`
    );
...
export async function storeCachedResponse(params: {
  ...
  try {
    ...
    await qdrant.upsert({
      collection: qdrant.collections.llm_cache,
      points: [
        {
          id: pointId,
          vector: {
    ...
    console.log(
      `[LLM Cache] STORED — tier: ${tier}, ttl: ${ttlSeconds}s, query: "${query.slice(0, 50)}...", expires: ${expiresAt.toISOString()}`
    );
  } catch (error) {
...
export async function cleanExpiredCache(): Promise<{ deleted: number }> {
  try {
    ...
    const scrollRes = await qdrantClient.scroll(qdrant.collections.llm_cache, {
      limit: 1000,
      with_payload: true,
      filter: {
        must: [
    ...
    if (expiredIds.length > 0) {
      await qdrantClient.delete(qdrant.collections.llm_cache, {
