...
- [ ] **test-coverage** `src/lib/server/streaming/sse-utils.ts` fanIn=10 — add G26-pattern test (10 consumers depend on this)
- [ ] **G17** Audit for remaining hardcoded `localhost` / `127.0.0.1` — replace with `ENV.*` getters
...
- `read_file({ filePath: "sveltekit-frontend/src/lib/server/streaming/<file>" })` — fetch any file's contents (sandboxed to src/)
