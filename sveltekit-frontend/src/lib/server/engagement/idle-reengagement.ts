...
export async function recordHeartbeat(userId: string): Promise<void> {
  try {
    ...
    const key = userEngagementKey.activity(userId);
    const now = Date.now();
    await redis.set(key, String(now), 'EX', TTL.USER_ACTIVITY);
  } catch {
...
async function hasNotifiedToday(userId: string, tier: IdleTier): Promise<boolean> {
  try {
    ...
    const key = userEngagementKey.notify(userId, tier, dateStr);
...
export async function scanIdleUsers(): Promise<{
  ...
  const doScan = async () => {
    ...
    for (const key of keys) {
      result.scanned++;
      const userId = key.slice(userEngagementKey.activity('').length);
      ...
      try {
        ...
        console.log(
          `[Idle] Sent ${applicableTier.name} to user ${userId.slice(0, 8)}... (idle ${Math.round(idleDuration / 60000)}min)`
        );
      } catch (err) {
        result.errors++;
