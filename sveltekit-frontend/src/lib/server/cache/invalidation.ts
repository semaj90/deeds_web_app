...
export const CACHE_PATTERNS = {
  ...
  // TurboQuant KV prefix cache (llama-server GPU slots)
  TURBO_PREFIX: 'turbo:prefix:*',
  TURBO_WARM: 'turbo:warm:*',
  TURBO_DYM: 'turbo:dym:*',

  // Graph neighbor caches (Neo4j → Qdrant retrieval filters)
  GRAPH_NEIGHBORS: 'graph:case:*:neighbors',

  // Context assembler bundles
  KB_BUNDLE: 'kb_bundle:*',
  RESEARCH_BUNDLE: 'research_bundle:*',

  // Cluster summaries (generated by GPU indexing pipeline)
  CLUSTER_SUMMARY: 'summary:cluster:*',

  // Community graph (GraphRAG Leiden communities over GPU clusters)
  COMMUNITY_GRAPH: 'hg:community:*',
  COMMUNITY_IDX: 'hg:community:idx',
...
/**
 * Invalidate all downstream caches after codebase re-indexing.
 * Clears TurboQuant KV prefixes, graph neighbors, context bundles,
 * cluster summaries, RAG search, and LLM semantic cache.
 * Also purges CouchDB DAG cache (non-Redis, fire-and-forget).
 */
export const invalidateIndexingCaches = async (userId?: string) => {
	// Fire-and-forget DAG cache purge (CouchDB, not Redis)
	import('$lib/server/cache/dag-cache.js')
		.then((m) => m.purgeAllDAGCache?.())
		.catch(() => {});

	// Also clear community graph so next ACE assembly gets fresh Leiden communities
	import('$lib/server/graph/community-graph.js')
		.then((m) => m.invalidateCommunityCache?.())
		.catch(() => {});

	return cacheInvalidation.invalidateMultiple(
		[
			CACHE_PATTERNS.TURBO_PREFIX,
			CACHE_PATTERNS.TURBO_WARM,
			CACHE_PATTERNS.TURBO_DYM,
			CACHE_PATTERNS.GRAPH_NEIGHBORS,
			CACHE_PATTERNS.KB_BUNDLE,
			CACHE_PATTERNS.RESEARCH_BUNDLE,
			CACHE_PATTERNS.CLUSTER_SUMMARY,
			CACHE_PATTERNS.COMMUNITY_GRAPH,
			CACHE_PATTERNS.RAG_SEARCH,
			CACHE_PATTERNS.LLM_SEMANTIC,
		],
		{ type: 'indexing_complete', userId, immediate: true }
	);
};

/**
 * Invalidate community graph caches (after cluster reassign or hypergraph rebuild).
 * Clears all hg:community:* keys so getCommunityContext() re-queries Postgres.
 */
export const invalidateCommunityGraphCaches = async (userId?: string) => {
	// Also fire the in-process invalidator from community-graph.ts
	import('$lib/server/graph/community-graph.js')
		.then((m) => m.invalidateCommunityCache?.())
		.catch(() => {});
	...
	return cacheInvalidation.invalidateMultiple(
		...
		{ type: 'cluster_reassign', userId, immediate: true }
	);
};

/**
 * Lighter invalidation after research/summary updates.
 * Clears TurboQuant prefix anchors (which embed research summaries)
 * and context bundles that may reference stale data.
 */
export const invalidateResearchCaches = async (userId?: string) => {
	return cacheInvalidation.invalidateMultiple(
		[
			CACHE_PATTERNS.TURBO_PREFIX,
			CACHE_PATTERNS.TURBO_WARM,
		...
		{ type: 'research_update', userId, immediate: true }
	);
};
