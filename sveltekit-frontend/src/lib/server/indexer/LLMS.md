...
- [ ] **test-coverage** `src/lib/server/indexer/karpathy-wiki.ts` fanIn=16 — add G26-pattern test (16 consumers depend on this)
- [ ] **test-coverage** `src/lib/server/indexer/dual-embedder.ts` fanIn=11 — add G26-pattern test (11 consumers depend on this)
- [ ] **test-coverage** `src/lib/server/indexer/legal-chunker.ts` fanIn=11 — add G26-pattern test (11 consumers depend on this)
...
- `read_file({ filePath: "sveltekit-frontend/src/lib/server/indexer/<file>" })` — fetch any file's contents (sandboxed to src/)
