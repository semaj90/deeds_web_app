...
export class DecisionEngine {
	 ...
	 */
	async decide(error: any, strategy: FixStrategy, context: any): Promise<DecisionResult> {
		this.stats.totalDecisions++;
		const confidence = strategy.confidence;

		if (confidence >= this.config.highConfidenceThreshold) {
			return { action: 'auto_apply', confidence, strategy };
		}

		if (confidence >= this.config.mediumConfidenceThreshold) {
			return { action: 'validate_then_apply', confidence, strategy };
		}

		if (confidence >= this.config.criticalConfidenceThreshold) {
			return { action: 'invoke_tools', confidence, strategy };
		}
	 ...
	 */
	async processError(error: any, strategy: FixStrategy, context: any): Promise<ProcessResult> {
	...
	private async handleAutoApply(
		error: any, strategy: FixStrategy, context: any, toolsInvoked: string[]
		...
		const applyResult = await synthesizer.synthesizeFix(error, []);
		...
		const recordResult = await recorder.recordExperience(
			error, strategy, outcome, context, toolsInvoked, false
		);
	...
	private async handleValidateThenApply(
		...
		const synthesizer = getFixSynthesizer();
		const validationResult = await synthesizer.validateFix(strategy, error);
	...
	private async handleInvokeTools(
		...
		if (updatedConfidence >= this.config.mediumConfidenceThreshold) {
			...
			const recorder = getExperienceRecorder();
			const recordResult = await recorder.recordExperience(
				error, updatedStrategy, outcome, context, toolsInvoked, false
			);
	...
	private async handleEscalate(
		error: any, strategy: FixStrategy, context: any, reason?: string
	): Promise<ProcessResult> {
		this.stats.escalated++;

		const recorder = getExperienceRecorder();
		const recordResult = await recorder.recordExperience(
			error, strategy, 'failure', context, [], true, reason
		);
