# LLMS.md — `sveltekit-frontend/src/lib/server/services`
...
- [ ] **test-coverage** `src/lib/server/services/couchdb-client.ts` fanIn=27 — add G26-pattern test (27 consumers depend on this)
- [ ] **hotspot** `src/lib/server/services/couchdb-client.ts` (fanIn=27) — consider splitting or adding circuit breaker; top callers: `tag-sync.ts`, `tag-sync.ts`, `agents-card-store.ts`
...
> Directory: sveltekit-frontend/src/lib/server/services
...
- `read_file({ filePath: "sveltekit-frontend/src/lib/server/services/<file>" })` — fetch any file's contents (sandboxed to src/)
