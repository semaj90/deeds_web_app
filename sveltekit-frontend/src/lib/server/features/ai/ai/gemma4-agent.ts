    ...
    if (name === 'trace_search') {
      ...
      return {
        ...
        result: hits.map((h) => ({
          id: h.id,
          score: h.score,
          source_ref: h.payload?.source_ref ?? h.payload?.path,
          path: h.payload?.source_ref ?? h.payload?.path,
          content: String(h.payload?.content ?? '').slice(0, 1000),
          summary: h.payload?.summary,
          workspace_revision: h.payload?.workspace_revision,
          representation_revision: h.payload?.representation_revision,
          source_revision: h.payload?.source_revision,
          source_representation_id: h.payload?.source_representation_id,
          source_dimension: h.payload?.source_dimension,
          pagerank_score: h.payload?.pagerank_score,
          canonical_join_status: h.payload?.canonical_join_status,
          canonical_join_coverage: h.payload?.canonical_join_coverage,
          lenses: h.lenses,
          tags: h.payload?.tags,
        })),
      };
