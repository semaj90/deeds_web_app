...
import { applyStyle, type LegalPersona } from '../../../ace/style-adapter.js';
import {
  webSearch,
...
import { fetchUserAnalyticsContext, fetchTopQueryTags } from '../../../ace/user-analytics-context.js';
import {
...
} from '../../../ace/context-cache-planner.js';
...
import type { HotCluster } from '../../../ace/hot-cluster-reader.js';
import {
  getTopoCandidates,
  setTopoCandidates,
...
import { fetchDeepImportGraphExpansion } from '../../../ace/graph-expander.js';
import { runRetrievalLanes, type MultiLaneOutput } from '../../../ace/retrieval-lanes.js';
import {
  QueryRouter4x4,
  extractSignal,
  rrfFuse,
...
import {
  ...
  MATRIX_VERSION,
} from '../../../ace/atlas-routing-matrix.js';
  ...
  return traceGraph(
    ...
    async () => {
        ...
        try {
          ...
          if (cartridgeResults.confidence >= configuredThreshold) {
            ...
            if (topChunk?.gpuCluster != null) {
              const { readLatestQdrantClusterTags } = await import('../../../ace/cluster-tags-cache.js');
      ...
      const [
        ...
        opts.enableCodebaseContext
          ? import('../../../ace/multi-lane-retrieval.js')
      ...
      (async () => {
        try {
          const { logHMMAnalysis } = await import('../../../ace/hmm-wiki-logger.js');
          const topChunks = [
      ...
      if (opts.tokenAwarePacking) {
        const { packAceContext } = await import('../../../ace/token-aware-context-packer.js');
        const redis = getRedis();
    ...
    try {
      ...
      if (hasEmbeds) {
        ...
        const attnScores = chunkEmbeds.length > 256
          ...
          : await attentionScoreChunks(embedding, chunkEmbeds);
        ragChunks = ragChunks
          .map((c, i) => ({
            ...c,
  ...
  try {
    ...
    try {
      ...
      if (bowQueryTerms.size > 0) {
        const { getRedis } = await import('$lib/server/redis.js');
        const { readHotClusters } = await import('../../../ace/hot-cluster-reader.js');
        const { mergeClusterCandidates } = await import('../../../ace/merge-cluster-candidates.js');
