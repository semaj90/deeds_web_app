...
- [ ] **test-coverage** `src/lib/server/observability/langfuse.ts` fanIn=48 — add G26-pattern test (48 consumers depend on this)
- [ ] **test-coverage** `src/lib/server/observability/inference-log.ts` fanIn=11 — add G26-pattern test (11 consumers depend on this)
- [ ] **hotspot** `src/lib/server/observability/langfuse.ts` (fanIn=48) — consider splitting or adding circuit breaker; top callers: `context-assembler.ts`, `self-prompt.ts`, `tag-sync.ts`
...
- `read_file({ filePath: "sveltekit-frontend/src/lib/server/observability/<file>" })` — fetch any file's contents (sandboxed to src/)
