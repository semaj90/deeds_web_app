# AGENTS.md — `src/lib/server/simulation`
...
> Directory audit: src/lib/server/simulation
...
- `src/lib/server/simulation/grpo-types.ts`
...
- `graph_search({ query: "simulation", topK: 8 })` — files in this dir with tags, TODOs, audit flags
- `wiki_note_lookup({ query: "server simulation", limit: 5 })` — KAG narrative + audit score
- `audit_hotspots({ limit: 10 })` — if this dir is failing gates, surfaces the broader hotspot set
- `read_file({ filePath: "src/lib/server/simulation/<file>" })` — fetch any file's contents (sandboxed to src/)
