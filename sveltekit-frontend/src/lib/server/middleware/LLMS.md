...
- [ ] **test-coverage** `src/lib/server/middleware/cache-headers.ts` fanIn=110 — add G26-pattern test (110 consumers depend on this)
- [ ] **hotspot** `src/lib/server/middleware/cache-headers.ts` (fanIn=110) — consider splitting or adding circuit breaker; top callers: `+server.ts`, `+server.ts`, `+server.ts`
- [ ] **G17** Audit for remaining hardcoded `localhost` / `127.0.0.1` — replace with `ENV.*` getters
- [ ] **G8a** Use `HttpServiceError` subclasses (`UnauthorizedError`, `NotFoundError`, etc.) — never `throw error()` from `@sveltejs/kit` in service layer
...
- `read_file({ filePath: "sveltekit-frontend/src/lib/server/middleware/<file>" })` — fetch any file's contents (sandboxed to src/)
