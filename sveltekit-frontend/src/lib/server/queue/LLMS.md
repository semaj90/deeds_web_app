...
- [ ] **test-coverage** `src/lib/server/queue/dispatch-inline.ts` fanIn=19 — add G26-pattern test (19 consumers depend on this)
...
- [ ] **hotspot** `src/lib/server/queue/dispatch-inline.ts` (fanIn=19) — consider splitting or adding circuit breaker; top callers: `invalidation.ts`, `cuda-bridge.ts`, `queue-worker.ts`
...
- `read_file({ filePath: "sveltekit-frontend/src/lib/server/queue/<file>" })` — fetch any file's contents (sandboxed to src/)
