...
export const eventFabricEnvelopeSchema = z.object({
  ...
  eventSource: z.string().min(1).default('parent-atlas'),
...
export const artifactMaterializedPayloadSchema = z.object({
  ...
  if ((value.storage === 'FILESYSTEM' || value.storage === 'ARROW_MMAP') && !value.locatorPath) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['locatorPath'], message: `${value.storage} artifacts require locatorPath` });
  }
});
...
export type EventFabricHandler<TEvent extends EventFabricEventV1 = EventFabricEventV1> = (event: TEvent) => Promise<void>;
