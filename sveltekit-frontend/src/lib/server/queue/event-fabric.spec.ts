...
describe('event fabric contracts', () => {
  ...
  it('parses artifact materialized events using the canonical storage vocabulary', () => {
    const event = eventFabricEventSchema.parse({
      ...baseEvent,
      eventType: 'artifact.materialized',
      payload: {
        actionKey: 'action-key-00000001',
        artifactId: 'feature-matrix-1',
        artifactHash: 'artifact-hash-0001',
        checksum: 'checksum-value-0001',
        revisionSetHash: 'revision-set-0001',
        storage: 'ARROW_IPC',
        locatorPath: '.tmp/feature-matrix.arrow',
        byteLength: 4096,
        producer: 'feature-matrix-materializer',
        producerRevision: 'producer-v1',
      },
    });

    expect(event.eventType).toBe('artifact.materialized');
    if (event.eventType === 'artifact.materialized') {
      expect(event.payload.storage).toBe('ARROW_IPC');
    }
  });

  it('parses artifact failed events with retry semantics', () => {
    const event = eventFabricEventSchema.parse({
      ...baseEvent,
      ...
      payload: {
        ...
        retryBudget: 3,
        errorHash: 'error-hash-1',
        inputArtifactRefs: ['artifact-input-1'],
        revisionSetHash: 'revision-set-0001',
      },
    });
