 ...
 *
 * Purpose: Ensure proper connection lifecycle and error isolation
 * when handling multiple concurrent QUIC requests.
 *
 * Features:
 * - Singleton pattern for each connection type
 * - Lazy initialization
 * - Error isolation
 ...
 */
export async function healthCheckRabbitMQ(): Promise<boolean> {
	try {
...
export async function closeAllConnections(): Promise<void> {
	...
	await Promise.allSettled([closeRedis(), closeNeo4j(), closeRabbitMQ()]);
