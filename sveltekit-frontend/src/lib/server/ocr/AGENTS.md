...
> Directory audit: src/lib/server/ocr
...
- `src/lib/server/ocr/tesseract.ts`
...
- `graph_search({ query: "ocr", topK: 8 })` — files in this dir with tags, TODOs, audit flags
- `wiki_note_lookup({ query: "server ocr", limit: 5 })` — KAG narrative + audit score
- `audit_hotspots({ limit: 10 })` — if this dir is failing gates, surfaces the broader hotspot set
- `read_file({ filePath: "src/lib/server/ocr/<file>" })` — fetch any file's contents (sandboxed to src/)
