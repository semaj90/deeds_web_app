...
export const OntologyTupleAdmissionStatusSchema = z.enum([
  'ADMITTED_DERIVED_EVIDENCE',
  'EVENT_MISSING',
  'IDENTITY_MISMATCH',
  'REVISION_MISMATCH',
  'DUPLICATE_TUPLE',
]);
export type OntologyTupleAdmissionStatus = z.infer<typeof OntologyTupleAdmissionStatusSchema>;

export const OntologyTupleAdmissionResultSchema = z
  .object({
    tuples: z.array(OntologyEventTupleSchema),
    rejected: z.array(
      z.object({
        tupleId: z.string().min(1),
        status: OntologyTupleAdmissionStatusSchema,
        reason: z.string().min(1),
      })
    ),
    candidateCount: z.number().int().min(0),
    admittedCount: z.number().int().min(0),
    voteCount: z.literal(0),
    canonicalAuthority: z.literal(false),
    writesPerformed: z.literal(false),
  })
  .strict();
export type OntologyTupleAdmissionResult = z.infer<typeof OntologyTupleAdmissionResultSchema>;

export interface OntologyTupleAdmissionCandidate {
  packetKey: string;
  sourceRef: string;
...
export function canonicalizeAtlasEvent(
  input: Omit<AtlasEvent, 'eventId'>
): Omit<AtlasEvent, 'eventId'> {
...
/**
 * Admit n-ary tuples only as evidence for already-admitted candidates.
 * This function never creates candidates, pairwise edges, retrieval hits, or
 * fusion votes. The tuple's event must carry the same packet/source and
 * revision coordinates as the admitted candidate.
 */
export function admitOntologyEventTuplesV1(input: {
  events: readonly AtlasEvent[];
  tuples: readonly OntologyEventTuple[];
  candidates: readonly OntologyTupleAdmissionCandidate[];
}): OntologyTupleAdmissionResult {
  const eventsById = new Map(input.events.map((event) => [event.eventId, event]));
  const candidatesByPacket = new Map(
    input.candidates.map((candidate) => [candidate.packetKey, candidate])
  );
  const seenTupleIds = new Set<string>();
  const admitted: OntologyEventTuple[] = [];
  const rejected: Array<{ tupleId: string; status: OntologyTupleAdmissionStatus; reason: string }> =
    [];

  for (const tuple of input.tuples) {
    if (seenTupleIds.has(tuple.tupleId)) {
      rejected.push({
        tupleId: tuple.tupleId,
        status: 'DUPLICATE_TUPLE',
        reason: 'tupleId already admitted in this batch',
      });
      continue;
    }
    seenTupleIds.add(tuple.tupleId);
    const event = eventsById.get(tuple.eventId);
    if (!event) {
      rejected.push({
        tupleId: tuple.tupleId,
        status: 'EVENT_MISSING',
        reason: 'tuple event is not present in the supplied event evidence',
      });
      continue;
    }
    if (
      tuple.sourceRevision !== event.sourceRevision ||
      tuple.representationRevision !== event.representationRevision
    ) {
      rejected.push({
        tupleId: tuple.tupleId,
        status: 'REVISION_MISMATCH',
        reason: 'tuple revision does not match its source event',
      });
      continue;
    }
    if (!event.packetKey || tuple.subjectId !== event.eventId) {
      rejected.push({
        tupleId: tuple.tupleId,
        status: 'IDENTITY_MISMATCH',
        reason: 'tuple is not anchored to a canonical packet-bearing event',
      });
      continue;
    }
    const candidate = candidatesByPacket.get(event.packetKey);
    if (
      !candidate ||
      candidate.sourceRef !== event.sourceRef ||
      candidate.workspaceRevision !== event.workspaceRevision ||
      candidate.sourceRevision !== event.sourceRevision ||
      candidate.representationRevision !== event.representationRevision
    ) {
      rejected.push({
        tupleId: tuple.tupleId,
        status: 'IDENTITY_MISMATCH',
        reason: 'event identity is not present in the admitted candidate cohort',
      });
      continue;
    }
    admitted.push(tuple);
  ...
  return OntologyTupleAdmissionResultSchema.parse({
    tuples: admitted,
    rejected,
...
export function judgeRecommendation(
  input: EventRecommendationFeatureRow & {
    policyRevision: string;
  }
): RecommendationJudgment {
  const structuralCore = (input.semanticScore + input.structuralScore + input.graphScore) / 3;
  const operationalSignals = [input.workflowScore, input.breadthScore, input.freshnessScore].filter(
  ...
  const rawScore =
    structuralCore * 0.45 +
    operationalCore * 0.35 +
    input.evidenceCoverage * 0.1 +
    input.utilityBias * 0.1 -
    approximationPenalty -
    costPenalty;
...
export function compareAgainstExactOracle(input: {
  ...
  const precisionAtK =
    candidateKeys.length === 0 ? 1 : intersectionKeys.length / candidateKeys.length;
  ...
  const ndcgAtK =
    idealScores.length === 0 ? 1 : dcg(relevanceScores) / Math.max(1e-9, dcg(idealScores));
