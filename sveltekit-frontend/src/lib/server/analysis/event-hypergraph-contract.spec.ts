...
describe('event-hypergraph contract', () => {
  ...
  it('admits tuples only for the current candidate cohort and emits no retrieval votes', () => {
    const event = buildAtlasEvent({
      schemaVersion: 'atlas.event.hypergraph.v1',
      eventType: 'test_execution',
      ...
      evidenceRefs: ['evidence:1'],
      participants: [
        { entityId: 'function:f', entityKind: 'symbol', role: 'actor' },
        { entityId: 'test:t', entityKind: 'test', role: 'result' },
      ],
      metadata: {},
    });
    const [tuple] = compileOntologyEventTuples(event);
    const result = admitOntologyEventTuplesV1({
      events: [event],
      tuples: [tuple!, tuple!],
      candidates: [
        {
    ...
    expect(result.rejected[0]?.status).toBe('DUPLICATE_TUPLE');
    expect(result.voteCount).toBe(0);
    expect(result.canonicalAuthority).toBe(false);
  });

  it('rejects a tuple when the event revision is not in the admitted cohort', () => {
    const event = buildAtlasEvent({
      schemaVersion: 'atlas.event.hypergraph.v1',
      eventType: 'test_execution',
      sourceRef: 'src/example.ts',
    ...
    const result = admitOntologyEventTuplesV1({
      events: [event],
      tuples: [{ ...tuple!, sourceRevision: 'source-old' }],
      candidates: [
        {
          packetKey: 'packet:1',
          sourceRef: 'src/example.ts',
    ...
    expect(result.rejected[0]?.status).toBe('REVISION_MISMATCH');
  });
...
describe('event ordering, idempotency, and replay stability (Lane A)', () => {
  function makeInput(
    overrides: Partial<Omit<AtlasEvent, 'eventId'>> = {}
  ...
  it('replaying the same source revision twice produces identical event IDs and participant sets', () => {
    ...
    expect(runTwo.map((event) => event.participants)).toEqual(
      runOne.map((event) => event.participants)
    );
  });
  ...
  it('sorting is idempotent — sorting an already-sorted list is a no-op', () => {
    const events = [
      buildAtlasEvent(
        makeInput({ eventType: 'call_execution', observedAt: '2026-08-11T00:00:00.000Z' })
      ),
      buildAtlasEvent(
        makeInput({ eventType: 'call_execution', observedAt: '2026-08-11T00:00:01.000Z' })
      ),
      buildAtlasEvent(
        makeInput({ eventType: 'test_execution', observedAt: '2026-08-11T00:00:00.000Z' })
      ),
    ];
  ...
  it('sort order is stable regardless of arrival order (replay from a different transport order)', () => {
    const events = [
      ...
      buildAtlasEvent(
        makeInput({ eventType: 'reference_link', observedAt: '2026-08-11T00:00:02.000Z' })
      ),
    ];
