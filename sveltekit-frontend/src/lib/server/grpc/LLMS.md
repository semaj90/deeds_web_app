...
- [ ] **test-coverage** `src/lib/server/grpc/embedding-client.ts` fanIn=103 — add G26-pattern test (103 consumers depend on this)
- [ ] **test-coverage** `src/lib/server/grpc/graph-ml-client.ts` fanIn=10 — add G26-pattern test (10 consumers depend on this)
- [ ] **hotspot** `src/lib/server/grpc/embedding-client.ts` (fanIn=103) — consider splitting or adding circuit breaker; top callers: `agents-context-source.ts`, `context-assembler.ts`, `context-assembler.ts`
...
- `read_file({ filePath: "sveltekit-frontend/src/lib/server/grpc/<file>" })` — fetch any file's contents (sandboxed to src/)
