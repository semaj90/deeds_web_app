# LLMS.md — `sveltekit-frontend/src/lib/server/storage`
...
|--------|-----------|---------|
| `51181671f6` | 2026-03-25T20:28 | feat(security): production readiness hardening — error leak sanitization, auth ownership, CSRF, rate limiting |
...
> Directory: sveltekit-frontend/src/lib/server/storage
...
- `graph_search({ query: "storage", topK: 8 })` — files in this dir with tags, TODOs, audit flags
- `wiki_note_lookup({ query: "server storage", limit: 5 })` — KAG narrative + audit score
- `audit_hotspots({ limit: 10 })` — if this dir is failing gates, surfaces the broader hotspot set
- `read_file({ filePath: "sveltekit-frontend/src/lib/server/storage/<file>" })` — fetch any file's contents (sandboxed to src/)
