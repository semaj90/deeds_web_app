# AGENTS.md — `src/lib/server/rg-atlas`
...
> Directory audit: src/lib/server/rg-atlas
...
- `src/lib/server/rg-atlas/cosine-blend.ts`
- `src/lib/server/rg-atlas/embed.ts`
- `src/lib/server/rg-atlas/karpathy-blend.ts`
- `src/lib/server/rg-atlas/kmeans-cluster.ts`
...
- `wiki_note_lookup({ query: "server rg-atlas", limit: 5 })` — KAG narrative + audit score
- `audit_hotspots({ limit: 10 })` — if this dir is failing gates, surfaces the broader hotspot set
- `read_file({ filePath: "src/lib/server/rg-atlas/<file>" })` — fetch any file's contents (sandboxed to src/)
