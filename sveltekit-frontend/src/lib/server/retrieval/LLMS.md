...
- [ ] **test-coverage** `src/lib/server/retrieval/manifold4-search.ts` fanIn=12 — add G26-pattern test (12 consumers depend on this)
...
- **Paired tests**: 7/47 files have paired tests
...
- `read_file({ filePath: "sveltekit-frontend/src/lib/server/retrieval/<file>" })` — fetch any file's contents (sandboxed to src/)
