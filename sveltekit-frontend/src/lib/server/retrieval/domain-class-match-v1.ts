...
function makeReceipt(input: Omit<DomainClassMatchV1, 'schema' | 'comparisonChecksum'>): DomainClassMatchV1 {
  const payload = {
    schema: 'atlas.domain-class-match.v1' as const,
    ...input,
  };
  return DomainClassMatchV1Schema.parse({
    ...payload,
    comparisonChecksum: hash(canonicalJson(payload)),
  });
}

/**
 * Produce a deterministic query↔candidate-domain feature without changing
 * retrieval lane membership or canonical rerank weights.
 *
 * Feature semantics intentionally preserve the historical XGBoost column
 * shape while replacing its weak concept-map authority:
 *   1.0  canonical query primary domain == proven candidate domain
 *   0.5  proven candidate domain appears as a canonical query secondary domain
 *   0.0  both sides are proven and disagree
 *   null either side lacks enough lineage/classification evidence
 *
 * `general` is a fallback label, not a canonical domain match and therefore
 * remains null rather than becoming a misleading numeric zero.
 */
export function compareQueryDomainToPacketV1(input: {
  ...
  if (!input.lineage.lineageProven) {
    return makeReceipt({
      ...base,
      domainClassMatch: null,
      matchKind: 'CANDIDATE_DOMAIN_LINEAGE_UNPROVEN',
      featureEligible: false,
    });
  }

  if (!input.lineage.domainClass || input.lineage.domainClass === 'general') {
    return makeReceipt({
      ...base,
      domainClassMatch: null,
      matchKind: 'CANDIDATE_DOMAIN_FALLBACK',
      featureEligible: false,
    });
  }
  ...
  if (!queryDomainClass) {
    return makeReceipt({
      ...base,
      domainClassMatch: null,
      matchKind: 'QUERY_DOMAIN_UNRESOLVED',
      featureEligible: false,
    });
  }

  if (input.lineage.domainClass === queryDomainClass) {
    return makeReceipt({
      ...base,
      domainClassMatch: 1,
  ...
  if (querySecondaryDomains.includes(input.lineage.domainClass)) {
    return makeReceipt({
      ...base,
      domainClassMatch: 0.5,
      matchKind: 'SECONDARY',
      featureEligible: true,
    });
  }
  ...
  return makeReceipt({
    ...base,
    domainClassMatch: 0,
    matchKind: 'PROVEN_MISMATCH',
    featureEligible: true,
  });
}

/**
 * Apply only the explicitly proven query/domain feature to the rerank row.
 * `domainScore` is intentionally untouched; non-zero domain blend promotion
 * remains a separate evaluation gate.
 */
export function applyDomainClassMatchToRerankCandidateV1(
  ...
  return {
    ...
    domainClassMatch: comparison.featureEligible
      ? comparison.domainClassMatch ?? undefined
      : undefined,
  };
}
