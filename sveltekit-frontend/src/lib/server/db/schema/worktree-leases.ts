...
export const worktreeLeases = pgTable(
  ...
  (table) => ({
    ...
    pathIdx: uniqueIndex('worktree_leases_path_unique_idx')
      .on(table.repositoryRoot, table.worktreePath)
      .where(sql`${table.status} IN ('ALLOCATED', 'ACTIVE', 'VALIDATING', 'READY_FOR_REVIEW')`),
