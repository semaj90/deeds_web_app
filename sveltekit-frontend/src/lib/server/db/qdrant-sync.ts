...
export async function processSyncQueue(): Promise<number> {
  try {
    ...
    const rows = pgRows<SyncQueueRow>(result);
    if (rows.length === 0) {
      return 0;
    }
      ...
      try {
        if (row.operation === 'delete') {
          ...
          const doc: KnowledgeDocument = {
            id: typeof row.document_id === 'number' ? row.document_id : undefined,
            title: row.title,
            content: row.content,
            embedding:
            ...
            couchdb_id: row.couchdb_id ?? undefined,
            metadata: row.metadata as KnowledgeDocument['metadata'],
            blob_url: row.blob_url ?? undefined,
          };
...
export async function fullResync(): Promise<number> {
  try {
    ...
    const allRows = pgRows<DocRow>(allDocs);
