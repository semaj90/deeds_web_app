# LLMS.md — `sveltekit-frontend/src/lib/server/tensor`
...
- [ ] **test-coverage** `src/lib/server/tensor/tensor-analysis-cache.ts` fanIn=11 — add G26-pattern test (11 consumers depend on this)
- [ ] **G17** Audit for remaining hardcoded `localhost` / `127.0.0.1` — replace with `ENV.*` getters
...
> Directory: sveltekit-frontend/src/lib/server/tensor
...
- `read_file({ filePath: "sveltekit-frontend/src/lib/server/tensor/<file>" })` — fetch any file's contents (sandboxed to src/)
