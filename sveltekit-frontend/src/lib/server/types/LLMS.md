...
- [ ] **test-coverage** `src/lib/server/types/retrieval.ts` fanIn=15 — add G26-pattern test (15 consumers depend on this)
- [ ] **test-coverage** `src/lib/server/types/glyph.ts` fanIn=15 — add G26-pattern test (15 consumers depend on this)
...
- `read_file({ filePath: "sveltekit-frontend/src/lib/server/types/<file>" })` — fetch any file's contents (sandboxed to src/)
