# LLMS.md — `sveltekit-frontend/src/lib/server/config`
...
- [ ] **test-coverage** `src/lib/server/config/vector-config.ts` fanIn=12 — add G26-pattern test (12 consumers depend on this)
...
> Directory: sveltekit-frontend/src/lib/server/config
...
- `read_file({ filePath: "sveltekit-frontend/src/lib/server/config/<file>" })` — fetch any file's contents (sandboxed to src/)
