...
import {
  expandNotecardNeighbors,
  getNotecardById,
  getNotecardBySourcePath,
  searchNotecards,
} from '../kb/search-logic.js';
import { runRgSearchAtlas } from '../rg-atlas/run.js';
import type { RgSearchAtlasOptions } from '../rg-atlas/types.js';
...
import {
  ...
  toolMirrorSyncNeo4j,
} from '../dispatch/mcp-tool-implementations.js';
import { getWikiStatus, searchWiki, explainWikiPage, refreshDirectory } from '../kb/wiki-logic.js';
import { switchVlmMode, type VlmMode } from '../inference/vlm-lifecycle.js';
import { resolveAgentsMdQuickHit } from '../graph/community-graph.js';
import { registerDispatcherToolsAsACP } from '../acp/acp-mcp-integration.js';
import { bootstrapACPRegistry } from '../acp/acp-grpc-quic-bridge.js';
import { withToolCallRecord } from '../telemetry/tool-call-recorder.js';
import {
  LDR_RESEARCH_TOOL,
  executeLDRResearch,
  formatLDRResultForAgent,
  type LDRToolInput,
} from './tools/ldr-research.js';
import {
  PHASE18_RERANKER_TOOL_SCHEMA,
  handlePhase18RerankerToolCall,
} from './tools/phase18-reranker-tool.js';
import {
  ATLAS_IDENTITY_AUDIT_SCHEMA,
  ATLAS_CROSS_STORE_PROOF_SCHEMA,
  handleAtlasIdentityAudit,
  handleAtlasCrossStoreProof,
} from './atlas_identity_audit_tools.js';
import {
  ATLAS_SEMANTIC_TOOL_DEFINITIONS,
  handleAtlasSemanticToolCall,
} from '../atlas/atlas-semantic-tools.js';
import { phase109aTools, getPhase109aToolDefinitions } from './phase109a-mcp-tools.js';

const SCHEMA_INDEXER_CONTRACT_CARDS_PATH = join(
  process.cwd(),
  'memory',
  'knowledge',
  'schema-indexer-contract-cards.jsonl'
);
...
function resolveS3Endpoint(): { endPoint: string; port: number; useSSL: boolean } {
  const rawEndpoint =
    process.env.SEAWEED_S3_ENDPOINT ??
    process.env.SEAWEED_ENDPOINT ??
    process.env.MINIO_ENDPOINT ??
    'http://127.0.0.1:8333';

  const normalized = /^[a-z][a-z0-9+.-]*:\/\//i.test(rawEndpoint)
    ? rawEndpoint
    : `http://${rawEndpoint}`;
  const parsed = new URL(normalized);

  const explicitPort =
    process.env.SEAWEED_S3_PORT ?? process.env.SEAWEED_PORT ?? process.env.MINIO_PORT;
  const port = Number(explicitPort ?? parsed.port ?? (parsed.protocol === 'https:' ? 443 : 8333));

  if (!Number.isInteger(port) || port < 1 || port > 65535) {
    throw new Error(`Invalid SeaweedFS/MinIO S3 port: ${String(explicitPort ?? parsed.port)}`);
  }

  return {
    endPoint: parsed.hostname,
    port,
    useSSL: parseBooleanEnv(
      process.env.SEAWEED_USE_SSL ?? process.env.MINIO_USE_SSL,
...
async function getMcpMinioClient() {
  if (!_mcpMinioClient) {
    ...
    _mcpMinioClient = new Client({
      ...endpoint,
      accessKey:
        process.env.SEAWEED_ACCESS_KEY ??
        process.env.MINIO_ACCESS_KEY ??
        process.env.AWS_ACCESS_KEY_ID ??
        'minioadmin',
      secretKey:
        process.env.SEAWEED_SECRET_KEY ??
...
async function mcpGetFile(objectKey: string, bucket?: string): Promise<Buffer> {
  ...
  const bucketName =
    bucket || process.env.MINIO_EVIDENCE_BUCKET || process.env.SEAWEED_S3_BUCKET || 'evidence';
  const chunks: Buffer[] = [];
 ...
 */
export async function setupToolHandlers(): Promise<void> {
  // Initialize optional ACP integrations without preventing the core MCP server
  // from starting when an external ACP dependency is unavailable.
  const registrationResults = await Promise.allSettled([
    Promise.resolve().then(() => bootstrapACPRegistry()),
    Promise.resolve().then(() => registerDispatcherToolsAsACP()),
  ]);
  for (const result of registrationResults) {
    if (result.status === 'rejected') {
      console.error('[mcp] ACP registration degraded:', result.reason);
    }
  }
  ...
  server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
      ...
      },
      {
        ...
        description:
          'Look up a prior LLM answer from the 3-tier cache (Redis L1 → Postgres L2 → Qdrant L3 semantic). Returns compressed CodeLlmOutputMeta envelope (summary, citations, confidence, grounding) suitable for ACE prompt preambles. Use BEFORE generating a new RAG answer to reuse compressed reasoning.',
        inputSchema: {
          type: 'object',
          properties: {
            path: {
              type: 'string',
              description: 'Code path or directory to look up first (cheapest L1 lookup).',
            },
            query: {
              type: 'string',
              description: 'Free-text query for L3 Qdrant semantic fallback when path misses.',
            },
            clusterId: {
              type: 'number',
              description: 'Optional glyph cluster filter for L3 search.',
            },
            includeFullOutput: {
              type: 'boolean',
              description: 'When true, include the full llmOutput; default false (only meta).',
            },
            minScore: {
              type: 'number',
              description: 'Minimum cosine similarity for L3 hits (default 0.78).',
            },
          },
        },
      },
      {
        ...
        inputSchema: {
          type: 'object',
          properties: {
            action: { type: 'string', enum: ['goto', 'click', 'fill', 'screenshot'] },
            url: { type: 'string' },
        ...
        inputSchema: {
          ...
          properties: {
            ...
            useAI: {
              type: 'boolean',
              description:
                'Use AI (Ollama gemma4-rotorquant:latest) to generate case-specific content',
              default: false,
            },
          },
      ...
      },
      {
        ...
        inputSchema: {
          ...
          properties: {
            ...
            model: {
              type: 'string',
              description: 'Model name (default: gemma4-rotorquant:latest)',
            },
      ...
      },
      {
        ...
        inputSchema: {
          ...
          properties: {
            source_ref: {
              type: 'string',
              description: 'File path in any form — canonicalized automatically',
            },
            feature_id: { type: 'string', description: 'Filter by exact feature_id' },
            concept_id: {
              type: 'string',
              description: 'Filter to packets whose concept_ids contains this',
            },
            summary_query: {
              type: 'string',
              description: 'Full-text search against packet summaries',
            },
        ...
        inputSchema: {
          ...
          properties: {
            ...
            includeAce: {
              type: 'boolean',
              description:
                'Whether to include ACE context packet for Gemma4 synthesis (default: true)',
              default: true,
            },
          },
          required: ['table'],
      ...
      },
      {
        ...
        inputSchema: {
          ...
          properties: {
            topK: {
              type: 'number',
              description: 'Max clusters to return (default: all)',
              default: 87,
            },
            query: {
              type: 'string',
              description:
                'Keyword filter — only clusters whose summary contains this string (case-insensitive).',
            },
          },
        },
      },
      ...
      },
      {
        ...
        description:
          'Semantic search across the codebase index returning structured "cards" (identity spine format). Use for finding relevant code with stable IDs for grounding.',
        inputSchema: {
          type: 'object',
          properties: {
            ...
            filters: {
              type: 'object',
              description: 'Optional metadata filters (kind, domain, extension)',
            },
          },
          required: ['query'],
        },
      },
      {
        ...
        description:
          'Semantic search across the standalone schema-indexer contract cards. Use for schema-focused prompt context engineering and MCP routing without touching workspace-gap cards.',
        inputSchema: {
          type: 'object',
          properties: {
            query: {
              type: 'string',
              description: 'Semantic query (e.g. "pgTable schema indexer")',
            },
            limit: { type: 'number', description: 'Max cards to return (1-20)', default: 10 },
          },
          required: ['query'],
      ...
      },
      {
        name: 'kb.get_card',
        description:
          'Retrieve a single codebase card by its stable chunk_id (card:path:hash). Returns full content, metadata, and cluster context.',
        inputSchema: {
          type: 'object',
          properties: {
      ...
      },
      {
        ...
        description:
          'Get immediate graph neighbors for a card from the hypergraph. Returns related cards linked by imports, shared symbols, or topological proximity.',
        inputSchema: {
          type: 'object',
          properties: {
      ...
      },
      {
        ...
        description:
          'Explain why a specific card was retrieved for a query. Returns rank, similarity score, cluster dominance analysis, and topological authority context.',
        inputSchema: {
          type: 'object',
          properties: {
      ...
      },
      {
        ...
        inputSchema: {
          ...
          properties: {
            query: {
              type: 'string',
              description: 'Search query — regex for lexical stage, NL for semantic stages',
            },
            paths: {
              type: 'array',
              items: { type: 'string' },
            ...
            file_types: {
              type: 'array',
              items: { type: 'string' },
              description: 'File extensions for rg. Default: ["ts","svelte"]',
            },
            variant_count: {
              type: 'number',
              description: 'Multi-query variants (1-5). Default: 3',
            },
            top_k_per_lane: { type: 'number', description: 'Qdrant hits per variant. Default: 20' },
            enable_marco: {
              type: 'boolean',
              description: 'Run MS-MARCO cross-encoder rerank. Default: true',
            },
            enable_langextract: {
              type: 'boolean',
              description: 'Run LangExtract GRPO validation. Default: false (slow)',
            },
            persist: {
              type: 'boolean',
              description: 'Write run + hits to Postgres for replay. Default: false',
            },
          },
          required: ['query'],
        },
      ...
      },
      {
        name: 'wiki.status',
        description:
          'Returns high-level status of the codebase knowledge base (Karpathy/AGENTS). includes page count, last updated, and stale directories.',
        inputSchema: { type: 'object', properties: {} },
      },
      {
        name: 'wiki.search',
        description:
          'Searches the codebase wiki (Karpathy/AGENTS) using a hybrid approach: lexical ripgrep, graph metadata, and semantic Qdrant. Returns ranked pages with summaries and confidence.',
        inputSchema: {
          type: 'object',
          properties: {
      ...
      },
      {
        ...
        description:
          'Returns a detailed explanation of a specific wiki page (directory or feature), including related files, imports, clusters, and activity scores.',
        inputSchema: {
          type: 'object',
          properties: {
      ...
      },
      {
        ...
        description:
          'Switch the VLM inference mode between TEXT (TurboQuant) and VISION (Ollama VLM). Use this to prevent VRAM OOM on 8GB cards when switching between text-heavy and image-heavy tasks.',
        inputSchema: {
          type: 'object',
          properties: {
            mode: {
              ...
              description:
                'Target mode (TEXT = TurboQuant, VISION = Ollama VLM, OFF = shut down both)',
            },
          },
          required: ['mode'],
        },
      },
      {
        ...
        inputSchema: {
          ...
          properties: {
            ...
            authUserId: {
              type: 'string',
              description: 'String Lucia user ID for cross-reference (optional)',
            },
            query: {
              type: 'string',
              description: 'The user query that triggered synthesis (required)',
            },
            profile: {
              type: 'string',
              description: 'ACE retrieval profile, e.g. code_debug, legal_qa (required)',
            },
            acePacket: {
              type: 'object',
              description: 'ACE context packet — must include lanes and sourceRefs (required)',
            },
            toolCalls: {
              type: 'array',
              items: { type: 'string' },
              description: 'MCP/ACE tool names invoked during this run',
            },
            sourceRefs: {
              type: 'array',
              items: { type: 'string' },
              description: 'File paths or doc IDs that grounded the answer',
            },
            cacheKeys: {
              type: 'object',
              description: 'Redis/BitFrost cache key state (exactHit, semanticHit, etc.)',
            },
            trustTier: {
              type: 'string',
              description: 'Trust classification, e.g. local_code_plus_official_docs',
            },
            model: { type: 'string', description: 'Model ID used for synthesis (required)' },
            validation: {
              type: 'object',
              description: 'Validation state — testsPassed, commandRun, etc.',
            },
          },
      ...
      },
      {
        name: 'agents_md',
        description:
          'Resolve the nearest AGENTS.md file for a given source path. Checks Redis first (agents:dir:<path>) then walks up the directory hierarchy on disk.',
        inputSchema: {
          type: 'object',
          properties: {
            path: {
              type: 'string',
              description: 'File path or directory relative to sveltekit-frontend root',
            },
          },
          required: ['path'],
        },
        ...
        if (cached) {
          ...
          if (Array.isArray(parsed?.content) && parsed.content[0]?.type === 'text') {
            try {
              ...
              parsed.content[0].text = JSON.stringify({
                ...payload,
                _rpc_cache: { hit: true, key },
              });
            } catch {
              /* non-JSON text content — return as-is */
            }
          }
          return parsed;
        }
  ...
  async function handleToolCall(name: string, args: Record<string, any>): Promise<any> {
    return withToolCallRecord({ toolName: name, toolSource: 'mcp', arguments: args }, () =>
    ...
    switch (name) {
        ...
        if (!hit) {
          return {
            content: [
              {
                type: 'text',
                text: JSON.stringify({ path: filePath, resolvedBy: null, markdown: null }),
              },
            ],
          };
        }
      ...
      case 'startup:briefing': {
        ...
        try {
          ...
          const briefingPath = join(process.cwd(), '.opencode', 'startup-briefing.json');
            ...
            content: [
              {
                type: 'text',
                text: JSON.stringify({
                  error: 'Could not load startup briefing. Run npm run agent:hello first.',
                }),
              },
            ],
          };
      ...
      case 'atlas.packet_search': {
        ...
        const { source_ref, feature_id, concept_id, summary_query, limit } = args as {
          source_ref?: string;
          feature_id?: string;
          concept_id?: string;
        ...
        try {
          const res = await fetch('http://127.0.0.1:8788/mcp', {
            ...
            body: JSON.stringify({
              ...
              params: {
                name: 'atlas.packet_search',
                arguments: { source_ref, feature_id, concept_id, summary_query, limit },
              },
            }),
          ...
          return {
            content: data.result?.content ?? [
              { type: 'text', text: JSON.stringify({ error: 'No content from TRACE' }) },
            ],
          };
        } catch (err) {
          return {
            content: [
              {
                ...
                text: JSON.stringify({
                  error: String(err),
                  hint: 'Is TRACE MCP server running on :8788?',
                }),
              },
            ],
          };
      ...
      case 'schema-dependents:find': {
        ...
        try {
          const { findSchemaDependents } = await import('../tools/schema-dependents.js');
          const { db } = await import('../db/client.js');
          // The current implementation passes a degraded Neo4j dependency until
          // a concrete driver/session adapter is wired. Avoid importing a module
          // and discarding it, which also triggered no-unused-vars.
          const neo4j = null;
      ...
      case 'identity:quarantine': {
        const { packet_keys, reason, timestamp } = args as {
          packet_keys: string[];
          reason: string;
          timestamp?: string;
        };
        try {
          ...
          console.log(
            `[MCP] identity:quarantine: ${packet_keys.length} packets → operator queue. Reason: ${reason}`
          );
          return {
            content: [
              {
          ...
          return {
            content: [
              { type: 'text', text: JSON.stringify({ error: String(err), queued: false }) },
            ],
          };
        }
      }
      ...
      case 'graph:expand': {
        ...
        try {
          ...
          console.log(
            `[MCP] graph:expand: ${feature_ids.length} features, ${hops} hops, ${limit_per_hop} limit/hop, relationship_types=${relationship_types.join(',') || 'all'}`
          );
      ...
      case 'retrieval:rerank': {
        ...
        try {
          ...
          console.log(
            `[MCP] retrieval:rerank: reranking ${candidates.length} candidates (GPU=${use_gpu}) → top ${top_k}`
          );
      ...
      case 'answer:synthesize': {
        ...
        try {
          ...
          console.log(
            `[MCP] answer:synthesize: generating answer (model=${synthesis_model}, tokens=${max_tokens}, temperature=${temperature}, citations=${include_citations}, query=${query.slice(0, 120)})`
          );
      ...
      case 'escalation:route': {
        ...
        try {
          ...
          console.log(
            `[MCP] escalation:route: ${severity} escalation. Decision: ${decision}. Reason: ${reason}. Candidates: ${candidate_count}. Query: ${query ?? 'n/a'}`
          );
          ...
          return {
            content: [
              { type: 'text', text: JSON.stringify({ error: String(err), escalated: false }) },
            ],
          };
        }
      }
      ...
      case 'memory:prior_answer_lookup': {
        const { lookupPriorAnswerMemory, renderPriorAnswerSection } = await import(
          '../cache/code-llm-index.js'
        );
        const hit = await lookupPriorAnswerMemory(
          args as Parameters<typeof lookupPriorAnswerMemory>[0]
        );
        if (!hit) {
      ...
      case 'playwright:browser_action': {
        ...
        // Execute directly through Playwright; no intermediary test URL is needed.
      ...
      case 'kb.search_cards': {
        const {
          ...
          limit = 10,
          filters = {},
        } = args as { query: string; limit?: number; filters?: Record<string, any> };
      ...
      case 'kb.get_card': {
        ...
        if (!card) {
          return {
            content: [
              { type: 'text', text: JSON.stringify({ error: 'Card not found', chunk_id }) },
            ],
            isError: true,
          };
        }
      ...
      case 'kb.explain_retrieval': {
        ...
        const card = chunk_id
          ? ((await getNotecardById(chunk_id)) ?? (await getNotecardBySourcePath(chunk_id)))
          : null;
        const effectiveChunkId = chunk_id ?? matches[0]?.card_id ?? null;
        const match =
          matches.find(
            (item) => item.card_id === effectiveChunkId || item.source_path === card?.source_path
          ) ?? matches[0];
      ...
      case 'analytics:deep_research': {
        const { generateDeepResearch } = await import('../analytics/deep-research.js');
      ...
      case 'analytics:research_topics': {
        const { queryResearchIndex, invalidateResearchIndex, getResearchIndexStats } = await import(
          '../analytics/research-cache.js'
        );
      ...
      case 'analytics:mapreduce_matrix': {
        const { executeMapReduceAnalysis } = await import(
          '../analytics/mapreduce-matrix-analysis.js'
        );
        const userId = String(args.userId ?? 'anonymous');
      ...
      case 'analytics:unified_research': {
        const { executeUnifiedResearch } = await import('../analytics/unified-research-query.js');
        const userId = String(args.userId ?? 'anonymous');
      ...
      case 'analytics:codebase_research': {
        const { executeCodebaseResearch } = await import('../analytics/codebase-research.js');
        const userId = String(args.userId ?? 'anonymous');
        const days = Math.min(30, Math.max(1, Number(args.days ?? 7)));
      ...
      case 'analytics:web_research': {
        ...
        } = await import('../analytics/web-research-crawler.js');
              ...
              lenses.push({
                ...
                label:
                  firstSentence.length > 80
                    ...
                    : firstSentence || `Cluster ${rec.clusterId}`,
                summary,
                size: rec.size ?? 0,
      ...
      case 'topology_search': {
        const { queryTopology: topoSearch } = await import(
          '../retrieval/topology-search-client.js'
        );
      ...
      case 'llm_synthesis.log_event': {
        ...
        if (!runId || !query || !profile || !model) {
          return {
            content: [
              { type: 'text', text: 'Error: runId, query, profile, and model are required' },
            ],
            isError: true,
          };
        }
        ...
        if (!acePacket || typeof acePacket !== 'object') {
          return {
            content: [{ type: 'text', text: 'Error: acePacket must be an object' }],
            isError: true,
          };
        }
  ...
  async function handleFaceIdentify(
    ...
    const { routeToolCall } = await import('../grpc/tool-router-client.js');
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
      ...
      if (name === 'phase18_reranker') {
        return await handlePhase18RerankerToolCall({
          ...request,
          params: {
            ...request.params,
            arguments: args,
