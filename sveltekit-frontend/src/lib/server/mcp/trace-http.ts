...
export async function callTraceMcp(
  ...
  try {
    ...
    let resultCount = Array.isArray(parsed) ? parsed.length : 1;
    if (Array.isArray(parsed) && parsed.length > 3 && toolArgs.limit === undefined) {
      const originalCount = parsed.length;
      parsed = parsed.slice(0, 3);
      resultCount = 3;
      console.warn({
        stage: 'mcp_result_truncate',
