# LLMS.md — `sveltekit-frontend/src/lib/server/agents`
...
- [ ] **test-coverage** `src/lib/server/agents/agents-card-store.ts` fanIn=17 — add G26-pattern test (17 consumers depend on this)
...
> Directory: sveltekit-frontend/src/lib/server/agents
...
- `agents-card-store.ts`
- `agents-context-source.ts`
- `memory-encoding-agent.ts`
- `ontology-sortation-agent.ts`
- `trace-subagent-orchestrator.ts`
- `trace-subagent-registry.ts`
...
- `read_file({ filePath: "sveltekit-frontend/src/lib/server/agents/<file>" })` — fetch any file's contents (sandboxed to src/)
