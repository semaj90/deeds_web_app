...
export function deriveAstNodeId(locator: AstNodeLocatorV1): string {
  ...
  return hash('ast', [value.workspaceRevision, value.sourceRevision, value.sourceRef, value.parserName, value.parserVersion, value.grammarRevision, value.rawAstPath, value.nodeType, value.span.startByte, value.span.endByte]);
}

export function deriveStructuralNodeId(locator: AstNodeLocatorV1): string {
  const value = AstNodeLocatorV1Schema.parse(locator);
  return hash('struct', [value.sourceRef, value.language, value.nodeKind, value.qualifiedSymbol, value.parentNamedAstPath, value.normalizedSignature, value.namedAstPath]);
}

export function deriveSymbolId(locator: AstNodeLocatorV1, lexicalParentSymbolId: string | null): string {
  const value = AstNodeLocatorV1Schema.parse(locator);
  return hash('sym', [value.sourceRef, value.language, value.nodeKind, lexicalParentSymbolId, value.qualifiedSymbol, value.normalizedSignature]);
}

export function deriveSymbolVersionId(input: { symbolId: string; sourceRevision: string; normalizedSignature: string; astNodeId: string }): string {
...
export function buildStructuralIdentity(input: {
  ...
  const symbolVersionId = deriveSymbolVersionId({ symbolId, sourceRevision: locator.sourceRevision, normalizedSignature: locator.normalizedSignature, astNodeId });
