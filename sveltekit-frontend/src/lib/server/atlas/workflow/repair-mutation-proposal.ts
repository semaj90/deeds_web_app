...
export function compileRepairProposalWorkflow(
  ...
  const dag: ContextToolDagV1 = validateContextToolDag(ContextToolDagV1Schema.parse({
    ...
    workspaceRevision: `proposal:${proposal.sourceRevision}`,
    graphRevision: 'proposal-context-manifest-v2',
    canonicalWritesAllowed: false,
    producerRevision,
    nodes: [
      {
...
export function compileAuthorizedRepairWorkflow(input: {
  ...
  const dag: ContextToolDagV1 = validateContextToolDag(ContextToolDagV1Schema.parse({
    ...
    requestId: proposal.requestId,
    workspaceRevision: `authorized:${proposal.sourceRevision}`,
    graphRevision: 'authorized-repair-context-manifest-v2',
