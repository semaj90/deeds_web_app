...
export async function executeLinearFeatureMatrix(
  ...
  if (canUseNative && native?.matrixMultiply) {
    // The generic native primitive computes raw Xw. Presence-aware masked rows
    // stay on the CPU oracle until the native contract supports masks exactly.
    const n = matrix.values.length;
    const f = matrix.featureNames.length;
    const flat = new Float32Array(n * f);
    matrix.values.forEach((row, i) => row.forEach((value, j) => { flat[i * f + j] = value; }));
    ...
    if (requestedBackend === 'AUTO' && !native?.matrixMultiply) fallbackReason = 'native_matrix_multiply_unavailable';
  }
