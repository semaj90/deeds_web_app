...
function float32L2Prefix(input: readonly number[], dimension: number): number[] {
  if (input.length !== CLASSIFICATION_NATIVE_DIMENSION) {
    throw new Error(`QUERY_ROUTER_NATIVE_EMBEDDING_DIMENSION_MISMATCH:${input.length}`);
  }
...
export function normalizeQueryRouterBudgets(input: QueryRouterEvaluationRecordV1['budgetTargets']): [number, number, number] {
  // Preserve a simple, invertible policy for the sigmoid regression head.
  return [
    (input.candidateBudget - 8) / (4096 - 8),
    input.graphHops / 6,
    input.rerankBudget / 256,
  ];
}
...
export function compileQueryRouterDatasetV1(rawRecords: readonly unknown[]): {
  rows: QueryRouterTrainingRowV1[];
  jsonl: string;
  receipt: QueryRouterDatasetReceiptV1;
} {
  ...
  // A training artifact may contain many query revisions but must never mix the
  // representation/label contract that defines the tensor and targets.
