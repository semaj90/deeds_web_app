...
function standardizePresentColumns(values: number[][], presence: number[][]): number[][] {
  ...
  for (let feature = 0; feature < featureCount; feature += 1) {
    ...
    for (let row = 0; row < rowCount; row += 1) {
      if (presence[row]![feature] !== 1 || std <= 1e-12) {
...
/** Build a measurement matrix from CandidateFeatureColumnarV1.
 * Presence bits are explicit columns so missing evidence is not treated as a real zero.
 */
...
/** Bind semantic rows by packet_key; file row position is never CandidateOrdinal. */
...
/** Only an exact CandidateOrdinalSetV1 may define EXACT_TOP_K target truth. */
