...
describe('Atlas recommendation fixed smoke', () => {
  it('keeps lexical evidence and global ANN active before bounded ACP task promotion', () => {
    ...
    validateTreeNodeIdentities([
      { packetKey: 'packet:score-clusters', treeNodeId: 'tree:src/retrieval/clusters.ts:scoreAllClusters', sourceRef: 'src/retrieval/clusters.ts', corpusSnapshotId: 'snapshot:fixture' },
      { packetKey: 'packet:router', treeNodeId: 'tree:src/retrieval/router.ts:retrievalRouter', sourceRef: 'src/retrieval/router.ts', corpusSnapshotId: 'snapshot:fixture' },
      { packetKey: 'packet:workflow-code-node', treeNodeId: 'tree:src/recommendations/workflow.ts:recommendationWorkflow', sourceRef: 'src/recommendations/workflow.ts', corpusSnapshotId: 'snapshot:fixture' },
    ]);
    ...
    const packet = ACPRecommendationPacketSchema.parse({
      ...
      candidates: [
        { tree_node_id: 'tree:src/retrieval/clusters.ts:scoreAllClusters', source_ref: 'src/retrieval/clusters.ts', relevance_probability: 0.95, reason_codes: ['EXACT_IDENTIFIER', 'KMEANS_ROUTE'], evidence_refs: ['rg:scoreAllClusters:8'], estimated_context_tokens: 450, graph_paths: [{ nodes: ['tree:src/retrieval/router.ts:retrievalRouter', 'tree:src/retrieval/clusters.ts:scoreAllClusters'], edges: ['CALLS'], path_score: 0.9 }] },
        { tree_node_id: 'tree:src/recommendations/workflow.ts:recommendationWorkflow', source_ref: 'src/recommendations/workflow.ts', relevance_probability: 0.8, reason_codes: ['GRAPH_WORKFLOW_PATH'], evidence_refs: ['graph:workflow'], estimated_context_tokens: 350, graph_paths: [{ nodes: ['tree:src/retrieval/router.ts:retrievalRouter', 'tree:src/retrieval/clusters.ts:scoreAllClusters', 'tree:src/recommendations/workflow.ts:recommendationWorkflow'], edges: ['CALLS', 'INVOKES_WORKFLOW'], path_score: 0.75 }] },
      ],
    });
