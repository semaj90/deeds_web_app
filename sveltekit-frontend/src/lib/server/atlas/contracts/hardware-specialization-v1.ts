 ...
 * - estimates must identify their estimator and whether that estimator supports the target;
...
export const HardwareArchitectureSchema = z.enum([
  'sm_80','sm_86','sm_87','sm_88','sm_89','sm_90','sm_90a','sm_100','sm_103','sm_110','sm_120','sm_121',
]);
export type HardwareArchitecture = z.infer<typeof HardwareArchitectureSchema>;

export const HardwarePrecisionSchema = z.enum(['fp32','tf32','bf16','fp16','int8','fp8','fp4']);
export const KernelBackendSchema = z.enum([
  'cpu','libtorch','cublas','cublaslt','cutlass','cutile','cuvs','cugraph','tensorrt','tensorrt_llm','tensorrt_rtx','custom_cuda',
]);
export const HardwareMeasurementKindSchema = z.enum([
  'real_target','analytical_estimate','learned_estimate','cross_compile_validation',
]);

export const HardwareEstimatorKindSchema = z.enum([
  'NONE',
  'CUTLASS_HEURISTIC',
  'LEARNED_COST_MODEL',
...
export const HardwareEstimatorEvidenceV1Schema = z.object({
  kind: HardwareEstimatorKindSchema,
  supported_target: z.boolean(),
  estimator_revision: z.string().min(1).nullable(),
  producer: z.string().min(1).nullable().optional(),
}).strict();
export type HardwareEstimatorEvidenceV1 = z.infer<typeof HardwareEstimatorEvidenceV1Schema>;
...
export const HardwareProfileV1Schema = z.object({
  ...
  capabilities: z.object({ fp16:z.boolean(), bf16:z.boolean(), tf32:z.boolean(), int8:z.boolean(), fp8:z.boolean(), fp4:z.boolean() }),
  ...
  if (profile.architecture !== expected) ctx.addIssue({ code:z.ZodIssueCode.custom, path:['architecture'], message:`architecture ${profile.architecture} does not match compute capability ${profile.compute_capability_major}.${profile.compute_capability_minor}` });
});
export type HardwareProfileV1 = z.infer<typeof HardwareProfileV1Schema>;

export const KernelProblemShapeV1Schema = z.object({
  operation: z.enum(['gemm','cosine','attention','topk','pca','projection','knn_exact','ann_search','graph']),
...
export const KernelPerfReceiptV1Schema = z.object({
  ...
  estimator: HardwareEstimatorEvidenceV1Schema,
  ...
  if (receipt.configuration.architecture_target !== receipt.measured_architecture) {
    ctx.addIssue({ code:z.ZodIssueCode.custom, path:['configuration','architecture_target'], message:'kernel architecture target must match the architecture represented by the receipt' });
  }

  if (receipt.measurement_kind === 'real_target') {
    if (receipt.estimator.kind !== 'NONE') ctx.addIssue({ code:z.ZodIssueCode.custom, path:['estimator','kind'], message:'real_target receipts must use estimator kind NONE' });
    if (!receipt.estimator.supported_target) ctx.addIssue({ code:z.ZodIssueCode.custom, path:['estimator','supported_target'], message:'real_target receipts represent supported target execution' });
    if (!receipt.measured_device_name) ctx.addIssue({ code:z.ZodIssueCode.custom, path:['measured_device_name'], message:'real_target measurements require the measured device name' });
    if (receipt.latency_us_p50 == null) ctx.addIssue({ code:z.ZodIssueCode.custom, path:['latency_us_p50'], message:'real_target measurements require measured latency' });
  } else if (receipt.estimator.kind === 'NONE' && receipt.measurement_kind !== 'cross_compile_validation') {
    ctx.addIssue({ code:z.ZodIssueCode.custom, path:['estimator','kind'], message:'estimated receipts require an explicit estimator identity' });
  }

  if (receipt.estimator.kind !== 'NONE' && !receipt.estimator.estimator_revision) {
    ctx.addIssue({ code:z.ZodIssueCode.custom, path:['estimator','estimator_revision'], message:'non-NONE estimators require estimator_revision' });
  }

  // Canonical CUTLASS GEMM Heuristics documentation currently supports Hopper sm9x and Blackwell sm10x only.
  // Do not let the official CUTLASS analytical heuristic masquerade as an Ampere/Ada estimator.
  if (receipt.estimator.kind === 'CUTLASS_HEURISTIC') {
    const cutlassHeuristicSupported = receipt.measured_architecture.startsWith('sm_9') || receipt.measured_architecture.startsWith('sm_10');
    if (!cutlassHeuristicSupported || !receipt.estimator.supported_target) {
      ctx.addIssue({ code:z.ZodIssueCode.custom, path:['estimator','supported_target'], message:`CUTLASS_HEURISTIC does not support target ${receipt.measured_architecture}` });
    }
  }
});
...
export const HardwareSpecializationV1Schema = z.object({
  ...
  if (specialization.status === 'TARGET_VALIDATED' && specialization.validated_real_target_receipt_ids.length === 0) {
    ctx.addIssue({ code:z.ZodIssueCode.custom, path:['validated_real_target_receipt_ids'], message:'TARGET_VALIDATED specialization requires at least one real target receipt' });
  }
});
...
export function promoteHardwareSpecializationV1(input: {
  ...
  for (const selection of input.candidate.selections) {
    ...
    if (matching.length === 0) throw new Error(`selection ${selection.operation_key} lacks a parity-passing real-target receipt`);
