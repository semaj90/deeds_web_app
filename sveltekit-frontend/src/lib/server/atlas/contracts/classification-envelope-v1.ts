...
export const ClassificationEnvelopeV1Schema = z.object({
  ...
  provenance: z.object({
    packetSchemaVersion: z.string().min(1).nullable(),
    featureSchemaVersion: z.string().min(1).nullable(),
    validationSchemaVersion: z.string().min(1).nullable(),
    ledgerSchemaVersion: z.string().min(1).nullable(),
...
export function buildClassificationEnvelopeV1(input: {
  ...
  return ClassificationEnvelopeV1Schema.parse({
    ...
    provenance: {
      packetSchemaVersion: input.provenance?.packetSchemaVersion ?? null,
      featureSchemaVersion: input.provenance?.featureSchemaVersion ?? null,
      validationSchemaVersion: input.provenance?.validationSchemaVersion ?? null,
      ledgerSchemaVersion: input.provenance?.ledgerSchemaVersion ?? null,
