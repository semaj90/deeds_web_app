...
function nomination(treeNodeId: string | null): ApiContractNominationV1 {
  ...
  return {
    ...
    coordinate: {
      sourceRef: 'src/routes/api/search/+server.ts', filePath: 'src/routes/api/search/+server.ts',
      startByte: 10, endByte: 100, startChar: 10, endChar: 100, startLine: 2, endLine: 8,
      treeNodeId, symbolVersionId: 'symbol-version:search-post',
    },
    transport: 'HTTP', method: 'POST', route: '/api/search', handlerSymbol: 'POST',
    inputSchemaRefs: ['zod:SearchRequest', 'zod:SearchRequest'], outputSchemaRefs: ['zod:SearchResponse'],
    authRequirements: ['session'], sideEffects: ['QDRANT_READ'], evidenceSources,
    evidenceRefs: ['src/routes/api/search/+server.ts#bytes=10-100'],
    grammarRevision: 'tree-sitter-typescript:test', semanticEngineRevision: 'ts-morph:test',
...
describe('ApiContractObservationV1', () => {
  it('requires a joined canonical tree node before producing an observation', () => {
    expect(() => compileApiContractObservationV1(nomination(null))).toThrow('API_CONTRACT_CANONICAL_TREE_NODE_REQUIRED');
  });
