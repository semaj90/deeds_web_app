...
export function materializeHardwareProfileV1(
	...
	const bridgeMemoryTrustworthy =
		cudaAvailable &&
		bridgeMemory.available &&
		bridgeMemory.totalBytes > 0;
	...
	// GPU MEM 01 invariant: zero bridge totals mean "not measured by bridge",
	// never "the target has zero VRAM".
	const vramSource = bridgeMemoryTrustworthy
		? 'bridge' as const
		: input.authoritative.source;
	const vramTotalBytes = bridgeMemoryTrustworthy
		? bridgeMemory.totalBytes
		: input.authoritative.totalVramBytes;
