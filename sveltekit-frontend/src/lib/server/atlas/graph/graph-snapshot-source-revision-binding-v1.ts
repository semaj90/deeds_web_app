...
/**
 * Projects authoritative WorkspaceSourceBindingV1 source revisions onto graph
 * nodes by exact normalized source_ref. It never derives sourceRevision from a
 * graph node hash, packet hash, tree ID, Qdrant ID, or ordinal.
 *
 * Nodes without source_ref remain revisionless because they are not source
 * backed. A source-backed node missing from the workspace manifest fails the
 * coverage gate and therefore prevents canonical graph APPLY/FANOUT.
 */
