...
describe('Svelte GraphSnapshotRevisionV1 compatibility adapter', () => {
  it('delegates deterministic logical revision ownership to the package owner', () => {
  ...
  it('changes graphRevision when manifest workspace state or topology changes', () => {
  ...
  it('rejects Git/prose workspace revisions, source-inventory drift, and tampering', () => {
    expect(() => buildGraphSnapshotRevisionV1({ ...fixture, workspaceRevision: 'git:0123456789abcdef' as never })).toThrow();
    expect(() => buildGraphSnapshotRevisionV1({
      ...fixture,
      sourceInventoryRevision: `sha256:${'9'.repeat(64)}`,
    ...
    expect(() => verifyGraphSnapshotRevisionV1({ ...revision, revisionChecksum: 'e'.repeat(64) })).toThrow(/GRAPH_SNAPSHOT_REVISION_CHECKSUM_MISMATCH/);
  });

  it('maps legacy materializer input to exact sourceInventoryRevision without inventing node sourceRevision', () => {
    const write = buildGraphSnapshotRevisionWriteV1({
      snapshotId: fixture.snapshotId,
      workspaceRevision: fixture.workspaceRevision,
      sourceInventorySnapshotId: 'legacy-occurrence-id-that-is-not-authority',
