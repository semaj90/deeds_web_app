	...
	for (const kernel of kernels) {
		...
		if (kernel.participants.length === 2) {
			...
			await session.run(
				...
				ON CREATE SET
					...
					r.relationshipRevision = $relationshipRevision,
				...
				`,
				{
					...
					fromRole: from!.role,
					toRole: to!.role,
					sourceRef: kernel.sourceRef,
					sourceRevision: kernel.sourceRevision,
					relationshipRevision: kernel.relationshipRevision,
		...
		await session.run(
			...
			ON CREATE SET
				...
				relation.sourceRef = $sourceRef,
				relation.sourceRevision = $sourceRevision,
				relation.relationshipRevision = $relationshipRevision,
				relation.evidenceRefs = $evidenceRefs,
