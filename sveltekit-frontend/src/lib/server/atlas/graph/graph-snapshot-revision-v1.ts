...
export const graphSnapshotRevisionV1Schema = z.object({
  ...
  /** Upstream owner: WorkspaceRevisionRecordV1, never Git HEAD or a cache epoch. */
  workspaceRevision: contentRevision,
  /** Deterministic revision of the exact graph materializer input inventory. */
...
/** Logical graph revision. snapshotId and Git provenance are occurrence/provenance, not graph world state. */
...
export function describeGraphSnapshotRevisionOwnership(): string {
  return [
    'WorkspaceRevisionRecordV1 owns workspaceRevision; Git commit/tree IDs and packet cache epochs are provenance/cache coordinates only.',
