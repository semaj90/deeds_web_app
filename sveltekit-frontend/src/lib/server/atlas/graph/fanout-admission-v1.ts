...
export const fanoutAdmissionV1Schema = z.object({
  ...
  graphRevision: z.string().regex(/^[a-f0-9]{64}$/),
  sourceRevision: sourceRevision.nullable(),
  representationId: z.literal('semantic_768').nullable(),
  representationRevision: id.nullable(),
  repositoryRevisionObserved: id.nullable(),
  legacyWorkspaceCacheRevisionObserved: id.nullable(),
  blockers: z.array(id),
...
/**
 * Final admission boundary before Qdrant/cuVS/CAGRA/TurboVec/graph executors
 * may emit snapshot-scoped CandidateOrdinal coordinates.
 *
 * CandidateOrdinal is materialized only after all canonical cross-store
 * coordinates agree. This receipt never mutates a store and never becomes an
 * identity owner itself.
 */
export function evaluateFanoutAdmissionV1(input: {
  ...
  let repositoryRevisionObserved: string | null = null;
  let legacyWorkspaceCacheRevisionObserved: string | null = null;
  ...
  if (node.snapshotId !== snapshot.snapshotId) {
    ...
    repositoryRevisionObserved = alignment.repositoryRevisionObserved;
    legacyWorkspaceCacheRevisionObserved = alignment.legacyWorkspaceCacheRevisionObserved;
    ...
    if (!alignment.canonicalIdentityMatch) {
      ...
      if (!alignment.workspaceWorldRevisionAligned) blockers.push('WORKSPACE_WORLD_REVISION_MISMATCH');
