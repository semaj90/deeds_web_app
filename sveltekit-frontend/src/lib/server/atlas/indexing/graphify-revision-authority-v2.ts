...
export const GRAPHIFY_REVISION_AUTHORITY_V2_SCHEMA = 'atlas.graphify-revision-authority.v2' as const;
export const GRAPHIFY_REVISION_AUTHORITY_V2_REVISION = 'atlas.graphify-revision-authority.workspace-manifest-plus-source-sha256.v2' as const;

const sha256 = z.string().regex(/^[a-f0-9]{64}$/);
const sourceRevision = z.string().regex(/^sha256:[a-f0-9]{64}$/);
const revision = z.string().min(1);

export const graphifyRevisionAuthorityV2Schema = z.object({
  schema: z.literal(GRAPHIFY_REVISION_AUTHORITY_V2_SCHEMA),
  authorityRevision: z.literal(GRAPHIFY_REVISION_AUTHORITY_V2_REVISION),
...
export type GraphifyRevisionAuthorityV2 = z.infer<typeof graphifyRevisionAuthorityV2Schema>;
...
function normalizeSourceRef(workspaceRoot: string, absolutePath: string): string {
  ...
  if (!relative || relative === '..' || relative.startsWith('../') || path.isAbsolute(relative)) {
    throw new Error(`GRAPHIFY_REVISION_SOURCE_OUTSIDE_WORKSPACE:${absolutePath}`);
  }
  return relative;
}

/** Durable Graphify authority derived only from canonical workspace/source bindings.
 * Git commit/tree identifiers remain provenance and never substitute for logical revisions. */
export function deriveGraphifyRevisionAuthorityV2(input: {
  ...
  if (sourceBinding.sourceRef !== sourceRef) throw new Error(`GRAPHIFY_REVISION_SOURCE_BINDING_REF_MISMATCH:${sourceRef}:${sourceBinding.sourceRef}`);
  if (sourceBinding.workspaceRevision !== workspaceRecord.workspaceRevision) throw new Error('GRAPHIFY_REVISION_WORKSPACE_BINDING_REVISION_MISMATCH');
  if (sourceBinding.baseCommitOid !== workspaceRecord.baseCommitOid) throw new Error('GRAPHIFY_REVISION_WORKSPACE_BINDING_GIT_COMMIT_MISMATCH');
  if (sourceBinding.sourceRevision !== `sha256:${sourceBinding.contentDigest}`) throw new Error('GRAPHIFY_REVISION_SOURCE_BINDING_DIGEST_MISMATCH');

  const payload = {
    schema: GRAPHIFY_REVISION_AUTHORITY_V2_SCHEMA,
    authorityRevision: GRAPHIFY_REVISION_AUTHORITY_V2_REVISION,
    workspaceRoot,
  ...
  return graphifyRevisionAuthorityV2Schema.parse({ ...payload, authorityChecksum: digest(payload) });
}
