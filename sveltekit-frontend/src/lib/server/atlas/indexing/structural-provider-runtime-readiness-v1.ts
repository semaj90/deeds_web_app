...
/**
 * Proof-only readiness classification. A provider can return a syntactically
 * valid HTTP payload while its parser engine is unavailable; that state must
 * block parity evaluation rather than count as a semantic mismatch.
 */
export function classifyStructuralProviderRuntimeReadiness(
  ...
  if (SIDECAR_UNAVAILABLE_PATTERN.test(diagnosticText)) {
    return {
      schema: 'atlas.structural-provider-runtime-readiness.v1',
      available: false,
      reason: 'SIDECAR_TRANSPORT_UNAVAILABLE',
    };
  }
  ...
  if (PARSER_PACKAGE_UNAVAILABLE_PATTERN.test(diagnosticText)) {
    return {
      schema: 'atlas.structural-provider-runtime-readiness.v1',
      available: false,
      reason: 'PARSER_PACKAGE_UNAVAILABLE',
    };
  }
