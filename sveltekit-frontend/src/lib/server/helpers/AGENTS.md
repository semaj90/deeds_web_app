# AGENTS.md — `src/lib/server/helpers`
...
> Directory audit: src/lib/server/helpers
...
- `src/lib/server/helpers/docker-discovery.ts`
...
- `graph_search({ query: "helpers", topK: 8 })` — files in this dir with tags, TODOs, audit flags
- `wiki_note_lookup({ query: "server helpers", limit: 5 })` — KAG narrative + audit score
- `audit_hotspots({ limit: 10 })` — if this dir is failing gates, surfaces the broader hotspot set
- `read_file({ filePath: "src/lib/server/helpers/<file>" })` — fetch any file's contents (sandboxed to src/)
