...
export class WebGPUTextureStreamer {
    ...
    public isInitialized = false; // Using simplified state for now
    ...
    private setupCompressionWorker() {
        ...
        // Clean up the worker URL to prevent memory leaks is handled in unload/destroy ideally
    ...
    async loadTexture(
        ...
        const { priority = 1, region = 'CHR_ROM', compress = true } = options;
        ...
        try {
            ...
            if (compress && this.compressionWorker) {
                finalData = await this.compressTexture(finalData, width, height, options.legalContext);
                compressed = true;
            }

            // Debug buffer info for legal context textures
            if (options.legalContext?.riskIndicator) {
    ...
    private createWebGLTexture(nesTexture: NESTexture): void {
        ...
        // ... (implementation suppressed for brevity, similar standard GL)
    ...
    private async garbageCollect(region: string): Promise<void> {
        ...
        const mem = this.memoryRegions.get(region);
        if (!mem) return;
        // Evict LRU ...
    }

    private async unloadTexture(id: string): Promise<void> {
         // ...
    }
