...
- [ ] **test-coverage** `src/lib/types/enhanced-svelte5-types.ts` fanIn=27 — add G26-pattern test (27 consumers depend on this)
- [ ] **hotspot** `src/lib/types/enhanced-svelte5-types.ts` (fanIn=27) — consider splitting or adding circuit breaker; top callers: `indexdb-cache.svelte.ts`, `loki-cache.svelte.ts`, `CacheMonitor.svelte`
