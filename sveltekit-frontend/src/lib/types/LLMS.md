...
- [ ] **test-coverage** `src/lib/types/enhanced-svelte5-types.ts` fanIn=26 — add G26-pattern test (26 consumers depend on this)
- [ ] **hotspot** `src/lib/types/enhanced-svelte5-types.ts` (fanIn=26) — consider splitting or adding circuit breaker; top callers: `indexdb-cache.svelte.ts`, `loki-cache.svelte.ts`, `CacheMonitor.svelte`
...
- `read_file({ filePath: "sveltekit-frontend/src/lib/types/<file>" })` — fetch any file's contents (sandboxed to src/)
