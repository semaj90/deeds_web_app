...
- `wiki_note_lookup({ query: "yorha cases", limit: 5 })` — KAG narrative + audit score
- `audit_hotspots({ limit: 10 })` — if this dir is failing gates, surfaces the broader hotspot set
- `read_file({ filePath: "src/lib/components/yorha/cases/<file>" })` — fetch any file's contents (sandboxed to src/)
