  ...
  const [state, send] = useMachine(chatMachine, {
    ...
    actors: {
      fetchResponse: async ({ input }) => {
        // Capture UI state right before sending
        ...
        const res = await fetch('/api/admin/ai-chat', {
          ...
          body: JSON.stringify({ 
            ...
            uiSnapshot,
            attachments: input.attachments // Pass attachments to API
          })
        });
...
<Dialog.Root bind:open>
  <Dialog.Content 
    ...
    transition={fade}
  >
    <Dialog.Overlay class="fixed inset-0 bg-black/60 backdrop-blur-md" />
    ...
    >
      ...
      >
        ...
        {#each $state.context.messages as msg}
        ...
        {#if $state.matches('retrieving_context')}
      ...
      <footer class="p-8 border-t border-white/5 bg-black/40 space-y-4">
        ...
        <form onsubmit={(e) => { e.preventDefault(); handleSubmit(); }} class="relative group">
          ...
          <button 
            ...
            disabled={isUploading || $state.matches('thinking')}
          >
            {#if isUploading}
          ...
          <button 
            ...
            disabled={!inputVal.trim() && attachments.length === 0 || $state.matches('thinking')}
          >
