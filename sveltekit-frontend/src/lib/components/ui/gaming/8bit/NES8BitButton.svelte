...
<script lang="ts">
	...
	const variantColors: Record<string, { bg: string; shadow: string; text: string }> = {
...
<button
	...
	class="relative font-bold uppercase tracking-wider {sizeStyles[size]} transition-transform duration-75 disabled:opacity-50 disabled:cursor-not-allowed {className}"
	style="
