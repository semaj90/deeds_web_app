...
// Metadata kind type for file type classification (different from DB evidenceType enum)
type MetadataKind = 'PDF' | 'IMAGE' | 'VIDEO' | 'AUDIO' | 'TEXT' | 'LINK' | 'UNKNOWN';
  ...
  upload: async ({ request, locals, url }) => {
	...
	errors: { case_id: ['Selected case not found'] } } });
      ...
      let tempMetadata: IntermediateEvidenceMetadata = {
        kind: evidenceType as MetadataKind,
