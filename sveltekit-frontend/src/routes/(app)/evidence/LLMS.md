...
- `read_file({ filePath: "sveltekit-frontend/src/routes/(app)/evidence/<file>" })` — fetch any file's contents (sandboxed to src/)

For route handlers in this dir, also try:
- `verify_fix({ filePath: "sveltekit-frontend/src/routes/(app)/evidence/+server.ts" })` — runs svelte-check / tsc on a single file
