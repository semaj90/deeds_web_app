...
export const load: PageServerLoad = async ({ locals, params, parent }) => {
  ...
  // Client view cache check (user-scoped, case-versioned)
  const vKey = clientKey.view(userId, id, 'overview');
  const memHit = getFromMemoryCache(vKey);
  if (memHit.found && memHit.value) {
    return memHit.value as any;
  }
  try {
  ...
  const result = {
    ...
    caseData: {
      ...
      where: caseRow.jurisdiction
        ? `${caseRow.jurisdiction}${caseRow.court ? ` — ${caseRow.court}` : ''}`
        : null,
