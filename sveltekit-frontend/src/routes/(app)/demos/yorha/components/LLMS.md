# LLMS.md — `sveltekit-frontend/src/routes/(app)/demos/yorha/components`
...
> Directory: sveltekit-frontend/src/routes/(app)/demos/yorha/components
...
| G20 | ✅ PASS | clean ✅ |
| G17 | ❌ FAIL | 1/21 files — use env.server.ts getters |

_Gates checked: G21-24, G20, G17. Run `npm run index:codebase:fast && npm run llms:write` to refresh._
## Todos + Enhancements

- **[HIGH]** [G17] Fix **G17** No hardcoded localhost URLs: 1/21 files — use env.server.ts getters
...
- `read_file({ filePath: "sveltekit-frontend/src/routes/(app)/demos/yorha/components/<file>" })` — fetch any file's contents (sandboxed to src/)
