...
> Directory audit: src/routes/(app)/yorha

## Snapshot

- route handler directory with 3 files, 0 API handlers
- Audit score: **90/100**
- no audit signals
...
- `src/routes/(app)/yorha/+page.server.ts`
- `src/routes/(app)/yorha/+page.server.ts`
- `src/routes/(app)/yorha/+page.server.ts`
...
- `wiki_note_lookup({ query: "(app) yorha", limit: 5 })` — KAG narrative + audit score
- `audit_hotspots({ limit: 10 })` — if this dir is failing gates, surfaces the broader hotspot set
- `read_file({ filePath: "src/routes/(app)/yorha/<file>" })` — fetch any file's contents (sandboxed to src/)
