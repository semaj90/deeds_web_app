...
import {
  getAllClusterNarratives,
  detectCodebaseClusters,
} from '$lib/server/graph/codebase-cluster-detection.js';
 ...
 */
const getNarrativesSchema = z.object({
  clusterId: z.coerce.number().int().min(0).optional(),
...
export const GET: RequestHandler = async ({ url, locals }) => {
  ...
  try {
    ...
    if (!parsed.success)
      return json({ narratives: [], total: 0, error: 'Invalid query params' }, { status: 400 });
    const { clusterId, search, limit } = parsed.data;
    ...
    if (clusterId != null) {
      const rows = await db
        .select()
        .from(clusterNarratives)
        .where(eq(clusterNarratives.clusterId, clusterId));
    ...
    const rows = await db
      .select()
      .from(clusterNarratives)
      .orderBy(desc(clusterNarratives.updatedAt))
      .limit(limit);
...
export const POST: RequestHandler = async ({ request, locals }) => {
  ...
  try {
    ...
    const k = parsed.success ? parsed.data.k : undefined;
    ...
    return json(
      { synthesized: 0, error: e instanceof Error ? e.message : 'Unknown error' },
      { status: 500 }
    );
  }
};
