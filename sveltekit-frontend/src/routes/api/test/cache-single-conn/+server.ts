...
const singleConnSchema = z.object({
...
export const POST: RequestHandler = async ({ request, locals }) => {
  ...
  try {
    ...
    const parsed = singleConnSchema.safeParse(raw);
    if (!parsed.success)
    ...
    const cached = retrieved ? (JSON.parse(retrieved) as typeof testResponse) : null;
    const contentMatches = cached?.content === testResponse.content;
