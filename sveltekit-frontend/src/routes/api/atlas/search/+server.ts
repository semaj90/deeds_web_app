...
export const POST: RequestHandler = async ({ request }) => {
  ...
  const packets: AtlasPacket[] = hits.map((hit, cosineRank) => {
    ...
    return {
      ...
      ontology:             (Array.isArray(p.ontology) ? p.ontology.map((value) => String(value)) : [domainLabel]),
      topology_label:       (p.topology_label as string | null) ?? (p.topologyLabel as string | null) ?? (p.domain_class as string | null) ?? domainLabel,
      ontology_label:       (p.ontology_label as string | null) ?? (p.ontologyLabel as string | null) ?? (p.domain as string | null) ?? (p.domain_class as string | null) ?? domainLabel,
