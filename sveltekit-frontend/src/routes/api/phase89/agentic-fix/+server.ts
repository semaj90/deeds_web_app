    ...
    async start(controller) {
      ...
      try {
        ...
        try {
          ...
          sendEvent(`Summary generation failed: ${e}`, 'warning');
        }

        // Step 2: Auto-tag with ripgrep
        sendEvent('Step 2/5: Extracting tags with ripgrep...', 'progress');
        ...
        try {
          ...
          sendEvent(`Tagging failed: ${e}`, 'warning');
        }
        ...
        const fixPrompt = [
          `Fix errors in cluster #${cluster_id}:`,
          `Pattern: ${pattern}`,
        ...
        try {
          ...
          sendEvent(`ACE failed: ${e}`, 'warning');
        }
        ...
        try {
          ...
          const tags = context.tags?.join(', ') ?? 'none';
          const similarList = context.similar_clusters.map((p: string) => '- ' + p).join('\n');
          ...
          sendEvent(`copilot.md update failed: ${e}`, 'warning');
        }
