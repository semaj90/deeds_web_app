...
- `read_file({ filePath: "sveltekit-frontend/src/routes/api/rag/search/<file>" })` — fetch any file's contents (sandboxed to src/)

For route handlers in this dir, also try:
- `verify_fix({ filePath: "sveltekit-frontend/src/routes/api/rag/search/+server.ts" })` — runs svelte-check / tsc on a single file
