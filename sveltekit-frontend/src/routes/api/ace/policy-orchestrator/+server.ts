...
export const POST: RequestHandler = async ({ request, locals }) => {
  try {
    ...
    // Stage 5 (external): Gemma4 synthesis would happen next
    // For now, we return the ACE context and policy scores
    ...
    await orchestrator.logRLMTrace(
      ...
      decomposition,
      policyScores,
      aceAssembly.selectedPackets,
      '', // gemmaResponse (not yet generated)
      // userFeedback (optional, collected later)
    );

    // Return ACE context + trace info
    return json(
      {
        success: true,
        aceContext: {
        ...
        trace: {
          ...
          decomposition,
          policyScores,
          acePacketCount: aceAssembly.selectedPackets.length
        }
      },
      { status: 200 }
    );
