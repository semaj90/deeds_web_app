...
async function buildAcePacket(query: string) {
  ...
  if (fs.existsSync(packetPath)) {
    ...
    if (!packet.varianceRecovery) {
      try {
        ...
        console.warn(`[ace:stream] Variance recovery normalization failed: ${(err as Error).message}`);
      }
    }
    return packet;
  }
  return {
    ...
    cacheSources: [],
    rankedCards: [],
    failureHints: ['Packet builder fallback'],
    ...
    async start(controller) {
      ...
      if (tokenMapPayload) {
        void persistTokenMapCartridge(queryHash, tokenMapPayload).catch((err) => {
          console.warn(`[ace:stream] token-map persistence failed: ${(err as Error).message}`);
        });
      }
