...
export const GET: RequestHandler = async ({ locals }) => {
  if (!locals.user) {
    ...
    const [
      ...
      (async () => {
        try {
          const res = await fetch(`${ENV.RABBITMQ_MGMT_URL}/api/overview`, {
    ...
    console.error('[InfrastructureStatus] Error:', err);
    return json(
      {
        tiers: {
          tier1_grpc: {
        ...
        inference: {
          tensorrt: { available: false, url: '' },
          triton: { available: false, url: '', model: '' },
          tritonVlm: { available: false, url: '', model: '', visionModel: '' },
        ...
        services: {
          ...
          qdrant: false,
          rabbitmq: false,
          langextract: { healthy: false, services: {}, version: '', latencyMs: -1 },
          whisperServer: { ok: false, enabled: false },
        },
        cache: { redis: null, qdrant: null },
        queues: null,
        dispatch: { total: 0, succeeded: 0, failed: 0, pending: 0 },
        inferenceLog: { total: 0, byBackend: {} },
        runtimeConfig: {
          maxOldSpaceSize: 0,
          gpuBatchSize: 0,
