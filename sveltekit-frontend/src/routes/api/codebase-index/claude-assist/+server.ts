...
export const POST: RequestHandler = async ({ request, locals }) => {
  ...
  const emit = (
    type: Parameters<typeof publishWorkflowEvent>[1]['type'],
    label: string,
  ...
  try {
    ...
    /* non-fatal — fall back to code defaults */
  }
  ...
  if (filters?.framework || filters?.modelType || filters?.precision) {
    ...
    try {
      ...
      /* Redis unavailable — proceed without schema filter */
    }
    timing.mlSchemaMs = Math.round(performance.now() - t1);
    emit('SCHEMA_LOOKUP_COMPLETE', 'ML schema lookup', {
      schemaIds: schemaIds.length,
      durationMs: timing.mlSchemaMs,
    });
  }
  ...
  try {
    ...
    if (!researchGraph) {
      ...
      const { formatGraphForClaudeCode } = await import('$lib/server/ai/langgraph-research.js');
      researchMarkdown = formatGraphForClaudeCode(researchGraph);
    }
  } catch (err) {
  ...
  emit('PIPELINE_STAGE_DONE', 'Research graph', {
  ...
  try {
    ...
    if (schemaIds.length > 0 && schemaIds.length <= 500) {
      ...
      if (ptRes?.ok) {
        ...
        candidatePaths = (ptData.result ?? [])
          .map(
            (p: { payload?: { path?: string; relativePath?: string } }) =>
    ...
    /* embedding/search failure — degrade gracefully */
  }
  timing.retrievalMs = Math.round(performance.now() - t3);
  emit('RERANK_COMPLETE', 'Tri-vector rerank', {
  ...
  emit('KAG_COMPLETE', 'KAG graph neighbors', {
    neighborCount: kagNeighbors.length,
    durationMs: timing.kagMs,
  });
  ...
  const cacheFingerprint = _ck(query + JSON.stringify(filters ?? {}) + maxContextChunks + compact);
  ...
  emitTimeline(locals.user.id, 'claude_assist.completed', {
    ...
    researchDomains:
      researchGraph?.domains ?? researchGraph?.workerFindings?.map((w: any) => w.domain) ?? [],
  ...
  const researchPayload: Record<string, unknown> = {
    ...
    totalChunks:
      researchGraph?.workerFindings?.reduce(
        (s: number, w: any) => s + (w.chunks?.length ?? 0),
        0
      ) ??
      researchGraph?.workers?.reduce((s: number, w: any) => s + (w.chunks?.length ?? 0), 0) ??
      0,
  };
  ...
  if (sessionId)
    publishWorkflowComplete(sessionId, {
      aceChunks: topChunks.length,
      totalMs: Math.round(totalMs),
    });

  // Emit non-blocking telemetry when returning assembled ACE context to the client
  try {
    const { getRedis } = await import('$lib/server/redis.js');
    const redis = getRedis();
    if (aceContext) {
      void recordContextCacheAccess(redis, {
        cacheKey: cacheSlot ?? `ca:ctx:${String(query).slice(0, 32)}`,
        cacheSource: 'no-cache',
        contextCacheHit: true,
        packId: undefined,
        query: query,
        intent: 'claude-assist',
        mode: 'codebase-assist',
