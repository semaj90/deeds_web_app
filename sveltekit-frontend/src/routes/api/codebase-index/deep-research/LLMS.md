# LLMS.md — `sveltekit-frontend/src/routes/api/codebase-index/deep-research`
...
> Directory: sveltekit-frontend/src/routes/api/codebase-index/deep-research
...
_Gates checked: G18, G19, G26. Run `npm run index:codebase:fast && npm run llms:write` to refresh._
...
- `read_file({ filePath: "sveltekit-frontend/src/routes/api/codebase-index/deep-research/<file>" })` — fetch any file's contents (sandboxed to src/)

For route handlers in this dir, also try:
- `verify_fix({ filePath: "sveltekit-frontend/src/routes/api/codebase-index/deep-research/+server.ts" })` — runs svelte-check / tsc on a single file
