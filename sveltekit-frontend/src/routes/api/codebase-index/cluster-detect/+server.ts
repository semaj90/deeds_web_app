...
export const POST: RequestHandler = async ({ request, locals }) => {
	...
	detectCodebaseClusters(opts).then((result) => {
		...
		job.result = result;
		if (job.status === 'error') {
			console.error('[codebase-index/cluster-detect] Pipeline errors:', result.errors);
			job.error = 'Cluster detection failed';
		}
	}).catch((err) => {
		console.error('[codebase-index/cluster-detect] Pipeline error:', err);
		job.status = 'error';
		job.finishedAt = new Date().toISOString();
		job.error = 'Cluster detection failed';
	});
