...
export const POST: RequestHandler = async ({ request, locals }) => {
  ...
  try {
    ...
    const qdrantState =
      qdrantResult.status === 'fulfilled' ? `${qdrantHits.length} qdrant` : 'qdrant offline';
    const rgState = rgResult.status === 'fulfilled' ? `${lexicalHitCount} rg` : 'rg offline';
    trace.push({
      stage: 'A2:qdrant+rg',
      durationMs: Math.round(performance.now() - t),
      detail: `${qdrantState} + ${rgState}`,
    });
  } catch {
  ...
  try {
    ...
    if (raw) {
      for (const [k, v] of Object.entries(raw)) {
        try {
          prMap[k] = JSON.parse(v as string).blend ?? 0;
        } catch {
          /* skip */
        }
      }
    ...
    trace.push({
      stage: 'A3:karpathy',
      durationMs: Math.round(performance.now() - t),
      detail: `${Object.keys(prMap).length} entries`,
    });
  } catch {
  ...
  const isArch = /architect|design|pattern|service|pipeline|system|module/.test(
    query.toLowerCase()
  );
  const prMax = Math.max(1, ...Object.values(prMap));

  const blended: AtlasChunk[] = codebaseHits
    .map((r) => {
      ...
      const score =
        ...
        effectiveWeights.pagerank * pagerank +
        effectiveWeights.topology * topo +
        bonus;
      return {
        ...c,
  ...
  trace.push({
    stage: 'A3:blend',
    durationMs: 0,
    detail: `top=${topScore.toFixed(3)} → ${mode} | ${effectiveWeights.reasons.slice(0, 2).join('; ')}`,
  });
  ...
  if (mode !== 'fast' && embedding.length) {
    ...
    try {
      ...
      trace.push({
        stage: 'A4:hyperrag',
        durationMs: Math.round(performance.now() - t),
        detail: `${wikiHits.length} wiki hits`,
      });
    } catch {
      trace.push({
        stage: 'A4:hyperrag',
  ...
  const allChunks = [...blended, ...wikiChunks]
    .filter((c) => {
      if (seen.has(c.source_ref)) return false;
  ...
  const nodes: AtlasNode[] = [
    { id: 'query', type: 'query', label: query.slice(0, 45), position: { x: CX, y: CY }, data: {} },
    {
      ...
      type: 'cache',
      label: 'Redis CHR97',
      position: { x: CX + 130, y: CY - 95 },
      data: { redis_hot: 0 },
    },
  ];
  const edges: AtlasEdge[] = [{ id: 'q-cache', source: 'query', target: 'cache', label: 'check' }];
  ...
  sourceRefs.forEach((sr, i) => {
    ...
    if (linked)
      edges.push({
        id: `${linked.chunk_id}-${sid}`,
        source: linked.chunk_id,
        target: sid,
        label: 'ref',
      });
  });

  // compute simple graph path strings from built edges
