...
import { traceCartridge } from '$lib/server/observability/langfuse.js';
...
export const POST: RequestHandler = async ({ request, locals }) => {
  ...
  const fetchRunes = async (): Promise<RuneData[]> => {
    ...
    return allPoints.map((point, i) => ({
      id: i,
      clusterId:
        typeof point.payload.chunk_index === 'number'
          ? Math.floor((point.payload.chunk_index as number) / 5)
          : 0,
  ...
  try {
    const response: TensorSearchResponse = await traceCartridge(
      'search',
      { caseId, query: query.slice(0, 100), topK: topK ?? 10, collection: collectionName },
      () =>
        searchCartridgeTensors(query, caseId, fetchRunes, { topK, minScore, hasActiveCase: true })
    );
