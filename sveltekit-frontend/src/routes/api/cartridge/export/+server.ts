...
export const POST: RequestHandler = async ({ request, locals }) => {
  ...
  try {
    ...
    if (cached) {
      ...
      // Fire-and-forget trace (cache hit)
      traceCartridge('cache-hit', { caseId, sizeBytes: bytes.length }, () =>
  ...
  // Trace the build (fire-and-forget)
  traceCartridge(
    'export',
    {
      caseId,
      runeCount: runes.length,
      sizeBytes: cartridgeBytes.length,
      collection: collectionName,
    },
    () => Promise.resolve(null)
  ).catch(() => {});
