<script lang="ts">
	...
	function handleExport() {
		...
		const exportData = {
			...
			vlmAnalysis: analysis.vlmAnalysis,
			frameAnalysis: analysis.frameAnalysis,
			sceneDetection: analysis.sceneDetection,
	...
	function handleKeydown(e: KeyboardEvent) {
		...
		if (!e.ctrlKey && !e.altKey && !e.shiftKey && !isInput) {
			...
			if (e.key === '2') activeTab = 'frames';
			if (e.key === '3') activeTab = 'scenes';
			if (e.key === '4') activeTab = 'transcription';
			if (e.key === '5') activeTab = 'analysis';
		}
		...
		if (!isInput && activeTab === 'frames' && analysis) {
			if (e.key === 'ArrowLeft') {
				e.preventDefault();
			...
			if (e.key === 'ArrowRight') {
				e.preventDefault();
				currentFrame = Math.min(analysis.frameAnalysis.length - 1, currentFrame + 1);
			}
		}
	}
</script>
