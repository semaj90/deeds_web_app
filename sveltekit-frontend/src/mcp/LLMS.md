# LLMS.md — `sveltekit-frontend/src/mcp`

# LLMS.md — `sveltekit-frontend/src/mcp`

# LLMS.md — `sveltekit-frontend/src/mcp`

# LLMS.md — `sveltekit-frontend/src/mcp`

# LLMS.md — `sveltekit-frontend/src/mcp`

## Audit Gates — MCP Tool Surface (TRACE)

> Auto-mapped from CLAUDE.md §"Unified Audit Gate System". Last enriched: 2026-05-11
> Run each check from the **sveltekit-frontend/** root.

### Tier A — Code Connectivity

**G1** Static ESM imports
```bash
rg "from.*MODULE" src/ --type ts --type svelte
```

**G2** Dynamic ESM imports
```bash
rg "import(.*MODULE" src/ --type ts --type svelte
```

**G8a** Service layer — use HttpServiceError subclasses, not kit error()
```bash
rg "import .*error.*from .@sveltejs/kit." src/lib/server/ --type ts  # 0 hits
```

### Tier D — Security + Runtime

**G18** Auth guard on API routes
```bash
rg "locals\.user|requireAuth|getSession" src/routes/api/MODULE/ --type ts
```

**G19** Zod validation on API routes
```bash
rg "import.*zod|from.*zod|z\." src/routes/api/MODULE/ --type ts
```


## TODO — Enhancements from ACE Analysis

> Generated from Redis ACE hits (code:graph:node:* + hotspot data). Regenerate: `node scripts/enrich-LLMS.md.mjs`.

- [ ] **G8a** Use `HttpServiceError` subclasses (`UnauthorizedError`, `NotFoundError`, etc.) — never `throw error()` from `@sveltejs/kit` in service layer

## Fix Timeline

> Recent commits touching this directory — newest first. Used by agents to correlate errors with fixes.

| Commit | Timestamp | Subject |
|--------|-----------|---------|
| `834c0f8ff8` | 2026-05-11T10:05 | fix(server): misc server-side fixups — SeaweedFS health, karpathy-hook metadata, retrieval types |
| `649272c5b6` | 2026-05-11T09:51 | feat(mcp): register kb.rg_atlas_search tool (rg_atlas_tools.ts helper) |
| `b7bfacf9c8` | 2026-05-11T09:50 | feat(mcp): T20 — kb.rg_atlas_search tool in MCP server |
| `618dee240c` | 2026-05-11T01:39 | 59_26_agents_master_hermes_quest |
| `2dccb6c40c` | 2026-05-10T19:57 | fix(audit-p1): clear remaining 15 tsgo errors (15 → 0) |
| `4cd6a4fbed` | 2026-05-10T18:43 | refactor(services): split lib/services server-only items to lib/server/services (PR-2) |

<!-- /LLMS-ENRICH -->







































































































































































































































































































































































































































































































<!-- LLMS-ENRICH v1 · auto-generated by enrich-llms-md.mjs · do not edit manually -->

## Enhanced Graph Context


| File | Authority | Flags |
|------|-----------|-------|
| `server.ts` | 3.000 | 0xb |
| `bifrost_tools.ts` | 0.608 | 0x9 |
| `research_tools.ts` | 0.560 | 0x9 |
| `admin_tools.ts` | 0.560 | 0xb |
| `codebase_tools.ts` | 0.540 | 0x9 |
| `new_tools.ts` | 0.497 | 0x9 |
| `db-inspection-tools.ts` | 0.497 | 0x9 |
| `index.ts` | 0.150 | 0x9 |
| `rg_atlas_tools.ts` | 0.000 | 0x9 |
| `skill_tools.ts` | 0.000 | 0x9 |
## Audit Gates — MCP Tool Surface (TRACE)

> Auto-mapped from CLAUDE.md §"Unified Audit Gate System". Last enriched: 2026-05-15
> Run each check from the **sveltekit-frontend/** root.

### Tier A — Code Connectivity

**G1** Static ESM imports
```bash
rg "from.*MODULE" src/ --type ts --type svelte
```

**G2** Dynamic ESM imports
```bash
rg "import(.*MODULE" src/ --type ts --type svelte
```

**G8a** Service layer — use HttpServiceError subclasses, not kit error()
```bash
rg "import .*error.*from .@sveltejs/kit." src/lib/server/ --type ts  # 0 hits
```

### Tier D — Security + Runtime

**G18** Auth guard on API routes
```bash
rg "locals\.user|requireAuth|getSession" src/routes/api/MODULE/ --type ts
```

**G19** Zod validation on API routes
```bash
rg "import.*zod|from.*zod|z\." src/routes/api/MODULE/ --type ts
```

## TODO — Enhancements from ACE Analysis

> Generated from Redis ACE hits (code:graph:node:* + hotspot data). Regenerate: `node scripts/enrich-llms-md.mjs`.

- [ ] **G8a** Use `HttpServiceError` subclasses — never `throw error()` in service layer



<!-- /LLMS-ENRICH -->
<!-- LLMS-GEN v1 · do not edit below this line -->
<!-- generated: 2026-05-15T17:39:23.555Z · llms.md spec · regen: npm run llms:write -->

> Directory: sveltekit-frontend/src/mcp

## Snapshot

- 14 file(s), 0 handler(s)
- Files: 14

## Top Files

- `admin_tools.ts`
- `bifrost_tools.ts`
- `codebase_tools.ts`
- `db-inspection-tools.ts`
- `index.ts`
- `kb-retrieval-server.ts`
- `new_tools.ts`
- `research_tools.ts`
- `rg_atlas_tools.ts`
- `server-fastmcp.ts`

## Agentic tool-calling — quick ACE hits

- `read_file({ filePath: "sveltekit-frontend/src/mcp/<file>" })`
- `verify_fix({ filePath: "sveltekit-frontend/src/mcp/+server.ts" })`

## How to use this file

Agents automatically pick up the nearest `LLMS.md` when editing. The root `LLMS.md` provides repo-wide rules.

Run `npm run llms:write` to regenerate.