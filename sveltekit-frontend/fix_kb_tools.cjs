...
const searchCardsStart = content.indexOf("server.tool(\\n  'kb.search_cards'");
const searchCardsEndMarker = ");\\n\\n// == kb.search_notecards ==";
const searchCardsActualEnd = content.indexOf(searchCardsEndMarker);

// Find the end of kb.explain_retrieval to place the new tools there instead
const explainRetrievalEnd = content.indexOf(');', content.indexOf("'kb.explain_retrieval'")) + 2;

const cleanSearchCards = \`server.tool(
  'kb.search_cards',
  {
...
// 1. Remove the misplaced tools
const startDel = content.indexOf('// == kb.search_notecards');
const endDel = content.indexOf('// ── kb.get_card'); // Move to next proper tool start
// Wait, kb.get_card is at line 1036.
const endDelReal = content.indexOf(');', content.indexOf("'kb.explain_context_pack'")) + 2;

if (startDel > -1 && endDelReal > startDel) {
    const part1 = content.slice(0, startDel);
    const part2 = content.slice(endDelReal);
    content = part1 + part2;
    console.log('Removed misplaced tools');
}

// 2. Re-insert after explain_retrieval
const insertPos = content.indexOf(');', content.indexOf("'kb.explain_retrieval'")) + 2;
if (insertPos > 1) {
    content = content.slice(0, insertPos) + kbTools + content.slice(insertPos);
    console.log('Inserted tools at proper location');
}
