 ...
 *
 * Route: src/routes/(app)/admin/api-testing/agentic-loop/+server.ts
 * Handlers: POST, GET
 *
 * G26 pattern: node env, vi.hoisted mocks (add as needed), lazy import in
 * beforeEach, 4 baseline cases per handler.
 *
 * Run:  npm run test -- admin/api-testing/agentic-loop
 */
...
describe('src/routes/(app)/admin/api-testing/agentic-loop/+server.ts', () => {
  describe('POST /admin/api-testing/agentic-loop', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/(app)/admin/api-testing/agentic-loop/+server.js') as Record<string, unknown>;
      handler = mod.POST as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/admin/api-testing/agentic-loop', body !== undefined ? { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) } : { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
    }
    function makeUrl() { return new URL('http://localhost/admin/api-testing/agentic-loop'); }
  ...
  describe('GET /admin/api-testing/agentic-loop', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;
    ...
    function makeReq(body?: unknown) {
      return new Request('http://localhost/admin/api-testing/agentic-loop', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/admin/api-testing/agentic-loop'); }
