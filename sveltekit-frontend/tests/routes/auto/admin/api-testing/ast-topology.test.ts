 ...
 *
 * Route: src/routes/(app)/admin/api-testing/ast-topology/+server.ts
 * Handlers: GET
 *
 * G26 pattern: node env, vi.hoisted mocks (add as needed), lazy import in
 * beforeEach, 4 baseline cases per handler.
 *
 * Run:  npm run test -- admin/api-testing/ast-topology
 */
...
describe('src/routes/(app)/admin/api-testing/ast-topology/+server.ts', () => {
  describe('GET /admin/api-testing/ast-topology', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/(app)/admin/api-testing/ast-topology/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/admin/api-testing/ast-topology', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/admin/api-testing/ast-topology'); }
