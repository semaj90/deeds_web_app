...
describe('src/routes/api/infrastructure/status/+server.ts', () => {
  describe('GET /api/infrastructure/status', () => {
    ...
    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/infrastructure/status/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/infrastructure/status', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/infrastructure/status'); }
