...
describe('src/routes/api/route-operations/log/+server.ts', () => {
  describe('GET /api/route-operations/log', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/route-operations/log/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/route-operations/log', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/route-operations/log'); }
