 ...
 *
 * Route: src/routes/api/admin/ai-chat/subagent/launch/+server.ts
 * Handlers: POST
 *
 * G26 pattern: node env, vi.hoisted mocks (add as needed), lazy import in
 * beforeEach, 4 baseline cases per handler.
 *
 * Run:  npm run test -- api/admin/ai-chat/subagent/launch
 */
...
describe('src/routes/api/admin/ai-chat/subagent/launch/+server.ts', () => {
  describe('POST /api/admin/ai-chat/subagent/launch', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../../../src/routes/api/admin/ai-chat/subagent/launch/+server.js') as Record<string, unknown>;
      handler = mod.POST as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/admin/ai-chat/subagent/launch', body !== undefined ? { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) } : { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
    }
    function makeUrl() { return new URL('http://localhost/api/admin/ai-chat/subagent/launch'); }
