...
describe('src/routes/api/admin/cache-stats/+server.ts', () => {
  describe('GET /api/admin/cache-stats', () => {
    ...
    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/admin/cache-stats/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/admin/cache-stats', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/admin/cache-stats'); }
