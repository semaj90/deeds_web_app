...
describe('src/routes/api/admin/inference-lane/+server.ts', () => {
  describe('GET /api/admin/inference-lane', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/admin/inference-lane/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/admin/inference-lane', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/admin/inference-lane'); }
