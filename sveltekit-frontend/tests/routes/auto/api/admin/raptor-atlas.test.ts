...
describe('src/routes/api/admin/raptor-atlas/+server.ts', () => {
  describe('GET /api/admin/raptor-atlas', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/admin/raptor-atlas/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/admin/raptor-atlas', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/admin/raptor-atlas'); }
