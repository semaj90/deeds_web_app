...
describe('src/routes/api/admin/observability/+server.ts', () => {
  describe('GET /api/admin/observability', () => {
    ...
    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/admin/observability/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/admin/observability', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/admin/observability'); }
