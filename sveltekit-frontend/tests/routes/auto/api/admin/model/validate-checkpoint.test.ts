 ...
 *
 * Route: src/routes/api/admin/model/validate-checkpoint/+server.ts
 * Handlers: POST, GET
 *
 * G26 pattern: node env, vi.hoisted mocks (add as needed), lazy import in
 * beforeEach, 4 baseline cases per handler.
 *
 * Run:  npm run test -- api/admin/model/validate-checkpoint
 */
...
describe('src/routes/api/admin/model/validate-checkpoint/+server.ts', () => {
  describe('POST /api/admin/model/validate-checkpoint', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../../src/routes/api/admin/model/validate-checkpoint/+server.js') as Record<string, unknown>;
      handler = mod.POST as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/admin/model/validate-checkpoint', body !== undefined ? { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) } : { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
    }
    function makeUrl() { return new URL('http://localhost/api/admin/model/validate-checkpoint'); }
  ...
  describe('GET /api/admin/model/validate-checkpoint', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;
    ...
    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/admin/model/validate-checkpoint', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/admin/model/validate-checkpoint'); }
