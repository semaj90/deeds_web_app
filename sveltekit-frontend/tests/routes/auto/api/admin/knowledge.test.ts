...
describe('src/routes/api/admin/knowledge/+server.ts', () => {
  describe('GET /api/admin/knowledge', () => {
    ...
    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/admin/knowledge/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/admin/knowledge', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/admin/knowledge'); }
