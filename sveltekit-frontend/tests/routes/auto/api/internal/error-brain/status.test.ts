 ...
 *
 * Route: src/routes/api/internal/error-brain/status/+server.ts
 * Handlers: GET
 *
 * G26 pattern: node env, vi.hoisted mocks (add as needed), lazy import in
 * beforeEach, 4 baseline cases per handler.
 *
 * Run:  npm run test -- api/internal/error-brain/status
 */
...
describe('src/routes/api/internal/error-brain/status/+server.ts', () => {
  describe('GET /api/internal/error-brain/status', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../../src/routes/api/internal/error-brain/status/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/internal/error-brain/status', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/internal/error-brain/status'); }
