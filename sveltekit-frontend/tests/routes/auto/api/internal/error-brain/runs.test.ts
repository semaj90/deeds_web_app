...
describe('src/routes/api/internal/error-brain/runs/+server.ts', () => {
  describe('GET /api/internal/error-brain/runs', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../../src/routes/api/internal/error-brain/runs/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/internal/error-brain/runs', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/internal/error-brain/runs'); }
  ...
  describe('POST /api/internal/error-brain/runs', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;
    ...
    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/internal/error-brain/runs', body !== undefined ? { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) } : { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
    }
    function makeUrl() { return new URL('http://localhost/api/internal/error-brain/runs'); }
