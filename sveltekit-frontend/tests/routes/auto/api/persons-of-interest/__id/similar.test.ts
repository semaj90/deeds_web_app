 ...
 *
 * Route: src/routes/api/persons-of-interest/[id]/similar/+server.ts
 * Handlers: GET
 *
 * G26 pattern: node env, vi.hoisted mocks (add as needed), lazy import in
 * beforeEach, 4 baseline cases per handler.
 *
 * Run:  npm run test -- api/persons-of-interest/__id/similar
 */
...
describe('src/routes/api/persons-of-interest/[id]/similar/+server.ts', () => {
  describe('GET /api/persons-of-interest/:id/similar', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../../src/routes/api/persons-of-interest/[id]/similar/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/persons-of-interest/:id/similar', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/persons-of-interest/:id/similar'); }
