...
describe('src/routes/api/gpu/queue/+server.ts', () => {
  describe('GET /api/gpu/queue', () => {
    ...
    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/gpu/queue/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/gpu/queue', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/gpu/queue'); }
  ...
  describe('POST /api/gpu/queue', () => {
    ...
    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/gpu/queue', body !== undefined ? { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) } : { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
    }
    function makeUrl() { return new URL('http://localhost/api/gpu/queue'); }
