...
describe('src/routes/api/reports/[id]/export/+server.ts', () => {
  describe('GET /api/reports/:id/export', () => {
    ...
    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../../src/routes/api/reports/[id]/export/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/reports/:id/export', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/reports/:id/export'); }
  ...
  describe('POST /api/reports/:id/export', () => {
    ...
    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/reports/:id/export', body !== undefined ? { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) } : { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
    }
    function makeUrl() { return new URL('http://localhost/api/reports/:id/export'); }
