...
describe('src/routes/api/v1/agentic/+server.ts', () => {
  describe('GET /api/v1/agentic', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/v1/agentic/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/v1/agentic', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/v1/agentic'); }
