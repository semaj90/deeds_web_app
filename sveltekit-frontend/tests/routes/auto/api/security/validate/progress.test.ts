...
describe('src/routes/api/security/validate/progress/+server.ts', () => {
  describe('GET /api/security/validate/progress', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../../src/routes/api/security/validate/progress/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/security/validate/progress', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/security/validate/progress'); }
