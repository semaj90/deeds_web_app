...
describe('src/routes/api/chat/stream/+server.ts', () => {
  describe('GET /api/chat/stream', () => {
    ...
    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/chat/stream/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/chat/stream', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/chat/stream'); }
  ...
  describe('POST /api/chat/stream', () => {
    ...
    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/chat/stream', body !== undefined ? { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) } : { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
    }
    function makeUrl() { return new URL('http://localhost/api/chat/stream'); }
