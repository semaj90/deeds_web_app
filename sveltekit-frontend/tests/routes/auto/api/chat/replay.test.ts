...
describe('src/routes/api/chat/replay/+server.ts', () => {
  describe('GET /api/chat/replay', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/chat/replay/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/chat/replay', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/chat/replay'); }
