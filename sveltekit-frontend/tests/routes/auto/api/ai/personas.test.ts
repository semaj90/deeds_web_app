...
describe('src/routes/api/ai/personas/+server.ts', () => {
  describe('GET /api/ai/personas', () => {
    ...
    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/ai/personas/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/ai/personas', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/ai/personas'); }
