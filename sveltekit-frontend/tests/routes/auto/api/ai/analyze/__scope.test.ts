 ...
 *
 * Run:  npm run test -- api/ai/analyze/__scope
 */
...
describe('src/routes/api/ai/analyze/[scope]/+server.ts', () => {
  describe('POST /api/ai/analyze/:scope', () => {
    ...
    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/ai/analyze/:scope', body !== undefined ? { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) } : { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
    }
    function makeUrl() { return new URL('http://localhost/api/ai/analyze/:scope'); }
