...
describe('src/routes/api/ai/models/+server.ts', () => {
  describe('GET /api/ai/models', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/ai/models/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/ai/models', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/ai/models'); }
