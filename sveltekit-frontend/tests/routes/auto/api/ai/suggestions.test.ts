...
describe('src/routes/api/ai/suggestions/+server.ts', () => {
  describe('GET /api/ai/suggestions', () => {
    ...
    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/ai/suggestions/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/ai/suggestions', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/ai/suggestions'); }
