 ...
 *
 * Run:  npm run test -- api/ai/generate-report/__scope
 */
...
describe('src/routes/api/ai/generate-report/[scope]/+server.ts', () => {
  describe('POST /api/ai/generate-report/:scope', () => {
    ...
    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/ai/generate-report/:scope', body !== undefined ? { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) } : { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
    }
    function makeUrl() { return new URL('http://localhost/api/ai/generate-report/:scope'); }
