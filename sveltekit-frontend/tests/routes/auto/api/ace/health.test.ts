...
describe('src/routes/api/ace/health/+server.ts', () => {
  describe('GET /api/ace/health', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/ace/health/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/ace/health', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/ace/health'); }
