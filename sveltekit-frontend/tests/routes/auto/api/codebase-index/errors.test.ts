...
describe('src/routes/api/codebase-index/errors/+server.ts', () => {
  describe('GET /api/codebase-index/errors', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/codebase-index/errors/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/codebase-index/errors', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/codebase-index/errors'); }
