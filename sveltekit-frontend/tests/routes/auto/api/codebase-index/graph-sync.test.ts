...
describe('src/routes/api/codebase-index/graph-sync/+server.ts', () => {
  describe('POST /api/codebase-index/graph-sync', () => {
    ...
    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/codebase-index/graph-sync/+server.js') as Record<string, unknown>;
      handler = mod.POST as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/codebase-index/graph-sync', body !== undefined ? { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) } : { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
    }
    function makeUrl() { return new URL('http://localhost/api/codebase-index/graph-sync'); }
  ...
  describe('GET /api/codebase-index/graph-sync', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;
    ...
    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/codebase-index/graph-sync', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/codebase-index/graph-sync'); }
