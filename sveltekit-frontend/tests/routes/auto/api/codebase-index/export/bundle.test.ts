 ...
 *
 * Route: src/routes/api/codebase-index/export/bundle/+server.ts
 * Handlers: GET
 *
 * G26 pattern: node env, vi.hoisted mocks (add as needed), lazy import in
 * beforeEach, 4 baseline cases per handler.
 *
 * Run:  npm run test -- api/codebase-index/export/bundle
 */
...
describe('src/routes/api/codebase-index/export/bundle/+server.ts', () => {
  describe('GET /api/codebase-index/export/bundle', () => {
    ...
    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../../src/routes/api/codebase-index/export/bundle/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/codebase-index/export/bundle', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/codebase-index/export/bundle'); }
