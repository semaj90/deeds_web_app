 ...
 *
 * Route: src/routes/api/codebase-index/claude-assist/feedback/analysis/+server.ts
 * Handlers: GET
 *
 * G26 pattern: node env, vi.hoisted mocks (add as needed), lazy import in
 * beforeEach, 4 baseline cases per handler.
 *
 * Run:  npm run test -- api/codebase-index/claude-assist/feedback/analysis
 */
...
describe('src/routes/api/codebase-index/claude-assist/feedback/analysis/+server.ts', () => {
  describe('GET /api/codebase-index/claude-assist/feedback/analysis', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../../../src/routes/api/codebase-index/claude-assist/feedback/analysis/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/codebase-index/claude-assist/feedback/analysis', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/codebase-index/claude-assist/feedback/analysis'); }
