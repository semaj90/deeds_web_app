 ...
 *
 * Route: src/routes/api/codebase-index/claude-assist/feedback/+server.ts
 * Handlers: POST, GET
 *
 * G26 pattern: node env, vi.hoisted mocks (add as needed), lazy import in
 * beforeEach, 4 baseline cases per handler.
 *
 * Run:  npm run test -- api/codebase-index/claude-assist/feedback
 */
...
describe('src/routes/api/codebase-index/claude-assist/feedback/+server.ts', () => {
  describe('POST /api/codebase-index/claude-assist/feedback', () => {
    ...
    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../../src/routes/api/codebase-index/claude-assist/feedback/+server.js') as Record<string, unknown>;
      handler = mod.POST as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/codebase-index/claude-assist/feedback', body !== undefined ? { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) } : { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
    }
    function makeUrl() { return new URL('http://localhost/api/codebase-index/claude-assist/feedback'); }
  ...
  describe('GET /api/codebase-index/claude-assist/feedback', () => {
    ...
    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/codebase-index/claude-assist/feedback', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/codebase-index/claude-assist/feedback'); }
