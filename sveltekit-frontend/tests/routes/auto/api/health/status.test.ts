...
describe('src/routes/api/health/status/+server.ts', () => {
  describe('GET /api/health/status', () => {
    ...
    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/health/status/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/health/status', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/health/status'); }
