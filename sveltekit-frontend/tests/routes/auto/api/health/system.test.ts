...
describe('src/routes/api/health/system/+server.ts', () => {
  describe('GET /api/health/system', () => {
    ...
    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/health/system/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/health/system', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/health/system'); }
