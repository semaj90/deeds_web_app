...
describe('src/routes/api/health/ocr/+server.ts', () => {
  describe('GET /api/health/ocr', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/health/ocr/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/health/ocr', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/health/ocr'); }
  ...
  describe('POST /api/health/ocr', () => {
    ...
    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/health/ocr', body !== undefined ? { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) } : { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
    }
    function makeUrl() { return new URL('http://localhost/api/health/ocr'); }
  ...
  describe('HEAD /api/health/ocr', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/health/ocr/+server.js') as Record<string, unknown>;
      handler = mod.HEAD as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/health/ocr', { method: 'HEAD' });
    }
    function makeUrl() { return new URL('http://localhost/api/health/ocr'); }
