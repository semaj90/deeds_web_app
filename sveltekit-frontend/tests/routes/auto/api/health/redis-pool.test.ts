...
describe('src/routes/api/health/redis-pool/+server.ts', () => {
  describe('GET /api/health/redis-pool', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/health/redis-pool/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/health/redis-pool', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/health/redis-pool'); }
