...
describe('src/routes/api/health/database/+server.ts', () => {
  describe('GET /api/health/database', () => {
    ...
    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/health/database/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/health/database', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/health/database'); }
