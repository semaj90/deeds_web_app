...
describe('src/routes/api/health/neo4j/+server.ts', () => {
  describe('GET /api/health/neo4j', () => {
    ...
    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/health/neo4j/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/health/neo4j', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/health/neo4j'); }
