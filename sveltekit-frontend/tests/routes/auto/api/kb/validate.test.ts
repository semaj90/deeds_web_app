...
describe('src/routes/api/kb/validate/+server.ts', () => {
  describe('POST /api/kb/validate', () => {
    ...
    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/kb/validate/+server.js') as Record<string, unknown>;
      handler = mod.POST as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/kb/validate', body !== undefined ? { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) } : { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
    }
    function makeUrl() { return new URL('http://localhost/api/kb/validate'); }
  ...
  describe('GET /api/kb/validate', () => {
    ...
    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/kb/validate', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/kb/validate'); }
