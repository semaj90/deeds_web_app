...
describe('src/routes/api/mcp/+server.ts', () => {
  describe('GET /api/mcp', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../src/routes/api/mcp/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/mcp', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/mcp'); }
  ...
  describe('POST /api/mcp', () => {
    ...
    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/mcp', body !== undefined ? { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) } : { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
    }
    function makeUrl() { return new URL('http://localhost/api/mcp'); }
