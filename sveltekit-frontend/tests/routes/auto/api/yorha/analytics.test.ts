...
describe('src/routes/api/yorha/analytics/+server.ts', () => {
  describe('GET /api/yorha/analytics', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/yorha/analytics/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/yorha/analytics', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/yorha/analytics'); }
