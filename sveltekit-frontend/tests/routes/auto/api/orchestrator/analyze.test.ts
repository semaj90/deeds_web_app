...
describe('src/routes/api/orchestrator/analyze/+server.ts', () => {
  describe('POST /api/orchestrator/analyze', () => {
    ...
    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/orchestrator/analyze/+server.js') as Record<string, unknown>;
      handler = mod.POST as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/orchestrator/analyze', body !== undefined ? { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) } : { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
    }
    function makeUrl() { return new URL('http://localhost/api/orchestrator/analyze'); }
  ...
  describe('GET /api/orchestrator/analyze', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;
    ...
    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/orchestrator/analyze', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/orchestrator/analyze'); }
