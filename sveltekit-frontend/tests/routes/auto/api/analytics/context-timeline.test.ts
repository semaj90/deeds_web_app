...
describe('src/routes/api/analytics/context-timeline/+server.ts', () => {
  describe('GET /api/analytics/context-timeline', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/analytics/context-timeline/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/analytics/context-timeline', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/analytics/context-timeline'); }
  ...
  describe('POST /api/analytics/context-timeline', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;
    ...
    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/analytics/context-timeline', body !== undefined ? { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) } : { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
    }
    function makeUrl() { return new URL('http://localhost/api/analytics/context-timeline'); }
