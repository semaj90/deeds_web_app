...
describe('src/routes/api/analytics/token-usage/+server.ts', () => {
  describe('GET /api/analytics/token-usage', () => {
    ...
    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/analytics/token-usage/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/analytics/token-usage', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/analytics/token-usage'); }
