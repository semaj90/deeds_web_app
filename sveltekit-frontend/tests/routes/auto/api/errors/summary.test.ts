...
describe('src/routes/api/errors/summary/+server.ts', () => {
  describe('GET /api/errors/summary', () => {
    ...
    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/errors/summary/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/errors/summary', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/errors/summary'); }
