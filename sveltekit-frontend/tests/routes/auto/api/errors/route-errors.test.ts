...
describe('src/routes/api/errors/route-errors/+server.ts', () => {
  describe('GET /api/errors/route-errors', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/errors/route-errors/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/errors/route-errors', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/errors/route-errors'); }
