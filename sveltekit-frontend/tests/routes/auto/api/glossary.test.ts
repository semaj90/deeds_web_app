...
describe('src/routes/api/glossary/+server.ts', () => {
  describe('GET /api/glossary', () => {
    ...
    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../src/routes/api/glossary/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/glossary', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/glossary'); }
  ...
  describe('POST /api/glossary', () => {
    ...
    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/glossary', body !== undefined ? { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) } : { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
    }
    function makeUrl() { return new URL('http://localhost/api/glossary'); }
