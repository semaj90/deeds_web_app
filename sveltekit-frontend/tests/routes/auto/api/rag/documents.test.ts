...
describe('src/routes/api/rag/documents/+server.ts', () => {
  describe('GET /api/rag/documents', () => {
    ...
    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/rag/documents/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/rag/documents', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/rag/documents'); }
