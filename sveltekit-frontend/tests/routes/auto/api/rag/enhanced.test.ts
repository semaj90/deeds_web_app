...
describe('src/routes/api/rag/enhanced/+server.ts', () => {
  describe('POST /api/rag/enhanced', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/rag/enhanced/+server.js') as Record<string, unknown>;
      handler = mod.POST as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/rag/enhanced', body !== undefined ? { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) } : { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
    }
    function makeUrl() { return new URL('http://localhost/api/rag/enhanced'); }
  ...
  describe('GET /api/rag/enhanced', () => {
    ...
    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/rag/enhanced', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/rag/enhanced'); }
