...
describe('src/routes/api/rag/suggestions/+server.ts', () => {
  describe('GET /api/rag/suggestions', () => {
    ...
    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/rag/suggestions/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/rag/suggestions', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/rag/suggestions'); }
