...
describe('src/routes/api/rag/unified/+server.ts', () => {
  describe('GET /api/rag/unified', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/rag/unified/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/rag/unified', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/rag/unified'); }
