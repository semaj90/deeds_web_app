 ...
 *
 * Route: src/routes/api/error-brain/diagnosis-history/similar/+server.ts
 * Handlers: POST
 *
 * G26 pattern: node env, vi.hoisted mocks (add as needed), lazy import in
 * beforeEach, 4 baseline cases per handler.
 *
 * Run:  npm run test -- api/error-brain/diagnosis-history/similar
 */
...
describe('src/routes/api/error-brain/diagnosis-history/similar/+server.ts', () => {
  describe('POST /api/error-brain/diagnosis-history/similar', () => {
    ...
    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../../src/routes/api/error-brain/diagnosis-history/similar/+server.js') as Record<string, unknown>;
      handler = mod.POST as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/error-brain/diagnosis-history/similar', body !== undefined ? { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) } : { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
    }
    function makeUrl() { return new URL('http://localhost/api/error-brain/diagnosis-history/similar'); }
