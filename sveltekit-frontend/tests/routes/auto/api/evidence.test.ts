...
describe('src/routes/api/evidence/+server.ts', () => {
  describe('GET /api/evidence', () => {
    ...
    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../src/routes/api/evidence/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/evidence', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/evidence'); }
