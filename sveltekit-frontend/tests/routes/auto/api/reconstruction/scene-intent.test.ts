 ...
 *
 * Route: src/routes/api/reconstruction/scene-intent/+server.ts
 * Handlers: POST, GET
 *
 * G26 pattern: node env, vi.hoisted mocks (add as needed), lazy import in
 * beforeEach, 4 baseline cases per handler.
 *
 * Run:  npm run test -- api/reconstruction/scene-intent
 */
...
describe('src/routes/api/reconstruction/scene-intent/+server.ts', () => {
  describe('POST /api/reconstruction/scene-intent', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/reconstruction/scene-intent/+server.js') as Record<string, unknown>;
      handler = mod.POST as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/reconstruction/scene-intent', body !== undefined ? { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) } : { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
    }
    function makeUrl() { return new URL('http://localhost/api/reconstruction/scene-intent'); }
  ...
  describe('GET /api/reconstruction/scene-intent', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;
    ...
    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/reconstruction/scene-intent', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/reconstruction/scene-intent'); }
