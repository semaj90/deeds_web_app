...
describe('src/routes/api/topology/+server.ts', () => {
  describe('GET /api/topology', () => {
    ...
    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../src/routes/api/topology/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/topology', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/topology'); }
