...
describe('src/routes/api/synthesis/qlora-export/+server.ts', () => {
  describe('GET /api/synthesis/qlora-export', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/synthesis/qlora-export/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/synthesis/qlora-export', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/synthesis/qlora-export'); }
