...
describe('src/routes/api/dashboard/stats/+server.ts', () => {
  describe('GET /api/dashboard/stats', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/dashboard/stats/+server.js') as Record<string, unknown>;
    ...
    function makeUrl() { return new URL('http://localhost/api/dashboard/stats'); }
