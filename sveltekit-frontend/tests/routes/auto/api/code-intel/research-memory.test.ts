...
describe('src/routes/api/code-intel/research-memory/+server.ts', () => {
  describe('GET /api/code-intel/research-memory', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/code-intel/research-memory/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/code-intel/research-memory', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/code-intel/research-memory'); }
