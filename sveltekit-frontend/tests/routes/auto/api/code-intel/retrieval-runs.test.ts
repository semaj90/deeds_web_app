...
describe('src/routes/api/code-intel/retrieval-runs/+server.ts', () => {
  describe('GET /api/code-intel/retrieval-runs', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/code-intel/retrieval-runs/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/code-intel/retrieval-runs', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/code-intel/retrieval-runs'); }
