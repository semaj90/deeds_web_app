...
describe('src/routes/api/code-intel/latest-index/+server.ts', () => {
  describe('GET /api/code-intel/latest-index', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/code-intel/latest-index/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/code-intel/latest-index', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/code-intel/latest-index'); }
