 ...
 *
 * Route: src/routes/api/code-intel/memory-gain/rejected/+server.ts
 * Handlers: GET
 *
 * G26 pattern: node env, vi.hoisted mocks (add as needed), lazy import in
 * beforeEach, 4 baseline cases per handler.
 *
 * Run:  npm run test -- api/code-intel/memory-gain/rejected
 */
...
describe('src/routes/api/code-intel/memory-gain/rejected/+server.ts', () => {
  describe('GET /api/code-intel/memory-gain/rejected', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../../src/routes/api/code-intel/memory-gain/rejected/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/code-intel/memory-gain/rejected', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/code-intel/memory-gain/rejected'); }
