...
describe('src/routes/api/chrrom/push/+server.ts', () => {
  describe('GET /api/chrrom/push', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/chrrom/push/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/chrrom/push', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/chrrom/push'); }
  ...
  describe('POST /api/chrrom/push', () => {
    ...
    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/chrrom/push', body !== undefined ? { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) } : { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
    }
    function makeUrl() { return new URL('http://localhost/api/chrrom/push'); }
