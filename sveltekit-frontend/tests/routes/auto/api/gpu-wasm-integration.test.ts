...
describe('src/routes/api/gpu-wasm-integration/+server.ts', () => {
  describe('POST /api/gpu-wasm-integration', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../src/routes/api/gpu-wasm-integration/+server.js') as Record<string, unknown>;
      handler = mod.POST as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/gpu-wasm-integration', body !== undefined ? { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) } : { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
    }
    function makeUrl() { return new URL('http://localhost/api/gpu-wasm-integration'); }
  ...
  describe('GET /api/gpu-wasm-integration', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;
    ...
    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/gpu-wasm-integration', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/gpu-wasm-integration'); }
