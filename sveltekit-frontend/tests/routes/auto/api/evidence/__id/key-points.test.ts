...
describe('src/routes/api/evidence/[id]/key-points/+server.ts', () => {
  describe('GET /api/evidence/:id/key-points', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../../src/routes/api/evidence/[id]/key-points/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/evidence/:id/key-points', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/evidence/:id/key-points'); }
  ...
  describe('POST /api/evidence/:id/key-points', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;
    ...
    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/evidence/:id/key-points', body !== undefined ? { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) } : { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
    }
    function makeUrl() { return new URL('http://localhost/api/evidence/:id/key-points'); }
