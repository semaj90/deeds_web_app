 ...
 *
 * Route: src/routes/api/evidence/[id]/chain-of-custody/+server.ts
 * Handlers: GET, POST
 *
 * G26 pattern: node env, vi.hoisted mocks (add as needed), lazy import in
 * beforeEach, 4 baseline cases per handler.
 *
 * Run:  npm run test -- api/evidence/__id/chain-of-custody
 */
...
describe('src/routes/api/evidence/[id]/chain-of-custody/+server.ts', () => {
  describe('GET /api/evidence/:id/chain-of-custody', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../../src/routes/api/evidence/[id]/chain-of-custody/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/evidence/:id/chain-of-custody', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/evidence/:id/chain-of-custody'); }
  ...
  describe('POST /api/evidence/:id/chain-of-custody', () => {
    ...
    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/evidence/:id/chain-of-custody', body !== undefined ? { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) } : { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
    }
    function makeUrl() { return new URL('http://localhost/api/evidence/:id/chain-of-custody'); }
