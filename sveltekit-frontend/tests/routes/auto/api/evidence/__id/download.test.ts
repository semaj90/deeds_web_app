...
describe('src/routes/api/evidence/[id]/download/+server.ts', () => {
  describe('GET /api/evidence/:id/download', () => {
    ...
    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../../src/routes/api/evidence/[id]/download/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/evidence/:id/download', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/evidence/:id/download'); }
