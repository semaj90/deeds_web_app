...
describe('src/routes/api/evidence/[docId]/status/+server.ts', () => {
  describe('GET /api/evidence/:docId/status', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../../src/routes/api/evidence/[docId]/status/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/evidence/:docId/status', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/evidence/:docId/status'); }
