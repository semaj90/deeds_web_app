...
describe('src/routes/api/phase109/tag-chunks/+server.ts', () => {
  describe('GET /api/phase109/tag-chunks', () => {
    ...
    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/phase109/tag-chunks/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/phase109/tag-chunks', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/phase109/tag-chunks'); }
  ...
  describe('POST /api/phase109/tag-chunks', () => {
    ...
    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/phase109/tag-chunks', body !== undefined ? { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) } : { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
    }
    function makeUrl() { return new URL('http://localhost/api/phase109/tag-chunks'); }
