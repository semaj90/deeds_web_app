...
describe('src/routes/api/consolidation/status/+server.ts', () => {
  describe('GET /api/consolidation/status', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/consolidation/status/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/consolidation/status', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/consolidation/status'); }
