...
describe('src/routes/api/phase89/node/[id]/docs/+server.ts', () => {
  describe('GET /api/phase89/node/:id/docs', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../../../src/routes/api/phase89/node/[id]/docs/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/phase89/node/:id/docs', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/phase89/node/:id/docs'); }
