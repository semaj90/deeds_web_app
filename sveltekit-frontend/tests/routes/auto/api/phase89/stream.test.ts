...
describe('src/routes/api/phase89/stream/+server.ts', () => {
  describe('GET /api/phase89/stream', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/phase89/stream/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/phase89/stream', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/phase89/stream'); }
