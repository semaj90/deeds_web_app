 ...
 *
 * Run:  npm run test -- api/recommendations/__userId
 */
...
describe('src/routes/api/recommendations/[userId]/+server.ts', () => {
  describe('GET /api/recommendations/:userId', () => {
    ...
    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/recommendations/:userId', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/recommendations/:userId'); }
  ...
  describe('POST /api/recommendations/:userId', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;
    ...
    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/recommendations/:userId', body !== undefined ? { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) } : { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
    }
    function makeUrl() { return new URL('http://localhost/api/recommendations/:userId'); }
