...
describe('src/routes/api/simulation/+server.ts', () => {
  describe('POST /api/simulation', () => {
    ...
    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../src/routes/api/simulation/+server.js') as Record<string, unknown>;
      handler = mod.POST as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/simulation', body !== undefined ? { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) } : { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
    }
    function makeUrl() { return new URL('http://localhost/api/simulation'); }
  ...
  describe('GET /api/simulation', () => {
    ...
    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/simulation', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/simulation'); }
