 ...
 *
 * Route: src/routes/api/cache/+server.ts
...
describe('src/routes/api/cache/+server.ts', () => {
  describe('GET /api/cache', () => {
    ...
    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../src/routes/api/cache/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/cache', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/cache'); }
  ...
  describe('POST /api/cache', () => {
    ...
    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/cache', body !== undefined ? { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) } : { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
    }
    function makeUrl() { return new URL('http://localhost/api/cache'); }
  ...
  describe('DELETE /api/cache', () => {
    ...
    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/cache', body !== undefined ? { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) } : { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
    }
    function makeUrl() { return new URL('http://localhost/api/cache'); }
