...
describe('src/routes/api/glossary/terms/+server.ts', () => {
  describe('GET /api/glossary/terms', () => {
    ...
    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/glossary/terms/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/glossary/terms', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/glossary/terms'); }
