...
describe('src/routes/api/cache/exact-match/stats/+server.ts', () => {
  describe('GET /api/cache/exact-match/stats', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../../src/routes/api/cache/exact-match/stats/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/cache/exact-match/stats', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/cache/exact-match/stats'); }
