...
describe('src/routes/api/cache/nintendo/+server.ts', () => {
  describe('GET /api/cache/nintendo', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/cache/nintendo/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/cache/nintendo', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/cache/nintendo'); }
