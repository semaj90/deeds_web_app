...
describe('src/routes/api/cache/llm/stats/+server.ts', () => {
  describe('GET /api/cache/llm/stats', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../../src/routes/api/cache/llm/stats/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/cache/llm/stats', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/cache/llm/stats'); }
