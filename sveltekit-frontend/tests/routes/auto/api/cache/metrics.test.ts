...
describe('src/routes/api/cache/metrics/+server.ts', () => {
  describe('GET /api/cache/metrics', () => {
    ...
    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/cache/metrics/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/cache/metrics', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/cache/metrics'); }
