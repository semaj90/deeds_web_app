...
describe('src/routes/api/cases/[id]/related/+server.ts', () => {
  describe('GET /api/cases/:id/related', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../../src/routes/api/cases/[id]/related/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/cases/:id/related', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/cases/:id/related'); }
