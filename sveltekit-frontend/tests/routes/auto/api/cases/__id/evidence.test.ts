...
describe('src/routes/api/cases/[id]/evidence/+server.ts', () => {
  describe('GET /api/cases/:id/evidence', () => {
    ...
    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../../src/routes/api/cases/[id]/evidence/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/cases/:id/evidence', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/cases/:id/evidence'); }
  ...
  describe('DELETE /api/cases/:id/evidence', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;
    ...
    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/cases/:id/evidence', body !== undefined ? { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) } : { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
    }
    function makeUrl() { return new URL('http://localhost/api/cases/:id/evidence'); }
