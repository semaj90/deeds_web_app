...
describe('src/routes/api/cases/[id]/similar/+server.ts', () => {
  describe('GET /api/cases/:id/similar', () => {
    ...
    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../../src/routes/api/cases/[id]/similar/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/cases/:id/similar', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/cases/:id/similar'); }
