 ...
 *
 * Route: src/routes/api/cases/[id]/notes/[noteId]/versions/+server.ts
 * Handlers: GET
 *
 * G26 pattern: node env, vi.hoisted mocks (add as needed), lazy import in
 * beforeEach, 4 baseline cases per handler.
 *
 * Run:  npm run test -- api/cases/__id/notes/__noteId/versions
 */
...
describe('src/routes/api/cases/[id]/notes/[noteId]/versions/+server.ts', () => {
  describe('GET /api/cases/:id/notes/:noteId/versions', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../../../../src/routes/api/cases/[id]/notes/[noteId]/versions/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/cases/:id/notes/:noteId/versions', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/cases/:id/notes/:noteId/versions'); }
