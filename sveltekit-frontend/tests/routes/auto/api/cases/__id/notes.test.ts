...
describe('src/routes/api/cases/[id]/notes/+server.ts', () => {
  describe('GET /api/cases/:id/notes', () => {
    ...
    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../../src/routes/api/cases/[id]/notes/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/cases/:id/notes', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/cases/:id/notes'); }
  ...
  describe('POST /api/cases/:id/notes', () => {
    ...
    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/cases/:id/notes', body !== undefined ? { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) } : { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
    }
    function makeUrl() { return new URL('http://localhost/api/cases/:id/notes'); }
