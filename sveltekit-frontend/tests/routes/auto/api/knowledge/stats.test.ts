...
describe('src/routes/api/knowledge/stats/+server.ts', () => {
  describe('GET /api/knowledge/stats', () => {
    ...
    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/knowledge/stats/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/knowledge/stats', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/knowledge/stats'); }
