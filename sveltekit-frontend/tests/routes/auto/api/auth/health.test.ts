...
describe('src/routes/api/auth/health/+server.ts', () => {
  describe('GET /api/auth/health', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/auth/health/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/auth/health', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/auth/health'); }
