...
describe('src/routes/api/auth/logout/+server.ts', () => {
  describe('POST /api/auth/logout', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/auth/logout/+server.js') as Record<string, unknown>;
      handler = mod.POST as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/auth/logout', body !== undefined ? { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) } : { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
    }
    function makeUrl() { return new URL('http://localhost/api/auth/logout'); }
    ...
    it('401 — returns Unauthorized when locals.user is missing', async () => {
      ...
      expect([200, 204, 302, 303, 400, 401, 403, 404, 405, 429, 500, 503]).toContain(status);
    });
  ...
  describe('GET /api/auth/logout', () => {
    ...
    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/auth/logout', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/auth/logout'); }
