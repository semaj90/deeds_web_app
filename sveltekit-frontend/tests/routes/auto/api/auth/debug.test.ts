...
describe('src/routes/api/auth/debug/+server.ts', () => {
  describe('GET /api/auth/debug', () => {
    ...
    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/auth/debug/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/auth/debug', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/auth/debug'); }
