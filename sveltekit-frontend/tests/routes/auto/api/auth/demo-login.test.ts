...
describe('src/routes/api/auth/demo-login/+server.ts', () => {
  describe('POST /api/auth/demo-login', () => {
    ...
    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/auth/demo-login/+server.js') as Record<string, unknown>;
      handler = mod.POST as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/auth/demo-login', body !== undefined ? { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) } : { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
    }
    function makeUrl() { return new URL('http://localhost/api/auth/demo-login'); }
  ...
  describe('GET /api/auth/demo-login', () => {
    ...
    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/auth/demo-login', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/auth/demo-login'); }
