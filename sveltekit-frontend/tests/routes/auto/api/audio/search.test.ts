...
describe('src/routes/api/audio/search/+server.ts', () => {
  describe('GET /api/audio/search', () => {
    ...
    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/audio/search/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/audio/search', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/audio/search'); }
