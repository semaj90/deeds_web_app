...
describe('src/routes/api/db/health/+server.ts', () => {
  describe('GET /api/db/health', () => {
    ...
    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/db/health/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/db/health', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/db/health'); }
