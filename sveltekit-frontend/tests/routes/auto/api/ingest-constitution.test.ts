...
describe('src/routes/api/ingest-constitution/+server.ts', () => {
  describe('GET /api/ingest-constitution', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../src/routes/api/ingest-constitution/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/ingest-constitution', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/ingest-constitution'); }
