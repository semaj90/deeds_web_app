...
describe('src/routes/api/embed/+server.ts', () => {
  describe('POST /api/embed', () => {
    ...
    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../src/routes/api/embed/+server.js') as Record<string, unknown>;
      handler = mod.POST as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/embed', body !== undefined ? { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) } : { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
    }
    function makeUrl() { return new URL('http://localhost/api/embed'); }
  ...
  describe('GET /api/embed', () => {
    ...
    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/embed', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/embed'); }
