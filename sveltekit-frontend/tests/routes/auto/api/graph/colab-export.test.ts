...
describe('src/routes/api/graph/colab-export/+server.ts', () => {
  describe('GET /api/graph/colab-export', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/graph/colab-export/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/graph/colab-export', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/graph/colab-export'); }
