...
describe('src/routes/api/graph/analyze/+server.ts', () => {
  describe('POST /api/graph/analyze', () => {
    ...
    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/graph/analyze/+server.js') as Record<string, unknown>;
      handler = mod.POST as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/graph/analyze', body !== undefined ? { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) } : { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
    }
    function makeUrl() { return new URL('http://localhost/api/graph/analyze'); }
  ...
  describe('GET /api/graph/analyze', () => {
    ...
    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/graph/analyze', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/graph/analyze'); }
