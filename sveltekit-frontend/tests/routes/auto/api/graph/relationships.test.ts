...
describe('src/routes/api/graph/relationships/+server.ts', () => {
  describe('GET /api/graph/relationships', () => {
    ...
    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/graph/relationships/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/graph/relationships', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/graph/relationships'); }
