 ...
 *
 * Route: src/routes/api/simulation/[sessionId]/keyframes/+server.ts
 * Handlers: GET, POST
 *
 * G26 pattern: node env, vi.hoisted mocks (add as needed), lazy import in
 * beforeEach, 4 baseline cases per handler.
 *
 * Run:  npm run test -- api/simulation/__sessionId/keyframes
 */
...
describe('src/routes/api/simulation/[sessionId]/keyframes/+server.ts', () => {
  describe('GET /api/simulation/:sessionId/keyframes', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../../src/routes/api/simulation/[sessionId]/keyframes/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/simulation/:sessionId/keyframes', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/simulation/:sessionId/keyframes'); }
  ...
  describe('POST /api/simulation/:sessionId/keyframes', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;
    ...
    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/simulation/:sessionId/keyframes', body !== undefined ? { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) } : { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
    }
    function makeUrl() { return new URL('http://localhost/api/simulation/:sessionId/keyframes'); }
