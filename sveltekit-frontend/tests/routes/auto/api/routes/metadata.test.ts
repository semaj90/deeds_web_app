...
describe('src/routes/api/routes/metadata/+server.ts', () => {
  describe('GET /api/routes/metadata', () => {
    ...
    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/routes/metadata/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/routes/metadata', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/routes/metadata'); }
