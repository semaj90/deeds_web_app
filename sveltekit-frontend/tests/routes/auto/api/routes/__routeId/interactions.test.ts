 ...
 *
 * Route: src/routes/api/routes/[routeId]/interactions/+server.ts
 * Handlers: POST, GET
 *
 * G26 pattern: node env, vi.hoisted mocks (add as needed), lazy import in
 * beforeEach, 4 baseline cases per handler.
 *
 * Run:  npm run test -- api/routes/__routeId/interactions
 */
...
describe('src/routes/api/routes/[routeId]/interactions/+server.ts', () => {
  describe('POST /api/routes/:routeId/interactions', () => {
    ...
    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../../src/routes/api/routes/[routeId]/interactions/+server.js') as Record<string, unknown>;
      handler = mod.POST as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/routes/:routeId/interactions', body !== undefined ? { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) } : { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
    }
    function makeUrl() { return new URL('http://localhost/api/routes/:routeId/interactions'); }
  ...
  describe('GET /api/routes/:routeId/interactions', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;
    ...
    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/routes/:routeId/interactions', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/routes/:routeId/interactions'); }
