 ...
 *
 * Route: src/routes/api/routes/[routeId]/error-brain-analysis/+server.ts
 * Handlers: POST
 *
 * G26 pattern: node env, vi.hoisted mocks (add as needed), lazy import in
 * beforeEach, 4 baseline cases per handler.
 *
 * Run:  npm run test -- api/routes/__routeId/error-brain-analysis
 */
...
describe('src/routes/api/routes/[routeId]/error-brain-analysis/+server.ts', () => {
  describe('POST /api/routes/:routeId/error-brain-analysis', () => {
    ...
    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../../src/routes/api/routes/[routeId]/error-brain-analysis/+server.js') as Record<string, unknown>;
      handler = mod.POST as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/routes/:routeId/error-brain-analysis', body !== undefined ? { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) } : { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
    }
    function makeUrl() { return new URL('http://localhost/api/routes/:routeId/error-brain-analysis'); }
