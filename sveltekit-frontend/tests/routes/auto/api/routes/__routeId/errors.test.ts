...
describe('src/routes/api/routes/[routeId]/errors/+server.ts', () => {
  describe('POST /api/routes/:routeId/errors', () => {
    ...
    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../../src/routes/api/routes/[routeId]/errors/+server.js') as Record<string, unknown>;
      handler = mod.POST as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/routes/:routeId/errors', body !== undefined ? { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) } : { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
    }
    function makeUrl() { return new URL('http://localhost/api/routes/:routeId/errors'); }
  ...
  describe('GET /api/routes/:routeId/errors', () => {
    ...
    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/routes/:routeId/errors', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/routes/:routeId/errors'); }
