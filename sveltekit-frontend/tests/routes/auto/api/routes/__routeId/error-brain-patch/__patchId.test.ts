 ...
 *
 * Run:  npm run test -- api/routes/__routeId/error-brain-patch/__patchId
 */
...
describe('src/routes/api/routes/[routeId]/error-brain-patch/[patchId]/+server.ts', () => {
  describe('PUT /api/routes/:routeId/error-brain-patch/:patchId', () => {
    ...
    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/routes/:routeId/error-brain-patch/:patchId', body !== undefined ? { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) } : { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
    }
    function makeUrl() { return new URL('http://localhost/api/routes/:routeId/error-brain-patch/:patchId'); }
