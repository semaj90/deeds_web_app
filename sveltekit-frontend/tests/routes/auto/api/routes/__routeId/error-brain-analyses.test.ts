 ...
 *
 * Route: src/routes/api/routes/[routeId]/error-brain-analyses/+server.ts
 * Handlers: GET
 *
 * G26 pattern: node env, vi.hoisted mocks (add as needed), lazy import in
 * beforeEach, 4 baseline cases per handler.
 *
 * Run:  npm run test -- api/routes/__routeId/error-brain-analyses
 */
...
describe('src/routes/api/routes/[routeId]/error-brain-analyses/+server.ts', () => {
  describe('GET /api/routes/:routeId/error-brain-analyses', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../../src/routes/api/routes/[routeId]/error-brain-analyses/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/routes/:routeId/error-brain-analyses', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/routes/:routeId/error-brain-analyses'); }
