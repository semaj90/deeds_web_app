...
describe('src/routes/api/indexing/+server.ts', () => {
  describe('POST /api/indexing', () => {
    ...
    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../src/routes/api/indexing/+server.js') as Record<string, unknown>;
      handler = mod.POST as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/indexing', body !== undefined ? { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) } : { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
    }
    function makeUrl() { return new URL('http://localhost/api/indexing'); }
  ...
  describe('GET /api/indexing', () => {
    ...
    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/indexing', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/indexing'); }
