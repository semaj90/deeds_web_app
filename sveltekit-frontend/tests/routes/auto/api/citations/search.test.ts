...
describe('src/routes/api/citations/search/+server.ts', () => {
  describe('GET /api/citations/search', () => {
    ...
    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/citations/search/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/citations/search', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/citations/search'); }
