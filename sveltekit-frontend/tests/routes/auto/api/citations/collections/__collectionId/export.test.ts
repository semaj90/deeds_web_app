 ...
 *
 * Route: src/routes/api/citations/collections/[collectionId]/export/+server.ts
 * Handlers: GET, POST
 *
 * G26 pattern: node env, vi.hoisted mocks (add as needed), lazy import in
 * beforeEach, 4 baseline cases per handler.
 *
 * Run:  npm run test -- api/citations/collections/__collectionId/export
 */
...
describe('src/routes/api/citations/collections/[collectionId]/export/+server.ts', () => {
  describe('GET /api/citations/collections/:collectionId/export', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../../../src/routes/api/citations/collections/[collectionId]/export/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/citations/collections/:collectionId/export', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/citations/collections/:collectionId/export'); }
  ...
  describe('POST /api/citations/collections/:collectionId/export', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;
    ...
    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/citations/collections/:collectionId/export', body !== undefined ? { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) } : { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
    }
    function makeUrl() { return new URL('http://localhost/api/citations/collections/:collectionId/export'); }
