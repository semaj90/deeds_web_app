 ...
 *
 * Route: src/routes/api/citations/collections/[collectionId]/citations/+server.ts
 * Handlers: POST, DELETE, GET
 *
 * G26 pattern: node env, vi.hoisted mocks (add as needed), lazy import in
 * beforeEach, 4 baseline cases per handler.
 *
 * Run:  npm run test -- api/citations/collections/__collectionId/citations
 */
...
describe('src/routes/api/citations/collections/[collectionId]/citations/+server.ts', () => {
  describe('POST /api/citations/collections/:collectionId/citations', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../../../src/routes/api/citations/collections/[collectionId]/citations/+server.js') as Record<string, unknown>;
      handler = mod.POST as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/citations/collections/:collectionId/citations', body !== undefined ? { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) } : { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
    }
    function makeUrl() { return new URL('http://localhost/api/citations/collections/:collectionId/citations'); }
  ...
  describe('DELETE /api/citations/collections/:collectionId/citations', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;
    ...
    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/citations/collections/:collectionId/citations', body !== undefined ? { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) } : { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
    }
    function makeUrl() { return new URL('http://localhost/api/citations/collections/:collectionId/citations'); }
  ...
  describe('GET /api/citations/collections/:collectionId/citations', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;
    ...
    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/citations/collections/:collectionId/citations', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/citations/collections/:collectionId/citations'); }
