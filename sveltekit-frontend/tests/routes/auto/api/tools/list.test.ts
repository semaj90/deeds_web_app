...
describe('src/routes/api/tools/list/+server.ts', () => {
  describe('GET /api/tools/list', () => {
    ...
    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/tools/list/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/tools/list', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/tools/list'); }
