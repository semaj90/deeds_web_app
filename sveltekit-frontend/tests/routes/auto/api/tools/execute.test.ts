...
describe('src/routes/api/tools/execute/+server.ts', () => {
  describe('POST /api/tools/execute', () => {
    ...
    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/tools/execute/+server.js') as Record<string, unknown>;
      handler = mod.POST as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/tools/execute', body !== undefined ? { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) } : { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
    }
    function makeUrl() { return new URL('http://localhost/api/tools/execute'); }
  ...
  describe('GET /api/tools/execute', () => {
    ...
    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/tools/execute', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/tools/execute'); }
