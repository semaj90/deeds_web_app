...
describe('src/routes/api/system/phase13/+server.ts', () => {
  describe('GET /api/system/phase13', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/system/phase13/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/system/phase13', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/system/phase13'); }
