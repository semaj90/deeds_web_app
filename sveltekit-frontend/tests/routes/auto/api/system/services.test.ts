...
describe('src/routes/api/system/services/+server.ts', () => {
  describe('GET /api/system/services', () => {
    ...
    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/system/services/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/system/services', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/system/services'); }
