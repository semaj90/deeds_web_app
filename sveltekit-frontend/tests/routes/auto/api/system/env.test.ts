...
describe('src/routes/api/system/env/+server.ts', () => {
  describe('GET /api/system/env', () => {
    ...
    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/system/env/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/system/env', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/system/env'); }
