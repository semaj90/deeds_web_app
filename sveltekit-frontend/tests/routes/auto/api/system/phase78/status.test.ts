...
describe('src/routes/api/system/phase78/status/+server.ts', () => {
  describe('GET /api/system/phase78/status', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../../src/routes/api/system/phase78/status/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/system/phase78/status', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/system/phase78/status'); }
