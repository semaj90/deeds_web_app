...
describe('src/routes/api/library/health/+server.ts', () => {
  describe('GET /api/library/health', () => {
    ...
    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/library/health/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/library/health', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/library/health'); }
