 ...
 *
 * Route: src/routes/api/library/documents/[documentId]/chunks/+server.ts
 * Handlers: GET
 *
 * G26 pattern: node env, vi.hoisted mocks (add as needed), lazy import in
 * beforeEach, 4 baseline cases per handler.
 *
 * Run:  npm run test -- api/library/documents/__documentId/chunks
 */
...
describe('src/routes/api/library/documents/[documentId]/chunks/+server.ts', () => {
  describe('GET /api/library/documents/:documentId/chunks', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../../../src/routes/api/library/documents/[documentId]/chunks/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/library/documents/:documentId/chunks', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/library/documents/:documentId/chunks'); }
