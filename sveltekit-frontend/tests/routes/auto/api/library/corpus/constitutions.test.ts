 ...
 *
 * Route: src/routes/api/library/corpus/constitutions/+server.ts
 * Handlers: GET, POST
 *
 * G26 pattern: node env, vi.hoisted mocks (add as needed), lazy import in
 * beforeEach, 4 baseline cases per handler.
 *
 * Run:  npm run test -- api/library/corpus/constitutions
 */
...
describe('src/routes/api/library/corpus/constitutions/+server.ts', () => {
  describe('GET /api/library/corpus/constitutions', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../../src/routes/api/library/corpus/constitutions/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/library/corpus/constitutions', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/library/corpus/constitutions'); }
  ...
  describe('POST /api/library/corpus/constitutions', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;
    ...
    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/library/corpus/constitutions', body !== undefined ? { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) } : { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
    }
    function makeUrl() { return new URL('http://localhost/api/library/corpus/constitutions'); }
