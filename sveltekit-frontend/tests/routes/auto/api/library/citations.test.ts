...
describe('src/routes/api/library/citations/+server.ts', () => {
  describe('GET /api/library/citations', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/library/citations/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/library/citations', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/library/citations'); }
