...
describe('src/routes/api/library/document/[id]/toc/+server.ts', () => {
  describe('GET /api/library/document/:id/toc', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../../../src/routes/api/library/document/[id]/toc/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/library/document/:id/toc', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/library/document/:id/toc'); }
