...
describe('src/routes/api/library/documents/+server.ts', () => {
  describe('GET /api/library/documents', () => {
    ...
    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/library/documents/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/library/documents', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/library/documents'); }
