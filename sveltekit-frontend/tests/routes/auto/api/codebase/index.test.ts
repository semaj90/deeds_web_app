...
describe('src/routes/api/codebase/index/+server.ts', () => {
  describe('POST /api/codebase/index', () => {
    ...
    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/api/codebase/index/+server.js') as Record<string, unknown>;
      handler = mod.POST as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/codebase/index', body !== undefined ? { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) } : { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
    }
    function makeUrl() { return new URL('http://localhost/api/codebase/index'); }
  ...
  describe('GET /api/codebase/index', () => {
    ...
    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/codebase/index', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/codebase/index'); }
