 ...
 *
 * Route: src/routes/api/docs/+server.ts
 * Handlers: GET
 *
...
describe('src/routes/api/docs/+server.ts', () => {
  describe('GET /api/docs', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../src/routes/api/docs/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/docs', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/docs'); }
