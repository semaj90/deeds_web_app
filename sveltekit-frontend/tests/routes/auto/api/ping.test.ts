...
describe('src/routes/api/ping/+server.ts', () => {
  describe('GET /api/ping', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../src/routes/api/ping/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/api/ping', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/api/ping'); }
