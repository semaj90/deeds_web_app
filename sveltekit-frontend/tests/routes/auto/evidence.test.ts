 ...
 *
 * Route: src/routes/(app)/evidence/+server.ts
 * Handlers: GET, POST
 *
...
describe('src/routes/(app)/evidence/+server.ts', () => {
  describe('GET /evidence', () => {
    ...
    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../src/routes/(app)/evidence/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/evidence', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/evidence'); }
  ...
  describe('POST /evidence', () => {
    ...
    function makeReq(body?: unknown) {
      return new Request('http://localhost/evidence', body !== undefined ? { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) } : { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
    }
    function makeUrl() { return new URL('http://localhost/evidence'); }
