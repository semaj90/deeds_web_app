 ...
 *
 * Route: sveltekit-frontend/src/routes/api/mcp/select-tools/+server.ts
 * Handlers: POST
 *
 * G26 pattern: node env, vi.hoisted mocks (add as needed), lazy import in
 * beforeEach, 4 baseline cases per handler.
 *
 * Run:  npm run test -- sveltekit-frontend/src/routes/api/mcp/select-tools
 */
...
describe('sveltekit-frontend/src/routes/api/mcp/select-tools/+server.ts', () => {
  describe('POST /sveltekit-frontend/src/routes/api/mcp/select-tools', () => {
    ...
    function makeReq(body?: unknown) {
      return new Request('http://localhost/sveltekit-frontend/src/routes/api/mcp/select-tools', body !== undefined ? { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) } : { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
    }
    function makeUrl() { return new URL('http://localhost/sveltekit-frontend/src/routes/api/mcp/select-tools'); }
