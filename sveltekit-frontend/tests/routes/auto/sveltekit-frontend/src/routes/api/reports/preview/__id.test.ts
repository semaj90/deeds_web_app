...
describe('src/routes/api/reports/preview/[id]/+server.ts', () => {
  describe('GET /src/routes/api/reports/preview/:id', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../../../../../src/routes/api/reports/preview/[id]/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/src/routes/api/reports/preview/:id', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/src/routes/api/reports/preview/:id'); }
  ...
  describe('POST /src/routes/api/reports/preview/:id', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;
    ...
    function makeReq(body?: unknown) {
      return new Request('http://localhost/src/routes/api/reports/preview/:id', body !== undefined ? { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) } : { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
    }
    function makeUrl() { return new URL('http://localhost/src/routes/api/reports/preview/:id'); }
