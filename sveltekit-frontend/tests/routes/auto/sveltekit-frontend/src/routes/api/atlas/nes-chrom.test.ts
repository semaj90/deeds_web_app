...
describe('src/routes/api/atlas/nes-chrom/+server.ts', () => {
  describe('GET /src/routes/api/atlas/nes-chrom', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../../../../src/routes/api/atlas/nes-chrom/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/src/routes/api/atlas/nes-chrom', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/src/routes/api/atlas/nes-chrom'); }
