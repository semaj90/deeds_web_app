 ...
 *
 * Route: src/routes/api/admin/summaries/scan-quality/+server.ts
 * Handlers: GET, POST
 *
 * G26 pattern: node env, vi.hoisted mocks (add as needed), lazy import in
 * beforeEach, 4 baseline cases per handler.
 *
 * Run:  npm run test -- src/routes/api/admin/summaries/scan-quality
 */
...
describe('src/routes/api/admin/summaries/scan-quality/+server.ts', () => {
  describe('GET /src/routes/api/admin/summaries/scan-quality', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../../../../../src/routes/api/admin/summaries/scan-quality/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/src/routes/api/admin/summaries/scan-quality', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/src/routes/api/admin/summaries/scan-quality'); }
  ...
  describe('POST /src/routes/api/admin/summaries/scan-quality', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;
    ...
    function makeReq(body?: unknown) {
      return new Request('http://localhost/src/routes/api/admin/summaries/scan-quality', body !== undefined ? { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) } : { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
    }
    function makeUrl() { return new URL('http://localhost/src/routes/api/admin/summaries/scan-quality'); }
