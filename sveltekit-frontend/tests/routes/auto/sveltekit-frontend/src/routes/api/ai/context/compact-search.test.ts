 ...
 *
 * Route: src/routes/api/ai/context/compact-search/+server.ts
 * Handlers: POST
 *
 * G26 pattern: node env, vi.hoisted mocks (add as needed), lazy import in
 * beforeEach, 4 baseline cases per handler.
 *
 * Run:  npm run test -- src/routes/api/ai/context/compact-search
 */
...
describe('src/routes/api/ai/context/compact-search/+server.ts', () => {
  describe('POST /src/routes/api/ai/context/compact-search', () => {
    ...
    function makeReq(body?: unknown) {
      return new Request('http://localhost/src/routes/api/ai/context/compact-search', body !== undefined ? { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) } : { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
    }
    function makeUrl() { return new URL('http://localhost/src/routes/api/ai/context/compact-search'); }
