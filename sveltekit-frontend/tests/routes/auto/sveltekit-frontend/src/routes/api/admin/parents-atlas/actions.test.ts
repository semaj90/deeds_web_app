 ...
 *
 * Route: sveltekit-frontend/src/routes/api/admin/parents-atlas/actions/+server.ts
 * Handlers: POST
 *
 * G26 pattern: node env, vi.hoisted mocks (add as needed), lazy import in
 * beforeEach, 4 baseline cases per handler.
 *
 * Run:  npm run test -- sveltekit-frontend/src/routes/api/admin/parents-atlas/actions
 */
...
describe('sveltekit-frontend/src/routes/api/admin/parents-atlas/actions/+server.ts', () => {
  describe('POST /sveltekit-frontend/src/routes/api/admin/parents-atlas/actions', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../../../../../sveltekit-frontend/src/routes/api/admin/parents-atlas/actions/+server.js') as Record<string, unknown>;
      handler = mod.POST as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/sveltekit-frontend/src/routes/api/admin/parents-atlas/actions', body !== undefined ? { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) } : { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
    }
    function makeUrl() { return new URL('http://localhost/sveltekit-frontend/src/routes/api/admin/parents-atlas/actions'); }
