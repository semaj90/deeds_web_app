 ...
 *
 * Route: sveltekit-frontend/src/routes/api/retrieval/dual-lane/+server.ts
 * Handlers: GET
 *
 * G26 pattern: node env, vi.hoisted mocks (add as needed), lazy import in
 * beforeEach, 4 baseline cases per handler.
 *
 * Run:  npm run test -- sveltekit-frontend/src/routes/api/retrieval/dual-lane
 */
...
describe('sveltekit-frontend/src/routes/api/retrieval/dual-lane/+server.ts', () => {
  describe('GET /sveltekit-frontend/src/routes/api/retrieval/dual-lane', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../../../../sveltekit-frontend/src/routes/api/retrieval/dual-lane/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/sveltekit-frontend/src/routes/api/retrieval/dual-lane', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/sveltekit-frontend/src/routes/api/retrieval/dual-lane'); }
