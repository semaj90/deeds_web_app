...
describe('src/routes/api/admin/atlas/node/[id]/+server.ts', () => {
  describe('GET /src/routes/api/admin/atlas/node/:id', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../../../../../../src/routes/api/admin/atlas/node/[id]/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/src/routes/api/admin/atlas/node/:id', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/src/routes/api/admin/atlas/node/:id'); }
