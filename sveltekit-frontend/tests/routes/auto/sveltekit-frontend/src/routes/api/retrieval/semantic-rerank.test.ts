 ...
 *
 * Route: src/routes/api/retrieval/semantic-rerank/+server.ts
 * Handlers: GET, POST
 *
 * G26 pattern: node env, vi.hoisted mocks (add as needed), lazy import in
 * beforeEach, 4 baseline cases per handler.
 *
 * Run:  npm run test -- src/routes/api/retrieval/semantic-rerank
 */
...
describe('src/routes/api/retrieval/semantic-rerank/+server.ts', () => {
  describe('GET /src/routes/api/retrieval/semantic-rerank', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../../../../src/routes/api/retrieval/semantic-rerank/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/src/routes/api/retrieval/semantic-rerank', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/src/routes/api/retrieval/semantic-rerank'); }
  ...
  describe('POST /src/routes/api/retrieval/semantic-rerank', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;
    ...
    function makeReq(body?: unknown) {
      return new Request('http://localhost/src/routes/api/retrieval/semantic-rerank', body !== undefined ? { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) } : { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
    }
    function makeUrl() { return new URL('http://localhost/src/routes/api/retrieval/semantic-rerank'); }
