 ...
 *
 * Route: src/routes/api/analytics/knowledge-triples/+server.ts
 * Handlers: GET
 *
 * G26 pattern: node env, vi.hoisted mocks (add as needed), lazy import in
 * beforeEach, 4 baseline cases per handler.
 *
 * Run:  npm run test -- src/routes/api/analytics/knowledge-triples
 */
...
describe('src/routes/api/analytics/knowledge-triples/+server.ts', () => {
  describe('GET /src/routes/api/analytics/knowledge-triples', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../../../../src/routes/api/analytics/knowledge-triples/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/src/routes/api/analytics/knowledge-triples', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/src/routes/api/analytics/knowledge-triples'); }
