...
describe('src/routes/api/trpc/[...procedure]/+server.ts', () => {
  describe('GET /src/routes/api/trpc/:...procedure', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../../../../src/routes/api/trpc/[...procedure]/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/src/routes/api/trpc/:...procedure', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/src/routes/api/trpc/:...procedure'); }
  ...
  describe('POST /src/routes/api/trpc/:...procedure', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;
    ...
    function makeReq(body?: unknown) {
      return new Request('http://localhost/src/routes/api/trpc/:...procedure', body !== undefined ? { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) } : { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
    }
    function makeUrl() { return new URL('http://localhost/src/routes/api/trpc/:...procedure'); }
