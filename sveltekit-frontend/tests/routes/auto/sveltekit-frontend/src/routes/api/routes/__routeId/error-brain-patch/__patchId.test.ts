 ...
 *
 * Route: src/routes/api/routes/[routeId]/error-brain-patch/[patchId]/+server.ts
 * Handlers: PUT
 *
 * G26 pattern: node env, vi.hoisted mocks (add as needed), lazy import in
 * beforeEach, 4 baseline cases per handler.
 *
 * Run:  npm run test -- src/routes/api/routes/__routeId/error-brain-patch/__patchId
 */
...
describe('src/routes/api/routes/[routeId]/error-brain-patch/[patchId]/+server.ts', () => {
  describe('PUT /src/routes/api/routes/:routeId/error-brain-patch/:patchId', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../../../../../../src/routes/api/routes/[routeId]/error-brain-patch/[patchId]/+server.js') as Record<string, unknown>;
      handler = mod.PUT as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/src/routes/api/routes/:routeId/error-brain-patch/:patchId', body !== undefined ? { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) } : { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
    }
    function makeUrl() { return new URL('http://localhost/src/routes/api/routes/:routeId/error-brain-patch/:patchId'); }
