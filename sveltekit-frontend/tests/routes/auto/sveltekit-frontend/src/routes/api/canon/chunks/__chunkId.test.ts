 ...
 *
 * Route: src/routes/api/canon/chunks/[chunkId]/+server.ts
 * Handlers: GET
 *
 * G26 pattern: node env, vi.hoisted mocks (add as needed), lazy import in
 * beforeEach, 4 baseline cases per handler.
 *
 * Run:  npm run test -- src/routes/api/canon/chunks/__chunkId
 */
...
describe('src/routes/api/canon/chunks/[chunkId]/+server.ts', () => {
  describe('GET /src/routes/api/canon/chunks/:chunkId', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../../../../../src/routes/api/canon/chunks/[chunkId]/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/src/routes/api/canon/chunks/:chunkId', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/src/routes/api/canon/chunks/:chunkId'); }
