...
describe('src/routes/.well-known/llms-full.txt/+server.ts', () => {
  describe('GET /.well-known/llms-full.txt', () => {
    ...
    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../src/routes/.well-known/llms-full.txt/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/.well-known/llms-full.txt', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/.well-known/llms-full.txt'); }
