 ...
 *
 * Route: src/routes/.well-known/appspecific/com.chrome.devtools.json/+server.ts
 * Handlers: GET
 *
 * G26 pattern: node env, vi.hoisted mocks (add as needed), lazy import in
 * beforeEach, 4 baseline cases per handler.
 *
 * Run:  npm run test -- .well-known/appspecific/com.chrome.devtools.json
 */
...
describe('src/routes/.well-known/appspecific/com.chrome.devtools.json/+server.ts', () => {
  describe('GET /.well-known/appspecific/com.chrome.devtools.json', () => {
    ...
    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../../src/routes/.well-known/appspecific/com.chrome.devtools.json/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/.well-known/appspecific/com.chrome.devtools.json', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/.well-known/appspecific/com.chrome.devtools.json'); }
