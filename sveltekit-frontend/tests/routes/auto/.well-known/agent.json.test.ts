...
describe('src/routes/.well-known/agent.json/+server.ts', () => {
  describe('GET /.well-known/agent.json', () => {
    let handler: (evt: { request: Request; locals: Record<string, unknown>; url: URL; params: Record<string, string> }) => Promise<Response>;

    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../src/routes/.well-known/agent.json/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/.well-known/agent.json', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/.well-known/agent.json'); }
