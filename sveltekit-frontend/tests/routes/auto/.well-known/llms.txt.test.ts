...
describe('src/routes/.well-known/llms.txt/+server.ts', () => {
  describe('GET /.well-known/llms.txt', () => {
    ...
    beforeEach(async () => {
      vi.resetAllMocks();
      const mod = await import('../../../../src/routes/.well-known/llms.txt/+server.js') as Record<string, unknown>;
      handler = mod.GET as typeof handler;
    });

    function makeReq(body?: unknown) {
      return new Request('http://localhost/.well-known/llms.txt', { method: 'GET' });
    }
    function makeUrl() { return new URL('http://localhost/.well-known/llms.txt'); }
