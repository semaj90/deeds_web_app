...
test.describe.serial('Route forensic: /persons-of-interest', () => {
	...
	test('GET /persons-of-interest — list or create CTA', async ({ page }) => {
		const log = await captureRouteLoad(page, '/persons-of-interest', { waitMs: 3000 });
		await page
			.locator('li, table tr, [data-testid*="poi"], article, a[href*="/persons-of-interest/create"], button')
			.first()
			.waitFor({ state: 'attached', timeout: 5000 })
			.catch(() => {});
		const rows = await page
			.locator('li, table tr, [data-testid*="poi"], article')
			.count();
		const createCTA = await page
			.locator('a[href*="/persons-of-interest/create"]')
			.or(page.locator('button').filter({ hasText: /create|new poi|new person/i }))
			.count();
