...
describe('resource-aware recommendation policy', () => {
  it('requires AST and exact promotion for file mutation', () => {
    const plan = selectRecommendationLanes(generous, defaultLaneEstimates({ queryKind: 'file_mutation' }));
  ...
  it('drops optional expensive lanes under a tight budget without dropping semantic', () => {
    const plan = selectRecommendationLanes(
      { ...generous, maxLatencyMs: 20, maxToolCalls: 0, maxContextTokens: 100, maxGpuBytes: 0, maxGraphHops: 0 },
  ...
  it('blocks mutation when a required proof lane cannot fit the envelope', () => {
    const plan = selectRecommendationLanes(
      { ...generous, maxLatencyMs: 25, maxToolCalls: 0 },
  ...
  it('derives mutation hints deterministically from tool key/value args', () => {
    const hints = inferRecommendationHintsFromToolArgs({
      operation: 'patch', filePath: 'src/lib/server/retrieval/search-runtime.ts', top_k: 36, max_hops: 2,
    });
  ...
  it('is deterministic for identical budgets and estimates', () => {
    ...
    expect(selectRecommendationLanes(generous, estimates)).toEqual(selectRecommendationLanes(generous, estimates));
  });
});
