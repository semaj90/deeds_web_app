...
| Gate | Status | Detail |
|------|--------|--------|
| G17 | ❌ FAIL | 1/204 files — use env.server.ts getters |

_Gates checked: G17. Run `npm run index:codebase:fast && npm run agents:write` to refresh._
## Todos + Enhancements

- **[HIGH]** [G17] Fix **G17** No hardcoded localhost URLs: 1/204 files — use env.server.ts getters
...
- **Paired tests**: 2/204 files have paired tests
