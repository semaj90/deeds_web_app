...
vi.mock('$lib/server/validation.js', () => ({
  isValidSafeId: vi.fn(
    (s: string) => typeof s === 'string' && s.length > 0 && /^[a-zA-Z0-9_-]+$/.test(s)
  ),
...
describe('/api/analytics/events (GET)', () => {
  ...
  it('returns 401 when unauthenticated', async () => {
    const res = await GET({
      url: mkUrl('/api/analytics/events'),
...
describe('/api/analytics/patterns (GET)', () => {
  ...
  it('returns 400 when userId missing', async () => {
    const res = await GET({
      url: mkUrl('/api/analytics/patterns'),
      request: new Request('http://localhost'),
      locals: authedLocals,
    });
    expect(res.status).toBe(400);
...
describe('/api/analytics/summary (GET)', () => {
  ...
  it('returns 400 when userId missing', async () => {
    const res = await GET({
      url: mkUrl('/api/analytics/summary'),
      request: new Request('http://localhost'),
      locals: authedLocals,
    });
    expect(res.status).toBe(400);
...
describe('/api/analytics/token-usage (GET)', () => {
  ...
  it('returns 401 when unauthenticated', async () => {
    // Route returns degraded response (200 with success: false) instead of throwing
    const res = await GET({
...
describe('/api/tags/search (GET)', () => {
  ...
  it('returns 401 when unauthenticated', async () => {
    const res = await GET({
      url: mkUrl('/api/tags/search', { q: 'contract' }),
      request: new Request('http://localhost'),
      locals: anonLocals,
    });
    expect(res.status).toBe(401);
...
describe('/api/user/preferences (GET/PATCH)', () => {
  ...
  it('GET returns 401 when unauthenticated', async () => {
    await expect(
      GET({ request: new Request('http://localhost'), locals: anonLocals })
    ).rejects.toThrow();
  });
  ...
  it('PATCH rejects empty update', async () => {
    await expect(PATCH({ locals: authedLocals, request: mkRequest({}) })).rejects.toThrow();
  });
