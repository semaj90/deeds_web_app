...
> Directory: sveltekit-frontend/scripts

## Snapshot

- 319 file(s), 0 handler(s)
- Audit score: _(no GPU audit)_
- 🟠 hardcoded localhost: 49 · TODOs: 2
...
- `agent-diagnose.mjs`
- `agentic-batch-fix.mjs`
- `agentic-error-fix.mjs`
- `agents-db-verify.mjs`
- `agents-md-regen.ts`
- `agents-smoke-all.mjs`
- `alter-evidence-type-enum.ts`
...
| Gate | Status | Detail |
|------|--------|--------|
| G17 | ❌ FAIL | 49/319 files — use env.server.ts getters |

_Gates checked: G17. Run `npm run index:codebase:fast && npm run agents:write` to refresh._
## Todos + Enhancements

- **[HIGH]** [G17] Fix **G17** No hardcoded localhost URLs: 49/319 files — use env.server.ts getters
...
- **Paired tests**: 1/319 files have paired tests
...
- `wiki_note_lookup({ query: "sveltekit-frontend scripts", limit: 5 })` — KAG narrative + audit score
- `audit_hotspots({ limit: 10 })` — if this dir is failing gates, surfaces the broader hotspot set
- `read_file({ filePath: "sveltekit-frontend/scripts/<file>" })` — fetch any file's contents (sandboxed to src/)
