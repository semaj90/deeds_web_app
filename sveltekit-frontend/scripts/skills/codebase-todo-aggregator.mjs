...
async function main() {
  ...
  const lines = [
    ...
    '## Provenance',
    '',
    `- Redis authority: \`HLEN ace:authority:top\` = ${Object.keys(redisSignals.authority).length} ${Object.keys(redisSignals.authority).length > 0 ? '' : '(optional mirror)'}`,
    `- Karpathy GPU:    \`HLEN gpu:karpathy:scores\` = ${Object.keys(redisSignals.karpathy).length} ${Object.keys(redisSignals.karpathy).length > 0 ? '' : '(optional mirror)'}`,
    `- Dirty files:     \`SMEMBERS ace:rank:dirty_files\` = ${redisSignals.dirty.length} ${redisSignals.dirty.length > 0 ? '' : '(startup mirror)'}`,
