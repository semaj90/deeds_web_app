const baseUrl = process.env.AGENT_OBSERVATION_URL ?? 'http://127.0.0.1:3030';
const url = new URL('/api/memory/agent-observation', baseUrl);
...
const payload = {
  ...
  sourceRefs: ['scripts/memory/post-agent-observation-smoke.mjs'],
  toolCalls: [{ tool: 'smoke', status: 'ok' }],
  rawJson: { smoke: true, note: 'agent-observation smoke' },
};
