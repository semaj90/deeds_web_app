...
const EMBEDDING_HTTP_PORT = parseInt(process.env.EMBEDDING_HTTP_PORT ?? '8097', 10);
const EMBEDDING_GRPC_PORT = parseInt(process.env.EMBEDDING_GRPC_PORT ?? '50051', 10);
...
async function isEmbeddingHealthy() {
  return httpHealthy(`http://127.0.0.1:${EMBEDDING_HTTP_PORT}/health`);
}

async function isRetrievalHealthy() {
  return httpHealthy(`http://${RETRIEVAL_HOST}:${RETRIEVAL_HTTP_PORT}/health`);
}
...
async function ensureEmbedding() {
  if (await isEmbeddingHealthy()) {
    console.log(c.green(`  ✓ Go embedding :${EMBEDDING_GRPC_PORT}/:${EMBEDDING_HTTP_PORT} already healthy`));
    return true;
  }

  const serviceDir = path.resolve(projectRoot, '..', 'services', 'go-embedding-service');
  console.log(c.dim(`  → spawning Go embedding service (${serviceDir})…`));

  const env = {
    ...process.env,
    HTTP_PORT: String(EMBEDDING_HTTP_PORT),
    GRPC_PORT: String(EMBEDDING_GRPC_PORT),
    REDIS_URL: process.env.VALKEY_URL ?? process.env.REDIS_URL ?? 'redis://127.0.0.1:6379',
  ...
  try {
    ...
    console.log(c.yellow(`  ○ Could not spawn Go embedding service: ${err.message}`));
    return false;
  }
  ...
  for (let i = 0; i < 60; i++) {
    ...
    if (await isEmbeddingHealthy()) {
      console.log(c.green(`  ✓ Go embedding :${EMBEDDING_GRPC_PORT}/:${EMBEDDING_HTTP_PORT} healthy`));
      return true;
    }
  }

  console.log(c.yellow(`  ○ Go embedding did not become healthy in 30s — retrieval may degrade`));
  return false;
}
...
async function ensureRetrieval() {
  ...
  const env = {
    ...process.env,
    DATABASE_URL: process.env.DATABASE_URL ?? 'postgresql://legal_admin:123456@127.0.0.1:5434/legal_ai_db',
    HTTP_PORT: String(RETRIEVAL_HTTP_PORT),
    GRPC_PORT: String(RETRIEVAL_GRPC_PORT),
    QDRANT_URL: process.env.QDRANT_URL ?? 'http://localhost:6333',
    REDIS_URL: process.env.VALKEY_URL ?? process.env.REDIS_URL ?? 'redis://:redis@127.0.0.1:6379',
    OLLAMA_URL: process.env.OLLAMA_URL ?? 'http://localhost:11434',
    EMBEDDING_BASE_URL: process.env.EMBEDDING_BASE_URL ?? `http://127.0.0.1:${EMBEDDING_HTTP_PORT}`,
...
const [turboOk, embeddingOk, retrievalOk] = await Promise.all([ensureTurbo(), ensureEmbedding(), ensureRetrieval()]);

console.log('');
if (turboOk && embeddingOk && retrievalOk) {
  console.log(c.green('gRPC stack ready — starting frontend'));
} else {
  const missing = [!turboOk && 'TurboQuant', !embeddingOk && 'Go embedding', !retrievalOk && 'Go retrieval'].filter(Boolean);
