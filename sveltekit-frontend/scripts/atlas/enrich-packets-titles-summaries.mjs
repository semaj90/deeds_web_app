...
function packetKeyToQdrantId(packet_key) {
  // Convert packet_key string to numeric ID for Qdrant (use stable hash)
  const hash = createHash('sha256').update(packet_key).digest();
  // Take first 8 bytes and convert to uint64
  let id = 0n;
  for (let i = 0; i < 8; i++) {
    id = (id << 8n) | BigInt(hash[i]);
  }
  // Convert to positive integer (mod 2^63-1 for Qdrant compatibility)
  return Number(id % 9223372036854775807n);
}
...
// ============================================================================
// STEP 4.5: Update Postgres with enriched metadata
// ============================================================================

async function updatePostgresMetadata(packets) {
  const pgPool = new postgres.Pool({ connectionString: DB_URL });
  const client = await pgPool.connect();
  let updatedCount = 0;
  ...
  try {
    for (const packet of packets) {
      try {
        ...
        metadata.summary = packet.summary;
        metadata.enriched_at = new Date().toISOString();

        const result = await client.query(
          `UPDATE atlas_packets
           SET metadata = $1, summary = $2, updated_at = NOW()
           WHERE packet_key = $3`,
          [JSON.stringify(metadata), packet.summary, packet.packet_key]
        );
        ...
        if (verbose) {
          console.warn(`[Postgres] Failed to update ${packet.packet_key}:`, err.message);
        }
      }
    }
  } finally {
...
async function upsertToQdrant(packets, embeddings) {
  ...
  for (let i = 0; i < packets.length; i += QDRANT_BATCH_SIZE) {
    ...
    const points = batch.map((packet, idx) => ({
      id: packetKeyToQdrantId(packet.packet_key),
      vector: {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (verbose) {
          console.warn(`[Qdrant] Upsert failed: ${res.status} ${res.statusText}`, errBody);
        } else {
...
async function main() {
  ...
  if (!isDryRun) {
    console.log('[ENRICH PACKETS: Step 4.5] Update Postgres metadata');
    const postgresUpdated = await updatePostgresMetadata(validatedPackets);
    if (verbose) {
      console.log(`[Postgres] Updated ${postgresUpdated} packets`);
    }
    ...
    metrics.postgres_updated = postgresUpdated;
