...
async function auditQdrantCohorts() {
  try {
    ...
    const sampleRes = await fetch(
      `${QDRANT_URL}/collections/codebase_chunks_768/points/scroll?limit=100`,
      {
        method: 'POST',
...
async function main() {
  ...
  try {
    ...
    if (apply) {
      ...
      console.log('[summary] Next: npm run atlas:phase-d:feature-cards');
    } else {
