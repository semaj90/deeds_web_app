...
const REPORT_JSON = path.join(REPORTS_DIR, 'turbovec-384-4bit-build-report.json');
const REPORT_MD = path.join(REPORTS_DIR, 'turbovec-384-4bit-build-report.md');
const MANIFEST_JSON = path.join(TMP_DIR, 'turbovec-384-4bit-index-manifest.json');

function renderMarkdown(report) {
  return [
    '# TurboVec 384 4-bit Build Report',
    '',
...
async function main() {
  ...
  console.log(`[turbovec-384-4bit] building index from ${config.collection}`);
  console.log(`[turbovec-384-4bit] limit=${LIMIT} dim=${config.dimension} bits=${config.bits} dryRun=${DRY_RUN}`);
  ...
  console.log(`[turbovec-384-4bit] accepted=${accepted.length} rejected=${rejected.length} durationMs=${durationMs}`);
  console.log(`[turbovec-384-4bit] manifest=${MANIFEST_JSON}`);
