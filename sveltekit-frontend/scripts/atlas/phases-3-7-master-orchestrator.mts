...
/**
 * Master Orchestrator: Phases 3-7 Complete Pipeline
 *
 * Chains all phases of the Atlas snapshot validation, GPU processing, and synthesis pipeline:
 ...
 * - Complete ACE packet assembly for agent reasoning
 *
 * Usage:
 * npx tsx scripts/atlas/phases-3-7-master-orchestrator.mts
 *
 * Output:
 * - master-pipeline-results/orchestration-report.json
 * - phase7-ace-results/ace-packets.ndjson
 * - phase7-ace-results/packet-audit.json
 * - phase7-ace-results/assembly-report.json
...
const phases = [
  ...
  },
  {
    ...
    description: 'Assemble unified ACE context packets for agent reasoning',
  },
];

function printBanner() {
  console.log('\n' + '='.repeat(80));
  console.log('MASTER ORCHESTRATOR: Atlas Phases 3-7 Complete Pipeline');
...
function printMasterSummary(report: MasterOrchestrationReport) {
  ...
  console.log(`  Phase 7 (ACE Assembly): 8 gates`);
  console.log(`  Total: 50 validation gates`);
