    ...
    if (process.argv.includes('--apply')) {
      ...
      for (const [query, scores] of Object.entries(allScores)) {
        for (const score of scores) {
          ...
          try {
            await client.query(`
              INSERT INTO packet_graph_scores (packet_id, source_ref, run_id, rrf_scores)
              VALUES ($1, $2, $3, $4)
              ON CONFLICT (packet_id) DO UPDATE SET rrf_scores = EXCLUDED.rrf_scores
            `, [
              score.packet_id,
              packet?.source_ref || 'unknown',
              runId,
              JSON.stringify({
                query, run_id: runId, timestamp: new Date().toISOString(),
                rrf_config: { k: RRF_K, weight_lexical: WEIGHT_LEXICAL, weight_vector: WEIGHT_VECTOR, weight_authority: WEIGHT_AUTHORITY },
                rrf_score: score.rrf_score, lexical_rank: score.lexical_rank, vector_rank: score.vector_rank, authority_rank: score.authority_rank,
                lexical_rrf: score.lexical_rrf, vector_rrf: score.vector_rrf, authority_rrf: score.authority_rrf,
                top_k_rank: score.top_k_rank, precision: 'fp32'
              })
            ]);
          } catch (e) {
