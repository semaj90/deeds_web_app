...
CREATE TABLE IF NOT EXISTS atlas_features (
  ...
  parent_feature_id uuid REFERENCES atlas_features(feature_id),
...
CREATE INDEX IF NOT EXISTS atlas_features_domain_status_idx
  ON atlas_features(domain, status);
CREATE INDEX IF NOT EXISTS atlas_features_revision_idx
  ON atlas_features(feature_revision);
CREATE INDEX IF NOT EXISTS atlas_features_metadata_gin_idx
  ON atlas_features USING gin(metadata jsonb_path_ops);

CREATE TABLE IF NOT EXISTS atlas_feature_aliases (
  feature_id uuid NOT NULL REFERENCES atlas_features(feature_id) ON DELETE CASCADE,
  alias text NOT NULL,
...
CREATE INDEX IF NOT EXISTS atlas_feature_aliases_normalized_idx
  ON atlas_feature_aliases(normalized_alias);
...
CREATE INDEX IF NOT EXISTS atlas_evidence_source_revision_idx
  ON atlas_evidence(source_ref, source_revision);
CREATE INDEX IF NOT EXISTS atlas_evidence_search_gin_idx
  ON atlas_evidence USING gin(search_vector);
CREATE INDEX IF NOT EXISTS atlas_evidence_payload_gin_idx
  ON atlas_evidence USING gin(payload jsonb_path_ops);
CREATE INDEX IF NOT EXISTS atlas_evidence_tags_gin_idx
  ON atlas_evidence USING gin(tags);

CREATE TABLE IF NOT EXISTS atlas_feature_evidence (
  feature_id uuid NOT NULL REFERENCES atlas_features(feature_id) ON DELETE CASCADE,
  evidence_id uuid NOT NULL REFERENCES atlas_evidence(evidence_id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS atlas_feature_evidence_evidence_idx
  ON atlas_feature_evidence(evidence_id, feature_id);
...
CREATE INDEX IF NOT EXISTS atlas_relationships_type_degree_idx
  ON atlas_relationships(relationship_type, relationship_degree);
CREATE INDEX IF NOT EXISTS atlas_relationships_revision_idx
  ON atlas_relationships(relationship_revision);
CREATE INDEX IF NOT EXISTS atlas_relationships_source_revision_idx
  ON atlas_relationships(source_ref, source_revision);
CREATE INDEX IF NOT EXISTS atlas_relationships_metadata_gin_idx
  ON atlas_relationships USING gin(metadata jsonb_path_ops);

CREATE TABLE IF NOT EXISTS atlas_relationship_members (
  relationship_id uuid NOT NULL REFERENCES atlas_relationships(relationship_id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS atlas_relationship_members_entity_idx
  ON atlas_relationship_members(entity_type, entity_id, relationship_id);
CREATE INDEX IF NOT EXISTS atlas_relationship_members_relationship_entity_idx
  ON atlas_relationship_members(relationship_id, entity_type, entity_id);
...
CREATE TABLE IF NOT EXISTS atlas_feature_state_receipts (
  ...
  satisfied_evidence uuid[] NOT NULL DEFAULT ARRAY[]::uuid[],
  blocking_evidence uuid[] NOT NULL DEFAULT ARRAY[]::uuid[],
...
CREATE TABLE IF NOT EXISTS atlas_dynamic_hyperedge_candidates (
  ...
  promoted_relationship_id uuid REFERENCES atlas_relationships(relationship_id),
  created_at timestamptz NOT NULL DEFAULT now()
);
CREATE INDEX IF NOT EXISTS atlas_dynamic_hyperedge_query_idx
  ON atlas_dynamic_hyperedge_candidates(query_id, relationship_type);
CREATE INDEX IF NOT EXISTS atlas_dynamic_hyperedge_participants_gin_idx
  ON atlas_dynamic_hyperedge_candidates USING gin(participants jsonb_path_ops);
...
CREATE OR REPLACE FUNCTION atlas_validate_relationship(p_relationship_id uuid)
RETURNS boolean
LANGUAGE sql
STABLE
AS $$
