...
Generated: 2026-05-05T13:59:31.191Z  
Clusters: 38 · members: 16626 · LLM hits: 0
...
This cluster provides comprehensive data modeling and services for a sophisticated legal or investigative platform, managing case metadata, digital evidence, knowledge graph construction, and advanced vector retrieval.

**Purpose:** Core Data Modeling and Knowledge Retrieval Layer

**Patterns:** Repository Pattern, Retrieval Augmented Generation (RAG), State Machine Pattern, Domain-Driven Design (DDD), Graph Modeling

**Warnings:** The use of `[key: string]: any;` in `CaseMetadata` weakens type safety and should be replaced with explicit property definitions.; The `EvidenceFile` interface is located in a `.svelte.ts` file, suggesting a potential mixing of concerns between data modeling and UI component logic.; Handling of `Date` objects (e.g., in `EscalationTicket`) requires careful serialization/deserialization logic (e.g., using ISO strings) to prevent runtime errors.

**Tags:** src/lib/types/database.ts, src/lib/stores/unified/evidence-store.svelte.ts, src/lib/machines/retrieval-machine.ts, src/lib/services/error-analysis/types.ts, src/lib/types/external-services.ts
