---
title: "Codebase Map"
generated: "2026-05-11T03:30:09.942Z"
fileCount: 500
clusterCount: 100
tags: ["index","codebase"]
---
...
```mermaid
graph LR
  C50["component chunks in `src/lib/components/ui/gaming/"]
  C21["component chunks in `src/lib/components/legal` (ta"]
  C70["route-handler chunks in `src/lib/server/analytics`"]
  C35["component chunks in `src/lib/components/legal-ai` "]
  C5["component chunks in `src/lib/components/ai` (tag: "]
  C72["function chunks in `src/lib/server/ace` (tag: vect"]
  C74["type chunks in `src/lib/types` (tag: vector)"]
  C57["const chunks in `src/lib/shims` (tag: embedding)"]
  C44["route-handler chunks in `src/lib/server/llm` (tag:"]
  C25["route-handler chunks in `src/lib/server` (tag: red"]
  C92["component chunks in `src/lib/components/evidence` "]
  C94["function chunks in `src/lib/server/cache` (tag: re"]
