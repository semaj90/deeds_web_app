import { enforceGatewayFlow } from '../gateway/flow-enforcer';
...
describe('Gateway Flow Enforcement', () => {
  ...
  it('should initialize state if no state exists for the message ID', async () => {
    ...
    expect(saveGatewayMessage).toHaveBeenCalledWith(expect.objectContaining({ id: mockMessagePayload.id, source: 'initial_request' }));
  ...
  it('should process and update state if state already exists', async () => {
    ...
    jest.spyOn(require('../gateway/flow-enforcer'), 'enforceGatewayFlow').mockImplementation(async (payload) => {
      // Simulate the internal processing logic for testing purposes
      await updateMessageStatus(payload.id, 'processed');
      await saveGatewayMessage({ id: payload.id, payload: { newState: { status: 'processed', data: 'processed_data' } }, source: 'gateway_flow' });
      return { success: true, result: { newState: { status: 'processed', data: 'processed_data' } }, error: null };
    });
    ...
    expect(saveGatewayMessage).toHaveBeenCalledWith(expect.objectContaining({ source: 'gateway_flow' }));
  });
});