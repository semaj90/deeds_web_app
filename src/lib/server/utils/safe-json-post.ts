...
async function readLimitedText(res: Response, maxBytes: number): Promise<string> {
  // Prefer streaming reader when available to avoid OOM on large responses
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const body: any = (res as any).body;
  if (!body || typeof body.getReader !== 'function') {
    // Fallback to text() for non-streaming environments
  ...
  while (true) {
    ...
    if (total > maxBytes) {
      // cancel reader to free resources
...
export async function safeJsonPost<T>(
  ...
  try {
    ...
    // Do not crash on circular or huge objects
  ...
  try {
    ...
    try {
      ...
      // Preview only — avoid logging full body
      // eslint-disable-next-line no-console
    ...
    if (!res.ok) {
      // Log a preview of the response only
      // eslint-disable-next-line no-console
