...
export const KEYWORD_DOMAINS: Record<Domain, string[]> = {
  auth: ['session', 'login', 'logout', 'authenticate', 'authorize', 'token', 'credential', 'password'],
  ui: ['button', 'component', 'render', 'state', 'effect', 'prop', 'modal', 'dialog'],
  retrieval: ['search', 'query', 'rank', 'score', 'candidate', 'embedding', 'vector', 'qdrant', 'rrf'],
  network: ['fetch', 'http', 'request', 'response', 'api', 'endpoint', 'server', 'client'],
  database: ['sql', 'postgres', 'drizzle', 'query', 'transaction', 'schema', 'migration'],
  cache: ['redis', 'valkey', 'cache', 'ttl', 'bifrost', 'bitfrost', 'memo'],
  agent: ['agent', 'tool', 'dispatcher', 'mcp', 'trace', 'workflow'],
  graph: ['neo4j', 'edge', 'node', 'pagerank', 'community', 'topology'],
  ml: ['xgboost', 'classifier', 'embedding', 'som', 'kmeans', 'tensor'],
  general: []
};
