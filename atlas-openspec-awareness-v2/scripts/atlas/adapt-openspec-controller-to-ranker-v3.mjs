...
for(const r of raw){
  const executionState=str(r.executionState??r.execution_state??r.controllerState??r.controller_state) ?? (source.includes('actionable-work')?'ACTIONABLE':null);
...
const output={schema:'atlas.actionable-workboard.v3',source,policy:{authority:'UPSTREAM_EXECUTION_CONTROLLER',laneAuthority:false,defaultMutationScope:'GLOBAL_SERIALIZATION_WHEN_UNSCOPED'},config:{maxWorkers:4,cpuCapacity:4,gpuCapacity:1,llamaSlotCapacity:2,dbWriterCapacity:1,warmAheadWaves:1,warmTopK:4},currentReceipts:[],tasks};
